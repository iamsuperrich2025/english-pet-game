...
## ✅ บั๊ก "ของขวัญโดนบัง" — แก้แล้ว (7 ก.ค. · Opus · commit จะตามด้านล่าง)
**ต้นตอจริง (ไม่ใช่ toast/z-index อย่างที่เดาไว้เดิมเลย):** กล่อง**ยืนยันส่งของขวัญ** (`confirmSendGift` → `askConfirm`) สร้าง `<div class="levelup-box">` แล้ว reuse markup `.ld-pic`/`.ld-name`/`.ld-note` — **แต่ CSS คุมขนาดรูป (`.ld-pic img{width:84px}`) ถูก scope ไว้เฉพาะ `.list-dialog`** (กล่องตั้งราคาขายใช้ `class="levelup-box list-dialog"` เลยรอด) · askConfirm ไม่มี class `list-dialog` → **รูปของขวัญ PNG 1024×1024 render เต็มขนาด** ดันกล่องยืนยันบวมเป็น "กล่องขาวสูงแนวตั้ง" บังทั้งจอ (ที่ผู้ใช้เห็นตอนปิดแผง picker)
- **วิธีแก้:** `css/style.css` เปลี่ยน scope กฎ `.ld-pic/.ld-pic img/.ld-name/.ld-note` จาก `.list-dialog` → `.levelup-box` (ครอบทั้งกล่องยืนยัน + กล่องตั้งราคา เพราะกล่องตั้งราคาก็มี `levelup-box`) — 4 บรรทัด
- **ยืนยันใน preview:** ก่อนแก้ รูป render 410px กล่องล้นจอ · หลังแก้ รูป 84px กล่องพอดีจอ (screenshot: การ์ดกะทัดรัด รูป+ชื่อ+ราคา+ปุ่ม ยกเลิก/ส่งเลย ครบ) ✅
- **หมายเหตุ:** commit `7dff74d` (ดัน `.toast` z-index 9990) ที่ทำรอบก่อน**ไม่เกี่ยว**กับบั๊กนี้เลย — toast ทำงานถูกอยู่แล้ว · แต่ปล่อยไว้ได้ (ไม่เสียหาย)

## 🎯 สถานะล่าสุด (7 ก.ค. 2026)
- **งานล่าสุด:** ✅ แก้บั๊ก "ของขวัญโดนบัง" (รอบสามสิบเอ็ด — รูปกล่องยืนยันไม่ถูกคุมขนาด) · ก่อนหน้า: ข้อ 0.5 ส่งของขวัญ + แผง emoji 7 หมวด + แยกปุ่มโรงงาน/ตลาด — push แล้วทั้งหมด
...
**✅ รอบสามสิบเอ็ด (7 ก.ค. · Opus): แก้บั๊ก "ของขวัญโดนบัง" สำเร็จ — เจอต้นตอจริง (ดูหัวข้อ ✅ บนสุด)** — รูปในกล่องยืนยัน `askConfirm` ไม่ถูกคุมขนาดเพราะ CSS scope ไว้เฉพาะ `.list-dialog` · แก้ `css/style.css` เปลี่ยน scope เป็น `.levelup-box` · ยืนยัน preview รูป 410px→84px กล่องพอดีจอ · version.json→2026-07-07.1

**⚠️ รอบสามสิบ (6 ก.ค. · Opus): พยายามแก้ toast โดนบัง — เดาผิดจุด (ที่จริงเป็นบั๊กรูปกล่องยืนยัน แก้แล้วรอบ 31)** — commit `7dff74d` ปรับ `.toast` z-index 95→9990 + bottom 30→76px (toast ทำงานถูกอยู่แล้ว ไม่เกี่ยวกับอาการจริง แต่ปล่อยไว้ได้)
