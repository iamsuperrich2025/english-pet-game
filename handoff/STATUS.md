...
## 🐞 บั๊กค้างต้องแก้ (งานถัดไป — ผู้ใช้ทดสอบจริง 2 บัญชีบน Pages 6 ก.ค.)
**อาการ:** เปิดบนคอม ให้ **hulk (Chrome) ส่งของขวัญให้ ครูรุต (Edge)** → มี "ข้อความอะไรสักอย่างเด้งขึ้นข้างล่างจอ แต่โดนบัง อ่านไม่ออก" (คาดว่าเป็น toast ยืนยันการส่ง / หรือ toast แจ้งของขวัญเข้าฝั่งผู้รับ)
- **ลองแก้แล้วแต่ผู้ใช้บอก "ยังไม่ได้ เหมือนเดิม"** (commit `7dff74d`): เดาว่า `.update-banner` (PWA แจ้งเวอร์ชันใหม่ z-index 120 ล่างกลาง) ทับ `.toast` (เดิม z95) → แก้ `.toast` เป็น z-index 9990 + bottom 76px + เงา (css/style.css) · **แต่ไม่หาย** → สมมติฐานนี้อาจผิด หรือ CSS ใหม่ยังไม่ deploy ตอนผู้ใช้เทสต์
- **สเต็ปแก้ต่อ (session ใหม่):**
  1. **ขอ screenshot จากผู้ใช้ให้เห็นชัดว่า "ข้อความที่โดนบัง" คืออะไร อยู่ตรงไหน อะไรมาบัง** (สำคัญสุด — ที่ผ่านมาเดาเอา ยังไม่เคยเห็นของจริง)
  2. เช็กว่า CSS ใหม่ deploy แล้วจริงไหม (hard refresh บน Pages · เปิด DevTools ดู `.toast` computed z-index=9990 bottom=76 · ดู `version.json` เป็น .4) — GitHub Pages build หน่วง 2–5 นาที ผู้ใช้อาจเทสต์ตอน cache เก่า
  3. พิจารณาว่า "ตัวที่บัง" อาจไม่ใช่ update-banner: อาจเป็น **กล่อง gift-pick-overlay ที่ยังไม่ถูกปิด** · **panel-box/panel-overlay** (แผงเพื่อนเปิดค้าง) · **ปุ่ม footer `.lobby-bottom`** · หรือข้อความนั้นไม่ใช่ toast แต่เป็น element อื่นที่ถูก panel-box ตัดขอบล่าง (overflow) — ตรวจ DOM ตอนเกิดจริงว่ามี element อะไรซ้อนแถวล่าง
  4. ทดสอบ 2 เบราว์เซอร์จริง (Chrome+Edge) หรือจำลอง 2 uid ใน preview เดียว (fake db แชร์กัน) ให้เห็น flow ส่ง→รับ ครบ แล้วดูว่า toast ทั้งฝั่งส่ง (`doSendGift`) และฝั่งรับ (`giftInWatch` "🎁 มีของขวัญส่งมาถึงหนู") โผล่ตรงไหน โดนอะไรบัง
- ไฟล์เกี่ยว: `css/style.css` (.toast/.update-banner) · `js/util.js` (`toast()`) · `js/ui.js` (`doSendGift`/`openGiftPicker`) · `js/online.js` (`giftInWatch`)
...
**⚠️ รอบสามสิบ (6 ก.ค. · Opus): พยายามแก้ toast โดนบัง — ยังไม่หาย (ดูหัวข้อ 🐞 บั๊กค้างบนสุด)** — commit `7dff74d` ปรับ `.toast` z-index 95→9990 + bottom 30→76px แต่ผู้ใช้ทดสอบจริงบอกยังเหมือนเดิม · version.json→.4
