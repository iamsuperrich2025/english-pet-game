...
1. **เฟส 2:** data การ์ดวิธีเล่น (object `INTRO` ในโซน ❓ ท้ายไฟล์) → `js/adv3d_intro.js` = `window.ADV3D_INTRO` (~ตัด 350-450 บรรทัด · ย้ายเฉพาะ data — ส่วน builder ที่มี `${}` คงไว้ไฟล์หลัก)
2. **เฟส 3:** โซน "Texture ตัวอักษร / emoji / ป้ายชื่อผู้เล่น" → `js/adv3d_tex.js` IIFE แยก expose `window.Adv3dTex` · ไฟล์หลักคง alias ชื่อเดิม 1 บรรทัด/ฟังก์ชัน จุดเรียกทั้งไฟล์ไม่ต้องแก้ (~330)
3. **เฟส 4 (ตรวจ coupling ก่อนลงมีด):** โซนเสียงที่ปัดน้ำฝน (รอบ 537) + หอบังคับการบิน ATC → `js/adv3d_sfx.js` (~550) — ถ้าพัวพัน closure เยอะเกิน ให้รายงานผู้ใช้แทนการฝืนผ่า
