...
### 📌 สรุปสถานะล่าสุด (23 ก.ค. · deploy `.495` · SW v165) — อ่านก่อน
- **รอบ 508:** 🎯📏 **กระสุน "ตก" ตามระยะ + ขีดวัดระยะในเลนส์ที่เล็งตามได้จริง** (ต่อยอดรอบ 504 ข้อ 3/3 — ครบทั้ง 3 ข้อแล้ว) — เพิ่มโซน `DROP` + `dropAt/dropRange/dropDir` + `layoutLadder()` ใน `js/invasion3d.js` · **ไม่แตะค่าล็อกปืนทั้งหมด** และไม่แตะ `BULLET_SPD_*` (รอบ 467)
  - **ต่อยอดของเดิม ไม่สร้างระบบซ้อน:** ขีดวัด = `b.md`+`u.mdl` ชุดเดิมของรอบ 464 (เดิมตำแหน่งตายตัว ไม่ผูกกับอะไร) → คำนวณจากมุมตกจริง + เปลี่ยนเลขกำกับเป็น "ระยะ ม." · ป้าย 📏 ระยะถึงเป้า (รอบ 464) ใช้คู่กัน = อ่านระยะ → ทาบขีดที่ตรง → ยิงโดน
  - **สูตร (จดไว้ใช้ซ้ำ):** `drop(s) = scale·½·g/v²·(s²−s·z)` (zero ที่ z → 0 พอดี · s<z → ลอยเหนือแนวเล็ง) · ตำแหน่งขีด `t=(drop/s)÷tan(ครึ่งมุมภาพในเลนส์)` → **รัศมีตัดกันหมด ขีดจึงถ่างตามกำลังขยายอย่างเดียว** · ค่าที่ตั้ง: `scale{r93:6,rifle:3}` `zero 100` ทั้งคู่ · marks r93 `[200,400,600,800]` rifle `[150,250,350,450]`
  - **🎚️ ทำไมต้องมี `scale`:** g จริงล้วน ๆ R93 ตกแค่ 4.8 ม. ที่ 800 ม. → **ขีดทั้งชุดกองที่ 51–53.5% ของวง** (แทบทับกากบาท) เล็งสูงไม่มีผลจริง → คูณเกมสเกลจนขีดไกลสุดอยู่ราว 75–80% ของวง
  - **ยืนยัน browser (โหลดไฟล์จริงหลังรีโหลด · วัดซ้ำทั้งชุดหลัง rebase ทับรอบ 507 ได้เลขเดิมเป๊ะ):** ขีด R93 (% ของวง) **4× 52.9/58.8/64.6/70.5 · 6× 53.8/61.3/68.9/76.4 · 8× 54.3/62.8/71.3/79.9** — ซูมแรงขีดถ่างจริง (ห่างจากกากบาทเป็น px: 4× 13→78 · 8× 25→157) · ไรเฟิล 2× 52.6/57.7/62.8/67.9 · **จอเตี้ย 812×375: ขีดครบ 4 ตัวทุกซูม อยู่ในวงเลนส์ ป้ายเลขไม่หลุดจอ และไม่ชนป้าย 📏 (88%)** · ขีดแรกอยู่ **ต่ำกว่ากากบาท 8–25 px = ไม่บังจุดเล็ง** · วิถีตกจริง: ยิงพื้นระยะ 327/318/315 ม. → ตกจริง **3.70/3.40/3.25 ม.** เทียบสูตร 3.78/3.55/3.45 (ต่างเพราะรอบวัดที่ 2 จุดตกใกล้เข้ามา — ตั้งใจ) · `DROP.on=false` → ขีดหาย 0 ตัว + วิถีเบี่ยง 0° · ยิงรวม **25 นัด** (ทุกซูม × ส่อง/ไม่ส่อง × เงยฟ้า/ก้มพื้น × 2 กระบอก) **ไม่มี error** · **ค่าล็อกไม่ขยับ** (`AIM_BY_GUN.r93 [−0.016,−0.018]` · ADS r93 `[0.012,−0.221,−1.378]/1.643`)
  - 🔧 จูนสด: `_t.setDrop({on,g,scale,zero,marks,maxPct,maxDeg})` · ดูค่า `_t.drop` (at/lad/dom) · `_t.dropAt(s)` · `_t.dropDrift()` (ระยะ+มุมเบี่ยงจากที่ยืนอยู่จริง)
  - 🪤 **กับดัก (จดไว้ กัน session หน้าเสียเวลา):** ① `dropDir` **ต้องวนซ้ำ 2 รอบ** — พอโค้งลงแล้วระยะถึงสิ่งที่จะโดนสั้นลงมาก เงยยิงฟ้าไม่โดนอะไร รอบแรกได้ระยะ = maxD 4000 → **ตก 795 ม. = พับลง 11°** (กระสุนดิ่งลงพื้นตรงหน้า) · มี `maxDeg:4` เป็นเพดานกันเคสสุดโต่ง (ต้องเกิน ~1,470 ม. ถึงชน = ไกลกว่าขีดไกลสุด) ② `_t.applyWeapon()` **ห้ามเรียกโดยไม่ใส่ชื่อปืน** — `weapon=undefined` แล้วพังทั้งโลก (ใช้ `applyWeapon('r93')`) ③ pane ไม่ compositing → screenshot ไม่ได้ ใช้ `getBoundingClientRect` อย่างเดียว (กฎทองข้อ 3)
