...
- **⚽🆕 โลก 3D ใหม่ "สนามฟุตบอล" (ผู้ใช้สั่ง 14 ก.ค. — งานใหญ่ แนะนำเปิด session ใหม่ทำ):** เพิ่มโหมด `soccer` ใน `adventure3d.js` (โครงเดียวกับ drive/heli/drone) — เตะบอลใส่ "แผ่นป้ายตัวอักษรอังกฤษ" ที่ลอยนิ่งกลางอากาศ ประกอบเป็นคำ = ได้เหรียญ (reuse `words`/`letters`/`inv`/`tryCompleteWords`/`completeWord`/`pickWords` เดิม + `M.reward`/`M.doneKey='soccerDone'`)
  - **สเปกผู้ใช้:** (1) สนามฟุตบอล 3D (2) ตั้งเตะบอลใส่ป้ายตัวอักษรลอยนิ่ง (3) **ปรับเบา-แรงด้วยน้ำหนัก/เวลากดปุ่มยิง** (charge meter: กดค้าง→พลังวิ่ง 0-100→ปล่อย=เตะ) (4) **ปุ่มซ้ายมือปรับมุมมอง/มุมเล็ง** (หมุน yaw + ก้ม-เงย pitch) (5) **เลือกสีเสื้อ + เบอร์หลังเสื้อ** (แผงก่อนเข้าเล่น แบบ carStart/pickBlockAvatar) (6) มุมมอง 1st/3rd person เลือกได้ตามสะดวก (แนะนำ 3rd person หลังบอล เห็นเสื้อ+เบอร์) (7) ประกอบคำครบ = เหรียญตามเหมาะสม (ใช้ M.reward เช่น 20)
  - **จุดต้องแก้ (ตามรอยโหมด drive):** `MODES.soccer{label,emoji:'⚽',reward,doneKey:'soccerDone',sky,ground,soccer:true,intro,hint,koTitle}` · `buildScene` เพิ่ม branch สร้างสนาม (พื้นเขียว+เส้นสนาม texture/mesh + ประตู goal + อัฒจันทร์ง่ายๆ + แสง) → `worlds.soccer` · `spawnLetter`+`relocateLetters` เพิ่ม branch วางป้ายลอยหน้าประตู (sprite ใหญ่ ~2.5 · หลายตำแหน่ง/ความสูง) · ฟิสิกส์บอล+เล็ง+charge = `tickSoccer(dt,now)` ใหม่ + เรียกใน `loop()`/`_t.step` · `start()` reset ตัวแปรบอล/charge + spawn บอลที่จุดโทษ · `buildDom` เพิ่ม HUD: power meter + ปุ่มเล็งซ้าย + ปุ่มยิงขวา (กดค้าง charge) · แผงเลือกเสื้อ/เบอร์ (เก็บ `state.soccerShirt`/`state.soccerNo`) โชว์ตัวผู้เล่น 3rd-person (เสื้อสีเลือก + เบอร์ที่หลัง canvas texture) · `INTRO.soccer` + ปุ่ม UI launch (ui.js `loadScriptOnce` แล้ว `Adventure3D.start('soccer')`) + ticket? (ดู state.*Ticket — อาจเปิดฟรีหรือเพิ่ม soccerTicket + ขายในร้าน)
  - **กฎเหล็ก reuse:** คำต้องตามระดับชั้น (`wordPool`/`vocabForStudent` ทำให้อยู่แล้ว) · การเก็บตัวอักษร = ตอนบอลชนป้าย (เช็คระยะ ball↔letter.spr ใน tickSoccer แล้ว `inv[ch]++`+`removeLetter`+`renderHudInv/Words`+`tryCompleteWords`) · **บอลชนป้าย≠ขับชน** — ต้องเขียน collision ใน tickSoccer เอง
  - **เทสต์:** ใช้ `Adventure3D._t.step(dt)` (rAF พักใน preview) + readPixels/getBoundingClientRect · ยิงจริงต้องจำลอง charge+release แล้วเดินฟิสิกส์บอลชนป้าย
- ── (งานค้างเดิมด้านล่าง) ──
