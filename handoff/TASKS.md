...
### 📌 สรุปสถานะล่าสุด
- รอบ 1598 · เกมพิมพ์คำศัพท์ (typing.js) ปรับรางวัลพิมพ์ครบ 1 คำ 10 → 100 เหรียญ: แก้ค่าคงที่ COIN_PER_WORD=100 (ป้าย +100 🪙 ใช้ค่าเดียวกันอยู่แล้ว) · ระบบคะแนนกระดานอันดับ (tpScore=ความยาว×4 + โบนัสไม่ผิด 10) และเข็มนักพิมพ์ไม่แตะ · syntax ผ่าน ไม่มี assertion ล็อกค่ารางวัลใน tools
- รอบ 1597 · Picture Dictionary ปรับรางวัลฟังคำในการ์ดซูม (zoom listen) จาก 1 เป็น 100 เหรียญ: js/picdict.js — ค่าคงที่ ZOOM_REWARD=100 ใช้ร่วมกันทั้ง addCoins/coinFlyFx/ป้าย "+100 🪙" และข้อความ "ได้รับ 100 เหรียญ" · รางวัลโหมดครูถาม (10 เหรียญ/คำ) ไม่แตะ · ผ่าน test_picdict_single_page.js (ไม่มี assertion ล็อกค่ารางวัล)
- รอบ 1596 · Vocab Force กดปัดพลังตอนลูกเพื่อนใกล้ตัว → ไม่เสีย HP: เดิมลูกเพื่อนตรวจ hit ฝั่งคนยิงแล้วแพ็ก strike 'G' มาให้ takeHit เสมอ แม้เราปัดสวนแล้ว → ทำ peer guard: hostile-orb-manager ตราเวลาลูก peer ผ่านใกล้ตัว (_peerNearAt รัศมี PEER_NEAR 2.4 — จำเป็นเพราะลูกเพื่อนวิ่ง ~420 หน่วย/วิ ผ่านรัศมีปัดในเสี้ยววิ จับที่ hitAt 0.26 อย่างเดียวไม่ทัน) · deflect-controller จำเวลากด (_lastDeflectAt) + เวลาปัดโดนลูกเพื่อน (_peerCatchAt) · peerGuardActive(now, orbs) = อยู่ในจังหวะท่า (กดถึง hitAt+0.4 วิ) และมีลูกเพื่อนใกล้/ถูกปัดใน ±หน้าต่าง PEER_GUARD_MS 1 วิ → runtime กลืน strike 'G' นั้น (toast '✋ ปัดสวน! ไม่เจ็บ') · ui.js ?v=1597
