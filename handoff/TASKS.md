...
**ผู้ใช้เคาะแล้ว (7 ก.ค.): ทำ "ข้อ 8 โลกผจญภัย 3D" เป็นงานถัดไป** — สเปกเต็ม 8.1–8.6 ท้าย `handoff/BACKLOG.md` (กลุ่ม D)
- **First-person WASD + mouse look** เก็บตัวอักษรอังกฤษกระจายใน map มาประกอบคำ (guideline 10 คำ ตรงระดับชั้น `vocabForStudent()` ไม่ซ้ำคำที่ทำแล้ว) · minimap เรดาร์ · monsters สุ่มเกิด พลังหมด→ออก Lobby จ่ายรักษา 1,000 · ปุ่มออกชัดเจน · เหรียญมากกว่าเกมจับคู่
- **ประตูเข้า = `state.advTicket`** (การ์ดตั๋วรอบ 37 ขายอยู่ในแผงร้านค้าแล้ว — แทนที่ป้าย 🚧 "กำลังก่อสร้าง" ใน `renderTicketCard` ด้วยปุ่มเข้าโลก)
- ⚠️ **ต้องเคาะ engine กับผู้ใช้ก่อนเริ่มเขียน:** เว็บ static ไม่มี Node/bundler — ทางเลือก (ก) Three.js ฝังไฟล์ vendor ใน repo (แนะนำ: PWA offline ได้ ไม่พึ่ง CDN) (ข) CDN (ค) WebGL ดิบ/Canvas 2.5D · โหลดเฉพาะตอนเข้าโหมด 3D (dynamic script) กันหน้าหลักหนัก · มือถือต้องมี touch control ด้วย (เกมหลักเล่นบนมือถือ landscape)
- 🖼️ ภาพชุดตัวละคร/รูปร่างครบ 11/11 แล้ว (รอบ 39) — ระบบพิษ/รูปร่าง/ตัวละคร/ตั๋ว จบสมบูรณ์ เหลือผู้ใช้ทดสอบจริงบน Pages (.12–.17)
