...
- 🐾 **สไปรต์น้องเดิน — ต่อยอด (แมวเสร็จแล้ว รอบ 239 · deploy .229):** ทำเพิ่มได้เลยตามสูตรเดิม
  - **หมา** (`pet_dog.glb` ไม่มี rig เหมือนแมว → ผู้ใช้ต้องพา Tripo: Animate → Quadruped → Walk → Export GLB **เปิด "Animation stay in Place"** สำคัญสุด ไม่งั้นแมว/หมาเดินหลุดกรอบ) · **มังกร** มีท่าติดมาแล้ว 1 ท่า อบได้ทันทีไม่ต้องแตะ Tripo
  - ท่าตามสถานะ (หิว/ดีใจ/ป่วย/หลับ) — `state.js` มี `p.sick/p.sleeping/hunger` ครบแล้ว · `PET_ANIM` ใน ui.js รองรับแค่ 1 ท่าต่อสัตว์ ต้องขยายเป็น map ต่อสถานะ
  - **สูตรที่ใช้ได้ผลกับแมว:** `bake({model,clipIndex:0,frames:24,cell:320,yaw:-2.35,pad:1.7})` → `pack_anim.py --cols 6 --frames 24 --fps 14` · **yaw หามุมด้วย `sweep:true`** (กวาด 360° ในภาพเดียว) อย่าเดาทีละมุมให้เสียเวลา
