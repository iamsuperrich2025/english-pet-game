...
> ⏳ **รอ deploy อยู่ในไฟล์ (ยังไม่ commit) — 📣 ทหารตะโกนบอกทิศศัตรู (ข้อ 1 ด้านบน ทำเสร็จแล้ว 22 ก.ค.)**
> โค้ดอยู่ใน `js/invasion3d.js` โซน `📣 ทหารฝ่ายเราตะโกนบอกทิศศัตรู` + `Snd.shout` + hooks ใน `_t` (`bearingKey/threats/shouts/resetCalls`) — **ทดสอบผ่านครบแล้ว** (8 ทิศตรงกับ yaw · ป้ายลอยหัว 1.9 วิ · ในเฮลิ = เห็นป้ายแต่ไม่มีเสียง)
> ❗ ไม่ commit เพราะ **session "โหมดกลางคืน 🌙" กำลังแก้ไฟล์เดียวกันอยู่** (ผู้ใช้สั่งให้รอ) → ใครทำกลางคืนเสร็จ **ให้ commit/บัมพ์ version/deploy ไปพร้อมกันทีเดียว** แล้วลบโน้ตนี้ทิ้ง

### 📌 สรุปสถานะล่าสุด (22 ก.ค. · deploy `.458` · SW v165) — อ่านก่อน
