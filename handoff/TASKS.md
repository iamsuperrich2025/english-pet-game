...
- **รอบ 1291 · Home V2 R23 (Admin Preview เท่านั้น):** เปลี่ยน dock/flyout สีเข้มแบบ HUD ผู้ใหญ่เป็นของเล่นแฟนตาซีพาสเทล 4 สี ตัวอักษรเข้มใหญ่ขึ้น และซ่อนคำโปรยจิ๋วเพื่อลดความรก
- ต้นตอข้อความซ้อนคือ skin เก่าบังคับ `.vw2-mode>b` กว้าง 100%; R23 แยกไอคอน/ชื่อด้วย grid ชัดเจน ล้าง background asset เก่า และให้ชื่อยาวขึ้นสองบรรทัดได้โดยไม่ทับไอคอน
- เส้นทางเดิมครบ 13 action/11 โลกและ `isAdmin() === true` ไม่เปลี่ยน; แก้ `css/home-v2.css`, `js/home-v2.js`, `index_classic.html`, regression test และไม่แตะ `css/petshopping3d.css` เก่า
- syntax/regression/build `.1161` ผ่าน; พรีวิว 1323×622 และ 812×375 ยืนยัน icon-text overlap=0, ปุ่มไม่หลุดแผง และเมนูอยู่ใน viewport
