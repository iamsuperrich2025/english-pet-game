...
- เพิ่ม cooldown 16 คำข้ามจุดวนคลัง; regression F1 13 ชุด + build/validator + Browser QA ครบ 5 ระดับที่ 1280×720/844×390 ผ่าน, console 0 error (`tools/test_f1_vocab.js`, `index_classic.html`, `tools/build_web.mjs`)
- **รอบ 1220 · ปิดคำเชิญแล้วไม่เตือนเดิมซ้ำ:** บันทึกลายนิ้วมือ `ผู้ชวน|โลก|เวลา` ลงเซฟเมื่อกด ✕/ปิด จึงไม่เด้งคำเชิญเดิมอีกหลัง reload แต่คำเชิญใหม่จากคนเดิมยังแจ้งได้ (`js/online.js`, `js/state.js`, `js/util.js`)
