...
## ⏬ ย้ายเมื่อ 2026-07-16 — จาก handoff/TASKS.md (รายละเอียดรอบ)

### ✅ รอบ 232 (14 ก.ค.) — 🚗 รถแต่ละคันมีสมรรถนะของตัวเอง (ป้ายในโชว์รูม + ผูกการขับจริง) (version .222 · SW v17)
ผู้ใช้: "เลือกรถคันไหน ก็เอาฟังก์ชันของคันนั้นมาใช้ เหมือนเลือกหุ่นออกรบ" · เลือก (AskUserQuestion) = **ป้ายบอกสมรรถนะในโชว์รูม**
- **ก่อนหน้า:** console+พวงมาลัยเปลี่ยนตามคันแล้ว (รอบ 230) แต่รถ 10 คันต่างแค่ชื่อ/สี/ราคา · **การขับเหมือนกันหมด** (ต่างจากหุ่นที่ `MECHA_WEAPONS` อาวุธต่างจริง)
- **items.js `CARS` +stat ต่อคัน:** `spd`(ความเร็ว) `acc`(อัตราเร่ง) `grip`(เกาะถนน) 1-5 · คันแพงเก่งขึ้นแต่มี**คาแรกเตอร์** (car_01 แดง spd4/grip2=ซิ่งคุมยาก · car_03 เขียว spd2/grip4=นุ่มเกาะถนน · car_07 เหลือง spd5=เร็วสุด · car_10 all5=ท็อป)
- **ป้ายในโชว์รูม (ui.js `carStatHtml`):** 3 แถบ 5 ขีด (⚡ความเร็ว/🚀อัตราเร่ง/🛞เกาะถนน) สีตามคัน · ใส่ใน `csShowBig` info ก่อนปุ่มซื้อ · css `.cs-stats/.cs-stat/.cs-stat-bar` (lobby.css)
- **ผูกการขับจริง (adventure3d.js · ป้ายไม่หลอก):** `drivePerf{vmaxMul,accMul,steerMul}` ตั้งตอน start(drive) จาก `carInfo(myCar().id)` stat · `vmaxMul=0.82+spd*0.045` · `accMul=0.82+acc*0.045` · `steerMul=0.90+grip*0.025` · ใช้ใน tickDrive (vmax·CAR_ACCEL·CAR_STEER_MAX) · ต่างเบาๆ (feel ใกล้เดิม ±7%)
- **ยืนยัน (browser):** ป้าย car_07 = ความเร็ว5/อัตราเร่ง4/เกาะถนน3 ตรง stat · drivePerf: car_03(spd2) topspeed 182 / car_07(spd5) 209 / car_10(all5) 209+acc1.04+steer1.02 กม/ชม — ต่างตามคัน · ไม่มี error · deploy live .222 · ⚠️ SW cache เก่า (items.js static) → เคลียร์ SW+reload ตอนเทส · screenshot ค้าง (capture tool)
- ⚠️ **ค้าง: ผู้ใช้ลองจริง** (ขับหลายคันดูความต่าง + ดูป้ายโชว์รูม) · 💡 ต่อได้: เสียงเครื่องต่างคัน (option ที่ยังไม่เลือก · ต้องมีไฟล์ engine_<id>.mp3) · ป้าย stat บน thumb ด้วย

### ✅ รอบ 231 (14 ก.ค.) — 🚗 fix ผู้ใช้: dash ชุดใหม่ยังติดกระจก(ถนนวาดในภาพ) + สูงบังทาง (version .221 · SW v16)
ผู้ใช้ลองจริงมือถือ (car_03 เขียว): "กระจกตัดสูงไป · ภาพถนนนิ่งๆ ยังติดไปกับ console บังมองไม่เห็นทาง"
- **ต้นตอ:** รอบ 230 ตั้ง `object-position 56%` — **น้อยกว่าเดิม(66%) = เลื่อนขึ้นโชว์กระจกมากขึ้น (ผิดทิศ!)** · % สูง=โชว์ล่างภาพ · ภาพ dash ทุกคันมีกระจกหน้า(ท้องฟ้า+ถนนวาด)ครึ่งบน แดชบอร์ดล่าง 60% · 56% ทำให้เห็นกระจกล่างติดมา
- **วิเคราะห์ (PIL montage 10 คัน):** กระจกจบที่ **~40% ของภาพ**สม่ำเสมอทุกคัน (car_02 ฟ้า มากสุด 40%) · แดชบอร์ด y400-820 · เบาะ 820+
- **แก้ (พิสูจน์ composite car_03+car_02 mobile 1311×630):** (1) `object-position 56%→65%` (crop เริ่ม y~446 > กระจกจบ 416 = **ตัดกระจกหมดทุกคัน**) (2) `max-height 54vh→46vh` (console เตี้ยลง เห็นฉาก 3D จริง 54%บน ไม่บังทาง) (3) พวงมาลัย `60vh→50vh · bottom -4vh→-1vh · left 75%→76%` (เตี้ยลงให้เกจอยู่ในกรอบแผงหน้าปัด ไม่ลอยบนถนน) (4) `RADIO_RECT→[560,514,835,606]` ตรงจอ head-unit (5) `offY .56→.65` (radioLayout+bobbleTick ตรง object-position ใหม่)
- **ยืนยัน (browser car_03 · 1311×630):** dash 46vh top54% (เห็นทางโล่ง) · crop ตัดกระจก · wheel 50vh aspect1.50 · radio บนแผง · bobble เท้าบนแผงซ้าย · ไม่มี error · deploy live .221 · ⚠️ screenshot ค้าง → composite PIL + getBoundingClientRect ยืนยัน
- ⚠️ **ค้าง: ผู้ใช้ลองจริงมือถืออีกครั้ง** (ยืนยันกระจกหายหมด + เห็นทาง)

### ✅ รอบ 230 (14 ก.ค.) — 🚗 โลกขับรถ: คอนโซล+พวงมาลัยชุดใหม่ (แยกภาพ · จัดวางมืออาชีพ) (version .220 · SW v15)
ผู้ใช้: console+พวงมาลัยเดิมขยับดูไม่โปร (พวงมาลัยวาดติดในภาพ dash → ซ้อนพวงมาลัยแยกที่หมุน) · เจนภาพใหม่แยก dash/wheel วางที่ `img/3d_car/` (3d_dash_car_01..10 · 3d_wheel_01..10) ขอใช้แทน + จัดวางสวยงาม
- **ต้นตอ "ไม่โปร":** ภาพ dash เก่า (`img/car/dash_<id>.png`) วาดพวงมาลัยติดในภาพ (ขวา) + เกจในภาพ → เกมมีพวงมาลัยแยก (wheel.png อันเดียว) หมุนทับ = เห็นพวงมาลัย 2 อัน/ขยับไม่ตรง · ภาพชุดใหม่แยก: **dash สมมาตร ไม่มีพวงมาลัย/เกจในภาพ** + **wheel โปร่งใสต่อคัน** (aspect ~1.5 ไม่ใช่ square!)
- **loader ต่อคัน (adventure3d.js):** `loadCarDash` ลอง `img/3d_car/3d_dash_<cid>.png` ก่อน → fallback `img/car/dash_<cid>.png` → `dash.png` → CSS · เพิ่ม `loadCarWheel()` โหลด `img/3d_car/3d_wheel_<NN>.png` (cid 'car_03'→'03') → fallback wheel.png → วง CSS (เดิมโหลด wheel.png ครั้งเดียวตอน build → ย้ายเป็นต่อคัน เรียกใน loadCarDash + start)
- **จัดวาง (พิสูจน์ด้วย PIL composite viewport จริง desktop+mobile ก่อนใส่โค้ด):** (1) **dash** `max-height 42vh→54vh` · `object-position 66%→56%` = แดชบอร์ดเต็มครึ่งล่าง เห็นคานหน้า+จอ+ช่องแอร์ (ตัดกระจกบน=ฉาก 3D) (2) **wheel** เปลี่ยนจาก `width:min(44vh,50vw);aspect-ratio:1`(บีบบิด!) → `height:min(60vh,60vw);aspect-ratio:1.5;width:auto` (คงสัดส่วนภาพ 1536×1024 ไม่บิด) · `left:76.5%→75%` · `bottom:-15vh→-4vh` (โผล่จากขอบล่างขวาแบบมองจากที่นั่งคนขับ เห็นเกือบเต็มวง) (3) **เกจ** `drawCarGauges` เปลี่ยน map จากพิกัดภาพ dash → **อิงตำแหน่งพวงมาลัย** (`offsetLeft/Top` ไม่รวม transform → พวงมาลัยหมุนเกจไม่สั่น) วาด 2 วงลอดช่องบนพวงมาลัย (z-gauge:3 < z-wheel:4 = พวงมาลัยทับบางส่วนสมจริง) · `drawCarDial` เพิ่ม**จานเกจ**(วงเข้ม+ขอบเงิน) เพราะ dash ใหม่ไม่มีวงในภาพ (4) **RADIO_RECT** `[622,682,806,780]→[556,486,838,598]` (จอ head-unit ใหม่) (5) **BOBBLE_FOOT** `[542,596]→[318,540]` (แผงเรียบซ้าย) (6) **offY object-position** 2 จุด (radioLayout+bobbleTick) `.66→.56` ตรง dash ใหม่
- **ยืนยัน (browser drive จริง · car_01+car_03):** dash/wheel โหลด `3d_car/3d_*_<id>` ต่อคัน · dash 54vh · wheel aspect **1.500 ไม่บิด** ตำแหน่งตรง composite (cx975/top264) · เกจ 2 วงวาดจริง (สีจานถูก · ลอดพวงมาลัย) · radio 471,383 ตรงจอเป๊ะ · bobble เท้าบนแผงซ้าย · ไม่มี console error · deploy live .220 · ⚠️ screenshot ค้าง (เมือง 3D หนัก · ยืนยันด้วย PIL composite + getBoundingClientRect ตามกฎทอง #3)
- ⚠️ **ค้าง: ลองจริงมือถือ** (ดูฟีลพวงมาลัยหมุน+เกจลอด) · ภาพต้นฉบับใหญ่ **38MB รวม** (dash 1.6MB + wheel 2.2MB/คัน · โหลดเฉพาะคันที่ขับ ~3.8MB) — 💡 เสนอย่อได้ (dash→JPG, wheel PNG optimize) ถ้าผู้ใช้อนุญาต · ภาพเก่า `img/car/dash_*`+`wheel.png` เก็บไว้เป็น fallback
...
## ⏬ ย้ายเมื่อ 2026-07-16 — จาก handoff/TASKS.md (## ประวัติรอบล่าสุด)
...
<!-- 🗄️ ย้ายจาก TASKS.md ตอนจบ session 21 ก.ค. (รอบ 412–445) -->
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.433` · SW v160) — อ่านก่อน
- **รอบ 445:** 🎯 **เทียบภาพเกมจริง (CoD Mobile) กับของเรา แล้วแก้การจัดวางปืน (ผู้ใช้สั่ง)** — **ต้นตอที่ทำให้ต่างกันลิบ: ครึ่งท้ายปืนอยู่ "หลังกล้อง"** (วัดได้ระยะติดลบ −0.10 ถึง −0.45 ม.) → โครงปืน/กล้อง/พานท้ายซึ่งเป็นส่วนที่สวยที่สุดถูกตัดทิ้ง เหลือให้เห็นแค่ลำกล้องเรียวๆ พาดจอ · **ทางแก้สวนสัญชาตญาณ: ต้อง "ย่อปืนลง + ดึงเข้ามาใกล้" ไม่ใช่ขยาย** (ยิ่งขยายยิ่งดันท้ายปืนไปหลังกล้อง) → `GUN_SCALE 1.15→.68 · GUN_POS[.22,−.18,−.62] · GUN_ROT[.02,.07,.13]`
  - **ผลวัด:** R93 ท้ายปืนห่างตา **0.21 ม.** · ปากกระบอก **1.03 ม.** (ลำกล้องพุ่งเข้าจอ 0.82 ม. = เห็นแบบ 3/4 จากหลัง-ขวาเหมือนภาพอ้างอิง) · พานท้ายไหลออกมุมขวาล่าง (0.97,−1.29) · **เป้าเล็งโล่ง** ✓ · ไรเฟิลก็เข้าเกณฑ์เดียวกัน ✓ · `ADS_POS/ADS_SCALE` ปรับตาม (ส่องกล้องแล้วท้ายปืนไม่หลุดหลังกล้อง · ทุกจุด x=0.00)
  - **🐛 เจอบั๊กซ้อน:** `forceGunForward` (รอบ 440) วัดใน "พิกัดของ obj เอง" ซึ่งไม่รวม rotation ของ obj → **โมเดลที่ `orientGunModel` จัดถูกแล้วโดนหมุนซ้ำอีก 90° ทำให้ไรเฟิลวางขวางจอ** (วัดได้แกนยาว→(0.99,0.13,−0.07)) · แก้เป็นวัดในพิกัดพ่อ + วัดซ้ำหลังหมุนทุกครั้ง
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.432` · SW v159) — อ่านก่อน
- **รอบ 444:** 🤚 **แก้ "นี่ไม่ใช่มือ นี่มันกระบอก" (ผู้ใช้ส่งภาพ)** — **ต้นตอ: ท่อนแขน** พุ่งเข้าหากล้อง (ใกล้แค่ 0.24–0.48 ม. ขณะที่ปืนอยู่ ~1 ม.) → เพอร์สเปกทีฟขยายจนกลายเป็น **ท่อยักษ์พาดจอ** · **แก้: ตัดท่อนแขนทิ้ง** เหลือ ฝ่ามือ + 4 นิ้วโอบ + นิ้วโป้ง + ข้อมือสั้น (มุมกล้องแบบนี้เกมจริงก็เห็นแค่ถุงมือเหมือนกัน) · สีถุงมือ/ปลอกแขนเข้มขึ้น (โอลีฟอ่อนเดิมเจอไฟ view model แล้วออก **เหลืองซีดเหมือนท่อพลาสติก**)
  - **🐛 อีกจุด:** โค้ดสมมติว่า "ลำกล้อง = แกน Z" ของทุกโมเดล → ไรเฟิลวางแกนต่างกัน มือเลยเล็กจิ๋ว 4 ซม. · แก้เป็น **หาแกนเอง** (ยาวสุด=ลำกล้อง · บางสุด=ด้านข้าง · ที่เหลือ=แนวตั้ง · ปากกระบอก=ฝั่งตรงข้ามมวล) แล้วประกอบมือด้วย `makeBasis`
  - **ยืนยัน browser:** มือ 19–24 ซม. เทียบปืน 118–161 ซม. (สัดส่วนคนจริง ~15%) ✓ **กรอบมืออยู่ในแนวตัวปืนทั้ง 3 สถานะ** (R93/ส่องกล้อง/ไรเฟิล) ✓ ใกล้กล้องสุด 0.54–0.79 ม. (ไม่บังจอ) ✓ ไม่มี error
  - **⚠️ ยังไม่มีใครเห็นภาพจริง** — เครื่องมือ screenshot ของ Claude ใช้ไม่ได้ ทุกอย่างวัดเป็นตัวเลข · ให้ผู้ใช้ดูแล้วบอก จูนที่ `grip`(ตำแหน่ง) · `R`(ขนาดมือ) · `HAND_GLOVE`(สี)
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.431` · SW v158) — อ่านก่อน
- **รอบ 443:** 🤚 **มือซ้ายประคองลำกล้อง (ผู้ใช้: "ของเดิมไม่เนียนมากๆ ทำยังไงให้เหมือน Delta Force")** — **4 หลักที่ทำให้เนียน จดไว้ในโค้ดแล้ว:** ① **มือเป็นลูกของ "โมเดลปืน" ไม่ใช่ของกล้อง** → ตามรีคอยล์/สวิง/ยกเล็งเป๊ะทุกเฟรม (ของเดิมติดกล้อง = ปืนขยับแต่มือไม่ตาม) ② **จุดจับคำนวณจากกรอบโมเดลปืนเอง** (65% ไปทางปากกระบอก ใต้ลำกล้อง) → ใช้ได้ทุกกระบอกโดยไม่ต้องตั้งค่าใหม่ ③ **ครบชุดชิ้นส่วน** ฝ่ามือ + 4 นิ้วโอบ + นิ้วโป้ง + ข้อมือ + ปลอกแขนเข้ม + ปลายแขน (เดิมเป็นท่อกลมสีเดียว = อ่านเป็นท่อ) ④ **ปลายแขนยาวทะลุขอบจอ** ไม่มีรอยตัดกลางอากาศ + นิ้วล้ำทับลำกล้องให้เกิดการบัง = สมองอ่านว่า "จับจริง"
  - **🐛 ที่เจอ:** รอบแรกวัดกรอบปืนด้วย `Box3.setFromObject` (พิกัดโลก) แล้วเอาไปใส่เป็นตำแหน่งลูก → **มือหลุดไปใต้จอ (y=−1.35)** → ต้องวัดในพิกัดของตัวปืนเอง (คูณ inverse matrixWorld) ก่อน
  - **ยืนยัน browser:** ฝ่ามืออยู่ระหว่างปากกระบอกกับกลางลำ (= ตรงการ์ดมือ) **ทั้ง R93 ท่าถือ (0.20,−0.55) · ตอนส่องกล้อง (−0.01,−0.29) · ไรเฟิล (0.44,−0.85)** ✓ ปลายแขนทะลุขอบจอทุกท่า ✓ มือแค่ 200 tris ✓ ไม่มี error
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.430` · SW v157) — อ่านก่อน
- **รอบ 442:** 🔫 **ขยายปืนที่ถือให้ยาว-ใหญ่ขึ้น (ผู้ใช้สั่ง)** — `GUN_SCALE .78→1.15` + ขยับ `GUN_POS[.34,−.34,−.58] · GUN_ROT[.04,.18,.16]` → **ความยาวปืนบนจอ 0.66 → 1.18 หน่วย NDC (≈59% ของความกว้างจอ = ใหญ่ขึ้น 1.8 เท่า)** พาดเฉียงเต็มมุมขวาล่าง · ปากกระบอก (0.12,−0.36) ไม่บังเป้าเล็ง · ยอดกล้องส่อง (0.19,0.06) เห็นเต็ม · ⚠️ ต้องดัน `ADS_SCALE 1.06→1.32` ตามด้วย **ไม่งั้นยกปืนเล็งแล้วปืนหดเล็กลง** (ADS_SCALE ต้อง > GUN_SCALE เสมอ) · ส่องกล้องยังเล็งกลางจอเป๊ะ (x=0.00 ทุกจุด)
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.429` · SW v156) — อ่านก่อน
- **รอบ 441:** 🛸 **เอา "แผ่นสี่เหลี่ยมดำ" ที่บังยานแม่ออก + ขยายลำเป็น 1,040 ม. (ผู้ใช้สั่ง)** — ① **ตัวการคือ `msPlate`** = แผ่นหลังแถวตัวอักษร `BoxGeometry(480×78×30)` สีเกือบดำ ลอยห่างผู้เล่นแค่ ~220 ม. → กินจอเป็นแถบดำยาวบังลำยานที่อยู่ไกลกว่า → **ลบถาวร** เหลือเฉพาะตัวอักษรเรืองแสง (ย่อช่องละ 60→42 ม. · ถอยไป `BOARD_Z −95` · วัดแล้วอ่านออกสบาย 7.8° ต่อตัว) ② `MS_R 320→520` (**กว้าง 1,040 ม.**) · `MS_Y 240→360` → บนจอ **x −1.02..1.02 · y 0.06..1.36 = คลุมครึ่งบนของจอทั้งผืนแบบ ID4** ③ ปลายหนามใต้ท้อง 110 ม. > ยอดเนิน 49 ม. ✓ · ลด `F_Y_MAX 95→80` กันยานลูกชนหนามใต้ท้อง (วัดจริงสูงสุด 53.6 ม.)
  - **ยืนยัน browser:** แผ่นดำเหลือ 0 ชิ้น ✓ ยิงแกนพลังงานยังโดน (99.2→98.3) ✓ ยานลูกไม่ชนลำ ✓ ฉาก 286k tris ✓ ไม่มี error
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.428` · SW v155) — อ่านก่อน
- **รอบ 440:** 🔫 **ท่าถือปืนเอียงแบบเกมจริง + แก้ท่าเพื่อนถือปืนผิดธรรมชาติ (ผู้ใช้ส่งภาพอ้างอิง)** — ① **ท่ามุมมองแรก:** เดิมปืนขนานแกนกล้องเป๊ะ = มองจากท้ายปืน เห็นเป็น "ท่อตั้งกลางจอ" → `GUN_POS[.30,−.26,−.66] · GUN_ROT[.04,.20,.16] · SCALE .78` วัดจากในเกมจริง: **ปากกระบอกอยู่ (0.12,−0.31) เฉียงเข้ากลางจอ 11° · ยอดกล้องส่อง (0.28,−0.03) · พานท้ายไหลออกมุมขวาล่าง** = องค์ประกอบเดียวกับภาพอ้างอิง · **ส่องกล้องยังเล็งกลางจอเป๊ะ (0.00,−0.14)** ② เพิ่ม **`forceGunForward()`** ด่านสุดท้าย วัดจากโมเดลจริงแล้วบังคับ "แกนยาว→ −Z + ปากกระบอกถูกด้าน" (โมเดล `new_gun_r93` เข้ามาเป็นก้อนเดียว `orientGunModel` เลยไม่จับ) ③ **ปืนของเพื่อน:** เดิมแขวนใต้ "แขนล่างขวา" → วัดได้ว่า **ลำกล้องชี้ (0,−0.92,+0.39) = ห้อยลงและชี้ไปข้างหลัง** (แกนท่อนแขนเปลี่ยนทุกท่า แก้ด้วยค่าคงที่ไม่ได้) → **ย้ายไปแขวนที่ลำตัวระดับอกฝั่งขวา** = ท่าประทับปืนแบบทหารจริง (ท่าเล็ง −0.98 · ท่าเดิน −1.00 ทั้งคู่ชี้หน้า ✓)
  - **ยืนยัน browser:** ทั้ง R93/ไรเฟิล ลำกล้องชี้หน้า ✓ ปืนอยู่หน้าอกเพื่อนทั้งท่าเดิน/เล็ง ✓ ฉาก 285k tris ✓ ไม่มี error
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.427` · SW v154) — อ่านก่อน
- **รอบ 439:** 🛸 **ยานแม่ลอยค้างฟ้าถาวรแบบ ID4 + ต่อยอด 1+2+3 (ผู้ใช้สั่ง)** — ① **ต้นตอที่ "ยานแม่หายไปอีกแล้ว" = `completeWord()` สั่ง `mother.visible=false`** รอลำใหม่ 3.2 วิ (และนานกว่านั้นถ้าหัวหน้าห้องยังไม่ประกาศคำ) → **ลำอยู่ค้างฟ้าเสมอ** แค่ "ไฟดับๆ ติดๆ + ตัวอักษรดับ" ตอนซ่อมตัว (`msRecover`) · `tickMother` ไม่ return ทิ้งตอน msDead แล้ว (ลำยังหมุน+ลอยหายใจ) ② **เอา "ลำแสงยึดแกน" ออกถาวร** — กรวยยักษ์กลางฟ้าที่ผู้ใช้เห็น 2 รอบติดแล้วนึกว่าเป็นยานแม่ ③ **เพื่อนหมอบกำบัง** เมื่ออยู่หลังกระสอบทราย ≤4.2 ม./ในบ้าน + หยุดนิ่ง (คำนวณฝั่งรับ ไม่ต้องส่งข้อมูลเพิ่ม) ④ **ซิงก์ปืน+สถานะยิงผ่านช่อง `av`** รูปแบบ `foot-sf` (7 ตัวอักษร ≤ 8 ที่ rules อนุญาต) → **ไม่ต้องแก้ Firebase rules เลย** · เครื่องรุ่นเก่าอ่านไม่ออก → ตกเป็น 'foot' ไม่พัง ⑤ เพื่อนถือปืนถูกกระบอก (สไนเปอร์ = ลำกล้องยาว+กล้องใหญ่+ขาทราย) + **ไฟปากลำกล้องตอนยิงจริง**
  - **ยืนยัน browser:** ยิงยานแม่ตาย+3 วิ → `visible=true y=226` ✅ ยังลอยอยู่ · กรวยยักษ์เหลือ 0 ชิ้น ✓ · เพื่อนหลังกระสอบทรายสูง **1.60 ม. vs ที่โล่ง 1.82 ม.** = หมอบจริง ✓ · ปืน: ไรเฟิลยาว 1.44 / สไนเปอร์ 1.87 ✓ · ไฟปากลำกล้อง opacity 1 ตอนยิง → 0 เองใน 220ms ✓ · ยานแม่บนจอ x −0.64..0.64 y −0.13..0.74 ✓ ฉาก 295k tris ✓ ไม่มี error
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.426` · SW v153) — อ่านก่อน
- **รอบ 438:** 🔫 **R93 โมเดลใหม่ + ซ่อนมือมุมมองแรก + เพื่อนถือปืนมีท่าทาง (ผู้ใช้สั่ง)** — ① `new_gun_r93.glb` (98,327 tris / 8.6MB) → **`new_gun_r93_lite.glb` 24,581 tris / 2.3MB** ต้นฉบับไม่ถูกแตะ · ตรวจแล้วแกนถูก (ยาวสุด=Z · มวลอยู่ท้าย = ปากกระบอกชี้ −Z) ② **มุมมองบุคคลที่ 1 ซ่อนแขน/มือทุกปืน** (`gunArms.visible=false` — ยังคำนวณแกว่งเหมือนเดิม แค่ไม่วาด) ③ **ผู้เล่นอื่นเห็นเราถือปืน**: `peerRifle()` 5 ชิ้น ห้อยใต้แขนล่างขวา (โมเดลตัวละครเป็น A-pose มือเปล่า + การเสียบเข้าข้อต่อจะล้าง mesh เดิมทิ้ง จึงต้องใส่กลับหลังประกอบ) ④ **ท่าทางตามสถานการณ์**: เดิน=ท่าเดิน · หยุด=ท่าเล็ง (ไม่ใช่ยืนปล่อยมือ) + เงยหน้าตามเป้าจริง (ยานลูกใกล้สุด/แกนยานแม่) + สะบัดไหล่เป็นจังหวะ (สถานะยิงไม่ได้ซิงก์ผ่านเน็ต — rules ไม่มี field ให้)
  - **ยืนยัน browser:** view model โชว์เฉพาะปืน+ไฟปากลำกล้อง (แขนซ่อน) ✓ ส่องกล้อง adsT 0→1→0 · เลนส์ R=92px · 6× ✓ ยิงได้ (10→9) ✓ เพื่อน: ปืนในมือ 6 ชิ้น ✓ ท่ายืน `armR −1.4` (เล็ง) → ตอนเดิน `armR +0.24 legR −0.49` (แกว่ง) → หยุดกลับมาเล็ง ✓ ฉาก 246k–289k tris ✓ ไม่มี error
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.425` · SW v152) — อ่านก่อน
- **รอบ 437:** 🛸 **ยานแม่ลงมาต่ำจนคลุมฟ้า + 🦶 แก้เท้าเพื่อนลอย (ผู้ใช้ส่งภาพ 2 ใบ)** — ① รอบ 435 ยังสูง 430 ม./ไกล 1,151 ม. → ตัวลำอยู่ **เหนือกรอบภาพ** ต้องแหงนหาถึงเจอ · คราวนี้ `MS_Y 430→240 · MS_Z −900→−260` (**ระยะเหลือ 500 ม.**) → **ลำกินจอ 58% ของความกว้าง · y −0.05..0.75 = ครึ่งบนของจอทั้งแถบ เห็นทันทีที่เข้าเกม** ② ย้ายแกนพลังงานมาใต้จมูกลำ (`CORE 150/−170`) เห็นพร้อมลำในเฟรมเดียว ③ **"กรวยสีน้ำตาล" ที่ผู้ใช้เห็นบังจอ = ลำแสงยึดแกน** (รัศมี 15–40 ม. ทึบ) → เปลี่ยนเป็นลำแสงเรืองแสงผอม ④ **เท้าเพื่อนลอย: เพื่อนส่ง `y` เป็น "ระดับตา" (พื้น+1.7)** แล้วเอาไปวางตัวโมเดลตรงๆ → ลอย 1.7 ม. → เพื่อนที่เดินเท้าให้ยืนบนพื้นที่เครื่องเราคำนวณเอง (ภูมิประเทศเป็นสูตรตายตัว ตรงกันทุกเครื่อง)
  - **⚖️ กฎตั้งค่ายานแม่ (จดในโค้ดแล้ว):** ปลายหนามใต้ท้อง `MS_Y − MS_R*0.50` ต้องสูงกว่ายอดเนินสูงสุด (~49 ม.) · กว้าง÷ระยะ ≈ 1.3 = คลุมฟ้า · เงยถึงกลางลำ ~28°
  - **ยืนยัน browser:** ปลายหนาม 88 ม. > เนิน 49 ม. ✓ แผงตัวอักษรไม่ชนลำ ✓ ยิงแกนยังโดน (99.2→98.6) ✓ **เท้าเพื่อนห่างพื้น 0.00 ม.** · ทหารพันธมิตร 9 คน 0–0.1 ม. ✓ ฉาก 245k–278k tris ✓ ไม่มี error
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.424` · SW v151) — อ่านก่อน
- **รอบ 436:** 🧭 **แก้บอทหันหลังยิง + ⌨️ WASD ใช้ได้บนแป้นพิมพ์ภาษาไทย (ผู้ใช้สั่ง)** — ① **ต้นตอบอทหันหลัง = `tickSquad` ใช้ `atan2(d.x,d.z)`** ซึ่งหันตัว **สวนทางเป้าเป๊ะ 180°** (โมเดลหันหน้า −Z ต้องใช้ `atan2(−d.x,−d.z)`) — โมเดลเองไม่ได้ผิด · ② **WASD ไม่ทำงานเพราะโค้ดอ่าน `e.key`** = ตัวอักษรที่พิมพ์ออกมา → สลับแป้นเป็นไทยแล้ว W=“ไ” A=“ฟ” S=“ห” D=“ก” เทียบไม่ตรง (ลูกศรไม่มีตัวอักษรจึงรอด) → เปลี่ยนเป็น **`e.code`** (ตำแหน่งปุ่มบนแป้น เหมือนที่ adventure3d ใช้อยู่แล้ว) · ③ แถม: `loadGlb` เข้าคิวต่อ path (เดิมทหาร 18 คนยิงโหลดไฟล์เดียวกัน 18 รอบ cache ทับกันไปมา) · ④ `applySoldierGlb` เส้นทางจับคู่ตามชื่อ แบนเป็นพิกัดโลกก่อนเข้าข้อต่อ (กันทิศหายแบบเดียวกับ autoRig)
  - **ยืนยัน browser:** ทหารพันธมิตร 4 คน **มุมระหว่างหน้ากับเป้า = 0° ทุกคน** ✓ · แป้นไทย (e.key เป็นอักษรไทย) W/A/S/D เดินหน้า/ซ้าย/ถอย/ขวาถูกทุกปุ่ม ✓ แป้นอังกฤษ+ลูกศรยังใช้ได้ ✓ ไม่มี error
  - **⚠️ บทเรียนการวัดผล:** วัดทิศหันของทหารจาก "ปลายเท้า" ตอนอยู่ท่าหมอบ/เล็ง **เชื่อไม่ได้** (ขาหมุนจนปลายเท้าสลับข้าง) — ต้องสั่ง `squadPose(i,'idle')` แล้ววัดทันทีก่อน `step` (ไม่งั้น tickSquad เปลี่ยนท่ากลับ) · เสียเวลาไล่ผิดทางเพราะข้อนี้
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.423` · SW v150) — อ่านก่อน
- **รอบ 435:** 🔻 **ยานแม่ลงต่ำ+ใกล้ขึ้นจนเห็นรายละเอียด + 🐛 แก้เพื่อนหันหลังเข้าหากัน (ผู้ใช้ส่งภาพ)** — ① `MS_R 900→450 · MS_Y 760→430 · MS_Z −1900→−900` = **ระยะ 2,215→1,151 ม. (ใกล้ขึ้น 1.9 เท่า)** ยังเห็นเต็มลำ **43°×15°** ของ FOV 68 · ⚠️ กฎที่ต้องรักษา: **ท้องลำ (MS_Y − MS_R×0.30) ต้องสูงกว่า CORE_Y=210** ไม่งั้นลำแสงยึดแกนกลับหัว (ตอนนี้ 277 > 207 ✓) ② **บั๊กเพื่อนหันหลัง:** `faceModelForward` เดิมเทียบ "กึ่งกลางกล่องชิ้นเท้า vs กึ่งกลางลำตัว" ต่างกันแค่ ~1% ของความสูง = **อยู่ในระดับ noise** → soldier_a เดาถูกแต่ **soldier_b (โมเดลเพื่อนออนไลน์) ไม่ถูกกลับหลัง** → เปลี่ยนเกณฑ์เป็น **"ปลายเท้ายื่นจากข้อเท้าไกลกว่าส้นเท้า" วัดระดับจุดยอด** (ต่างกันชัด 0.08 vs 0.15)
  - **ยืนยัน browser:** ทั้ง soldier_a และ soldier_b หลังแก้ **หันหน้า −Z ถูกทั้งคู่** ✓ ทดสอบในเกม: เพื่อน yaw=0 → หันหน้า −Z · yaw=π → หันหน้า +Z ✓ (หันหน้าเข้าหากันแล้วจริง) · ยานแม่ยิงแกนพลังงานยังโดน (เกราะ 99.2→98.6) ✓ ไม่บังจอ (8% ตอนเงยมองแกน) ✓ ไม่มี error
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.422` · SW v149) — อ่านก่อน
- **รอบ 434:** 🚁 **เฮลิคอปเตอร์จอดจริง 5 ลำในสนามรบ — ใช้งานเหมือนโลกเฮลิฯ (ผู้ใช้สั่ง)** — โซนใหม่ `🚁🅿️` ใน `js/invasion3d.js` · `helicopter_lite.glb` **9.6k tris (จาก 48k)** ต้นฉบับที่โลกเฮลิฯ ใช้ไม่ถูกแตะ · จัดทิศ **หัวชี้ −Z + สกีแตะพื้น** ด้วยสูตรเดียวกับ `heliGlbAssemble` · **ย้อมสีทะเลทราย** `HELI_DESERT=0xb6a678` (material เดียวแชร์ทุกลำ) · ขึ้นเครื่องได้เฉพาะเดินถึงลำ (`BOARD_DIST`10) ปุ่ม 🚁 โผล่เมื่อยืนข้างลำ · **สตาร์ทเครื่อง 5 ขั้น ~10 วิ** (`START_PHASES` · ใบพัดไต่รอบ + เครื่องสั่น + บินไม่ได้จนพร้อม) · ปุ่ม **👁️ ปรับมุมมองห้องนักบิน 3 ระดับ** (เต็มลำ/มุมบิน/บินต่ำ — ขยับกล้อง+กรอบกระจก+แผงหน้าปัด · จำใน `state.heliSeat`) · **ลงจากเครื่อง = ลำจอดค้างตรงนั้น** เดินกลับขึ้นใหม่ได้ · ลำเพื่อน/บอทใช้โมเดลเดียวกัน · แผนที่มีหมุด 🚁 เลือกเกิดข้างลำได้
  - **ยืนยัน browser:** ยืนไกล 60 ม. → ปุ่มไม่โผล่+ขึ้นไม่ได้ ✓ ยืนข้าง 6 ม. → ขึ้นได้ ✓ **ระหว่างสตาร์ทดันคันเร่ง 1 วิ ไม่ขยับเลย (Δy=0)** ✓ ครบ 10 วิ → พร้อมบิน+เบาะสลับเป็น "มุมบิน" อัตโนมัติ ✓ เทคออฟขึ้น 8.6 ม. · บินหน้าได้ · **ลำจริงตามตัวมาด้วย หันตาม yaw** ✓ ลงเครื่องแล้วลำจอดตรงจุดที่ลงเป๊ะ + ขึ้นใหม่ได้ ✓ เฮลิเพื่อน 9.8k tris โมเดลจริง ใบพัดหมุนได้ ✓ หมุดแผนที่วาดจริง ✓ **ผังไม่ทับ/ไม่หลุดจอ 1280×720 · 812×375 · 651×306 (ทั้งเดินเท้า/ในเฮลิ)** ✓ ฉาก 245k–286k tris ✓ ไม่มี error
  - **🐛 ที่เจอ:** ปุ่มยิงซ้าย (รอบ 433) ชนกระดานคะแนน/แผงสถานะบนจอเตี้ยตอนอยู่ในเฮลิ (แผงสูงขึ้นเพราะแถวจรวด) → จอ ≤400px เลื่อนไป left:136 + คุมกว้างกระดาน ≤120px · จอ ≤330px ย้ายไปชิดขวาจอย (left:100 bottom:56)
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.421` · SW v148) — อ่านก่อน
- **รอบ 433:** 🔫 **ปุ่มยิงใบที่ 2 เหนือจอยบังคับ + ย้ายช่องกระสุนไปข้างปุ่ม "ออก" (ผู้ใช้สั่งจากภาพ)** — `#inv-fire2` วางกึ่งกลางตรงกับจอยเป๊ะ เหนือจอย 10–12 px (นิ้วซ้ายเดิน+ยิงพร้อมกันได้ ไม่ต้องย้ายมือ) · ผูก handler ตัวเดียวกับปุ่มยิงเดิม · `#inv-ammo` ออกจากแผงสถานะมาเป็นชิปกลมถัดจากปุ่มออก (แผงสถานะเตี้ยลงด้วย)
  - **ยืนยัน browser 3 ขนาด:** 1280×720 · 812×375 · 651×306 → **ไม่มีปุ่มทับกัน/หลุดจอเลย** (เช็ก 17 องค์ประกอบทุกคู่ · ทั้งเดินเท้าและขับเฮลิ) ✓ กดปุ่มใหม่ยิงออกจริง (กระสุน 10→9) ปล่อยนิ้วหยุดยิง ✓ ช่องกระสุนอยู่แถวเดียวกับปุ่มออก ห่าง 18px ✓ ไม่มี error
  - **🐛 ที่เจอ:** จอ 651×306 ปุ่มยิงใหม่ชนแผงพลังชีวิต 58×23px → ที่ `max-height:330` ย้ายไปมุมบนขวาของจอยแทน (ยังอยู่ในระยะนิ้วซ้าย)
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.420` · SW v147) — อ่านก่อน
- **รอบ 432:** 🛸 **ตอบ "ที่ตั้งอยู่นั่นคืออะไร" + ยานลูกใหญ่เท่าเฮลิ (ผู้ใช้ส่งภาพจอครึ่งชมพู/แถบดำทแยง)** — **วัดจากฉากจริง**: แถบดำ = **ยานแม่ของผู้ใช้เอง** (glb โหลดถูก) แต่รอบ 417 ขยายเป็น **5,600 ม. วางห่างแค่ 2,200 ม. → กินมุมมองเกิน 100° เห็นได้แค่เสี้ยวเดียว** · ผืนชมพู = **ออร่าแกนพลังงาน** (sprite 315 ม. opacity .35) · ลูกกลมส้ม = แกนพลังงาน
  - **แก้:** ยานแม่ `MS_R 2800→900 · MS_Y 1500→760 · MS_Z −1500→−1900` = **กว้าง 1,800 ม. ที่ระยะ 2,215 ม. = 44° ของ FOV 68°** (เห็นทรงเต็มลำแต่ยังมหึมา) · ออร่า `CORE_R*7→*3.4` · **ลำแสงยึดแกนเอียงชี้ท้องยานจริง** (เดิมตั้งตรงลอยๆ พอยานถอยไปไกลจะไม่ต่อกัน) · ⚖️ **กฎใหม่จดในโค้ด: ความกว้างลำ ÷ ระยะ ≤ 0.85**
  - **ยานลูก:** `FIGHTER_SIZE=11.5` เท่าเฮลิ (วัดเฮลิจริง 11.5×5.0×9.0) เดิม 7 ม. · กันชนเล็งยิงโตตาม (`FIGHTER_SIZE*0.66`) · ป้ายตัวอักษรยกตาม · **`alien_fighter_lite.glb` 8.2k tris (จาก 16.4k)** เพราะมีได้ถึง 8 ลำพร้อมกัน — ต้นฉบับไม่ถูกแตะ
  - **ยืนยัน browser:** ยานแม่ 44°×15° ในเฟรม ✓ ไม่มีอะไรกินจอ >25% แล้วนอกจากโดมฟ้า ✓ ยานลูก 8.9×14.2 ม. · ยิงเล็งตรงลำเข้าเป้า ✓ ฉากรวม **276,596 tris / 458 draw call** ✓ แท่งกระสุนเก็บกวาดหมดไม่รั่ว ✓ ไม่มี error
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.419` · SW v146) — อ่านก่อน
- **รอบ 431:** 🏠 **บ้าน house_01 เข้าไปหลบซุ่มยิงได้ + จุดสูงข่ม + ยานลูกเลียดเนิน + ความชันมีผล (ผู้ใช้สั่งต่อยอด)** — ① **โมเดลผู้ใช้หนักมาก 62MB / 1.88M tris** → ทำ `house_01_lite.glb` = **1.1MB / 37.8k tris** (สูตรอยู่ใน NOTES.md · กุญแจคือ **ตัด NORMAL ทิ้งก่อน weld** ไม่งั้น simplify ตันที่ 285k) · ต้นฉบับ 62MB ใส่ .gitignore ไว้ในเครื่อง ② **กันชนบ้านสร้างอัตโนมัติจากสามเหลี่ยมโมเดล** (`buildBlockGrid` เอาเฉพาะผนัง "ตั้ง" ช่วงสูง 0.4–2.1 ม. ปั๊มตามขอบสามเหลี่ยม) → ประตู/ช่องว่างเดินผ่านได้เอง ใช้ซ้ำกับอาคารอื่นได้ ③ อยู่ในบ้าน = ที่กำบัง **ดาเมจเหลือ 35%** + ป้าย `#inv-cover` ④ ยานลูกบิน**อิงพื้น** โฉบต่ำเลียดสันเขา ⑤ **วิ่งขึ้นเนินช้า ×0.52 / ลงเนินเร็ว ×1.22** ⑥ แผนที่มีหมุด 🏠 + **🎯 จุดสูงข่ม** (หายอดเนินจริง 4 จุด) + ปุ่มวนเลือก
  - **ยืนยัน browser:** เดินเข้าบ้านจริงด้วย `tickPlayer` (ลอง 8 ทิศ เจอประตู) **ทั้ง 2 หลัง ✅ พื้นที่ในบ้านเดินถึง 179 ตร.ม.** · ดาเมจในบ้าน 7 vs นอกบ้าน 20 ✓ ป้ายกำบังขึ้น/ดับถูก ✓ **LOD: ใกล้ 232k tris (ก่อนลดโพลีคือ 490k!) · ไกลสลับกล่องแทน** ✓ ยานลูกห่างพื้นต่ำสุด **7.4 ม. ไม่มุดเนิน** · บินต่ำ 32% ของเวลา ✓ ความเร็ว ราบ 7.44 / ขึ้นเนิน 5.37 / ลงเนิน 9.15 m/s ✓ ปุ่ม 🎯 วนครบ 4 จุด ✓ **การ์ดวิธีเล่น+กล่องแผนที่ไม่มีแถบเลื่อนที่ 1280×720 / 812×375 / 651×306** ✓ ไม่มี error
  - **🐛 ที่เจอ:** ① ปั๊มกันชนด้วย "กรอบสี่เหลี่ยมของสามเหลี่ยม" → บ้านตันทั้งหลัง เดินเข้าไม่ได้ (สามเหลี่ยมใหญ่ทแยงกินทั้งห้อง) → เปลี่ยนเป็นปั๊มตามขอบ + ข้ามสามเหลี่ยมที่เกือบราบ ② เพิ่มบรรทัดวิธีเล่นแล้วการ์ด intro ล้นจอ 812×375 → จัด 2 คอลัมน์บนจอกว้าง + ย่อฟอนต์บนจอเตี้ย
  - **ค้าง: ผู้ใช้ดูภาพจริง** — จูนได้ที่ `HOUSE_SPOTS` (ตำแหน่งบ้าน) · `HOUSE_SIZE`18 · `HOUSE_COVER`.35 · `HOUSE_LOD`115
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.418` · SW v145) — อ่านก่อน
- **รอบ 430:** ⛰️ **ภูมิประเทศมีเนินเขา/ลาดชัน + กล่องเลือกจุดลงสนามกางออกด้านข้างเกือบเต็มจอ (ผู้ใช้สั่งจากภาพจริง)** — ① เดิมพื้นเป็นคลื่นทราย ±3 ม. = ราบทั้งแผนที่ → เพิ่ม `HILLS` **เนิน 7 ลูก (ค่าตายตัว ห้ามสุ่ม เพราะออนไลน์ต้องเห็นตรงกัน)** + สันเขา/ร่องน้ำแห้ง + ลูกคลื่นเล็ก · **ความสูง −9.6…+49 ม. · 42.6% ของพื้นที่ชันเกิน 15%** · รีดเนินออกเฉพาะแนวถนนสมรภูมิ (ขวางถนนราบ 2.6–3.6 ม.) · seg 80→120 · เพิ่ม `baseLow()` วางตึกที่มุมต่ำสุด (**ตึก 98/98 หลังไม่ลอย**) ② แผนที่เลือกจุดลงมี **ชั้นไล่เฉดความสูง + เส้นชั้นความสูงทุก 6 ม.** (สร้างครั้งเดียวแล้ว blit · redraw 0.1ms) ③ กล่อง = **คอลัมน์คู่ แผนที่ซ้าย/คำแนะนำ+ปุ่มขวา** กว้าง 97vw (สูงสุด 1400px)
  - **ยืนยัน browser:** 1600×900 card 1400px map 860×530 · 1280×720 card 1242(97%) map 822×506 · **812×375 map 458×282 (เดิม 304×188)** · 651×306 ทุกอย่างอยู่ในจอ · **ทุกขนาด scrollHeight=clientHeight ไม่มีแถบเลื่อน (กฎ #7)** ✓ แตะเลือกจุดยังแม่น ✓ ยืนยอดเนินสูงกว่าเชิงเนิน 25.7 ม. ตาอยู่เหนือพื้น 1.70 พอดี ✓ ความชันไม่กระโดด (สูงสุด 0.62 ม./2 ม.) ✓ **calls 438 · tris 209,277** (เดิม 193k — ยังในงบ) ✓ ไม่มี error
  - **🐛 ที่เจอ:** breakpoint คอลัมน์คู่ตั้งไว้ 900px ทำให้จอ **812×375 ตกไปเรียงบนล่าง แผนที่เหลือนิดเดียว+กล่องล้นจอ** → เกมบังคับแนวนอนอยู่แล้ว จึงลด breakpoint เหลือ 640px
  - **ค้าง: ผู้ใช้ดูภาพจริงในเกม** — จูนเนินได้ที่ตาราง `HILLS` (x,z,r,h) ใน `js/invasion3d.js`
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.417` · SW v144) — อ่านก่อน
- **รอบ 429:** 🪖 **โมเดลจริงของผู้ใช้ขึ้นเว็บครบ (soldier_a/soldier_b/gun_r93) + จัดแกนปืนอัตโนมัติ** — ผู้ใช้ export จาก Tripo มาแล้ววางครบ · **ตรวจไฟล์จริง `soldier_a.glb`: 110 ชิ้น · 1 material (Pack UV ทำงาน) · 7,452 tris · ยืนตรงแกน Y ✓ สัดส่วนคนปกติ** → auto-rig จับเข้า **11 ข้อต่อ รวมเหลือ 11 mesh** ✓ เท้าแตะพื้นทุกท่า ✓
  - **🐛 บั๊กที่เจอจากโมเดลจริง (ทรงจำลองไม่เจอ):** ① **โมเดลปืนวางตามแกน X** (ลำกล้องชี้ข้าง) ทำให้ `stretchGunBarrel` ไม่ทำงานเลย → เขียน **`orientGunModel()`** หมุนแกนยาวสุดมาเป็น Z + หาปลายลำกล้องแล้วให้ชี้ −Z ② **เกณฑ์หาปลายลำกล้องถูกขาทราย/ปากกระบอก/ศูนย์หน้าหลอก** (อ้วนพอกับพานท้าย) → เปลี่ยนไปใช้ **"จุดศูนย์กลางมวล"** แทน (ปืนหนักทางท้ายเสมอ · ลำกล้องผอมเบา) ③ **`mergeGunParts` ไม่ล้าง transform ของ obj หลัง bake** → ปืนไรเฟิลโดนหมุนซ้ำ 2 รอบ แกนเพี้ยน → เพิ่มการล้าง transform
  - **ยืนยัน browser (วัดทั้งกระบอกในพิกัดปืน):** R93 = **0.30×0.34×1.25 แกน Z ✅ 1 ก้อน 10,727 tris** · ไรเฟิล = **0.15×0.48×0.95 แกน Z ✅ 1 ก้อน 13,868 tris** · มวลอยู่ +Z ทั้งคู่ = ปากกระบอกชี้ −Z ถูกต้อง ✓ ทหาร glb:true 11 ข้อต่อ ✓ ฉากรวม **447 draw call / 193,341 tris** ✓ ไม่มี error
  - **ค้าง: ผู้ใช้ดูภาพจริงในเกม** (เครื่องมือ screenshot ของ Claude ยังใช้ไม่ได้ · วัดได้แค่ตัวเลข) — ถ้าเห็นทหาร/ปืนวางผิดท่า บอกได้ จูนที่ `poseSoldier()` · `GUN_POS/GUN_ROT` · `GUN_STRETCH`
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.416` · SW v143) — อ่านก่อน
- **รอบ 428:** ⚡ **รวมชิ้นส่วนปืนเป็นก้อนเดียว + ตอบค่า Polygon Count ของปืน** — พบว่าตัวโหลดปืน .glb **ยังไม่ได้รวมชิ้น** (ต่างจากทหารที่รวมตั้งแต่รอบ 425) · ปืนเป็น **view model ที่วาดทุกเฟรมและอยู่ใกล้กล้องที่สุด** → แตก 81 ชิ้น = +81 draw call ตลอดเกม → เพิ่ม `mergeGunParts()` (ใช้ `mergeMeshList` ตัวเดิม) เรียก**หลัง** `stretchGunBarrel` เสมอ (ต้องยืดตอนยังแยกชิ้นอยู่) · ใส่ให้ทั้ง `gun_r93.glb` และ `gun_rifle.glb`
  - **📊 คำตอบ Polygon Count ปืน: 8,000 ที่ผู้ใช้ตั้งอยู่ = พอดีแล้ว** (คิดเป็น 3.4% ของฉากรวม 238k) · ขยับเป็น 10,000-12,000 ได้สบาย (~5%) ถ้าอยากให้ท่อกลม/กล้องเนียนตอนส่อง · 15,000 ยังไหวแต่ไฟล์เริ่มเกิน 2MB · **ปืนให้ละเอียดกว่าทหารได้เพราะมีกระบอกเดียวบนจอ** (ทหารมี 18 คน) และวาดแค่ 1 ครั้ง/เฟรม (ตอนส่องกล้อง PiP ซ่อนปืนในรอบเลนส์อยู่แล้ว)
  - **ยืนยัน browser:** จำลองปืน Tripo 81 ชิ้น → **รวมเหลือ 1 mesh · สามเหลี่ยมเท่าเดิมเป๊ะ (1,016)** ✓ ยืดลำกล้องยังทำงานถูกก่อนรวม ✓ ไม่มี error
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.415` · SW v142) — อ่านก่อน
- **รอบ 427:** 🔧 **ยืดลำกล้องปืนอัตโนมัติหลังโหลดโมเดล (ผู้ใช้: โมเดล R93 ลำกล้องสั้นไป)** — `stretchGunBarrel()` ตัดปืนที่ระนาบ `GUN_CUT`(0.46 ของความยาววัดจากท้าย) แล้ว: ชิ้น**ยาวเรียวฝั่งหน้า = ลำกล้อง → ยืดตามแกน Z อย่างเดียว** ตรึงปลายท้ายไว้ · ชิ้นอื่นฝั่งหน้า (ปากลำกล้อง/ศูนย์หน้า/ขาทราย) → **เลื่อนตามไปข้างหน้า**เท่าที่ลำกล้องยาวขึ้น · ฝั่งท้าย (โครง/กล้อง/ด้าม/พานท้าย) ไม่แตะ · **ยืดแล้วไม่เพี้ยนเพราะลำกล้องเป็นทรงกระบอกตรง หน้าตัดคงเดิม** · เรียกก่อน `fitInto` ในตัวโหลด `gun_r93.glb`
  - **🐛 ที่เจอ:** เกณฑ์ "ยาวเรียว" หลวมไป (`z > max(x,y)×1.8`) ทำให้**ปากลำกล้องโดนยืดตามไปด้วย** (0.13→0.221) แล้วลอยอยู่กลางลำกล้อง → เข้มขึ้นเป็น `z > max(x,y)×2.6 && z > ความยาวปืน×0.18`
  - **ยืนยัน browser:** ลำกล้อง 0.52→0.884 (**1.7 เท่าเป๊ะ**) · **ความหนาเท่าเดิม 0.042** (ไม่อ้วนขึ้น) · ปากลำกล้องไม่ถูกยืด (0.13 คงเดิม) · **รอยต่อลำกล้อง↔ปากลำกล้อง = 0.005 เท่ากันทั้งก่อน-หลัง** (ต่อสนิท ไม่ลอย ไม่จม) · ขาทรายเลื่อนตาม · โครง/พานท้าย/กล้องไม่ขยับ ✅ ไม่มี error
  - **จูน:** `GUN_STRETCH`(1.7) ความยาว · `GUN_CUT`(0.46) จุดตัด — ปรับหลังเห็นโมเดลจริงได้
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.414` · SW v141) — อ่านก่อน
- **รอบ 426:** 🧭 **หันหน้าตัวละคร + ปรับความสูงอัตโนมัติ (ผู้ใช้ถามว่าตั้งใน Tripo ยังไง)** — **คำตอบ: Tripo ตั้งไม่ได้ → ให้เกมจัดการเองถูกกว่า** · `faceModelForward()` ดูจาก **ปลายเท้ายื่นไปทางไหน เทียบกับจุดกึ่งกลางลำตัว** (เป้สะพายหลังดันจุดกึ่งกลางไปข้างหลัง = สัญญาณเสริมที่ตรงกันเสมอ) แล้วหมุน 180° ให้เองถ้าโมเดลหันหน้า +Z · ความสูงมี `fitInto(1.8)` อยู่แล้ว
  - **🅰️ ตอบคำถาม A-pose vs T-pose (ทดสอบเทียบจริง):** **A-pose ได้ข้อต่อครบ 11/11** · **T-pose ได้แค่ 9/11 — แขนล่างหายทั้ง 2 ข้าง** เพราะระบบแยกแขนบน/ล่างด้วย "ความสูง" เป็นหลัก และจุดหมุนใช้ "ขอบบนของกลุ่มชิ้น" ซึ่งใช้ไม่ได้กับแขนที่นอนขนานพื้น → **บังคับ A-pose** (แขนห้อยกาง 15-30° ศอกตรง) จดไว้ใน PROMPTS_INVASION.md แล้ว
  - **ยืนยัน browser (4 เคส):** โมเดลหันหน้า −Z → ไม่หมุน · หันหน้า +Z → หมุนให้ · สเกล ×37 · สเกล ×0.02 → **ทุกเคสจบที่ "ปลายเท้าชี้ −Z + สูง 1.800 ม." ถูกหมด** ✅ ไม่มี error
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.413` · SW v140) — อ่านก่อน
- **รอบ 425:** ⚡ **รวมชิ้นส่วนโมเดลทหารเป็นก้อนเดียวต่อข้อต่อ + ตอบคำถามค่า Retopo** — ผู้ใช้ถามว่าตั้ง Retopo เท่าไหร่ให้เล่นหลายคนได้แต่ยังสวย → **พบว่า "จำนวนชิ้นส่วน" สำคัญกว่าจำนวนโพลี**: Smart Segment แตก ~110 ชิ้น = **110 draw call ต่อทหาร 1 คน** (18 คน = 1,980 draw call มือถือเด็กเอาไม่อยู่) → เขียน `mergeMeshList()` รวมทุกชิ้นในข้อต่อเดียวกันเป็น mesh เดียว (จัดกลุ่มตาม material · แบก matrix เข้า geometry · three.min.js ไม่มี BufferGeometryUtils จึงเขียนเอง) → **110 → 11 ก้อน/คน ลด 10 เท่า หน้าตาเหมือนเดิม**
  - **📊 คำตอบเรื่อง Retopo (วัดจากฉากจริง):** ฉากไม่รวมทหาร = **122,005 สามเหลี่ยม / 452 draw call** · ที่ 18 คน → 3,000/คน=176k ✅ · **6,000/คน=230k ✅ แนะนำ** · 8,000/คน=266k ✅ · 10,000/คน=302k ⚠️ เสี่ยงมือถือเก่า · **สรุปแนะนำ 6,000** (soldier_b เห็นใกล้กว่าใช้ 8,000 ได้) · ตารางเต็มอยู่ใน `PROMPTS_INVASION.md`
  - **ยืนยัน browser:** จำลองโมเดล 110 ชิ้น → รวมเหลือ **11 mesh/คน** (`rigInfo.meshes=11`) ✓ จับเข้าข้อต่อยังถูก 110/110 ✓ draw call ต่อทหารคงที่ 11 ไม่ว่าโพลีเท่าไหร่ ✓ ไม่มี error
### 📌 สรุปสถานะล่าสุด (21 ก.ค. · deploy `.412` · SW v139) — อ่านก่อน
- **รอบ 424:** 🤖 **จับชิ้นส่วนโมเดลเข้าข้อต่อ "อัตโนมัติจากตำแหน่ง" — ผู้ใช้ไม่ต้องตั้งชื่อชิ้นเลย** — ผู้ใช้ใช้ Tripo Smart Segment ได้ชิ้นส่วนละเอียดมาก (`tripo_part_1..109`) ชื่อไม่สื่ออะไร → เขียน `autoRigSoldier()`: ① หากรอบทั้งตัวแล้วแปลงตำแหน่งทุกชิ้นเป็น**สัดส่วน** (สูง 0=พื้น 1=หัว · กว้าง −1..1) ② เทียบ **`BODY_MAP`** (แผนผังข้อต่อร่างคน A-pose) หาข้อต่อใกล้สุด (ถ่วง Y มากกว่า X เพราะความสูงบอกส่วนของร่างกายชัดกว่า) ③ **จุดหมุนคำนวณจากชิ้นจริง** (ไหล่/ศอก/สะโพก/เข่า = ขอบบนของกลุ่ม · คอ = ขอบล่างของหัว · เอว = ขอบล่างลำตัว) ④ แบนพิกัดเป็นโลกแล้วย้ายไปห้อยใต้ข้อต่อ · **ลำดับใช้งาน: จับคู่จากชื่อก่อน → ไม่ได้ค่อยจับจากตำแหน่ง → ยังไม่ได้ค่อยตกไปโหมดนิ่ง** (ไม่พังทุกกรณี)
  - **🦶 แถม `fitSoldierGround()`:** สัดส่วนขาโมเดลจริงไม่เท่าทรงที่โค้ดวาด ทำให้ท่าหมอบจมพื้น/ท่าเดินลอย → เพิ่มชั้น `inner` ในโครง แล้ววัดกรอบจริง **ครั้งเดียวตอนเปลี่ยนท่า** (ไม่ใช่ทุกเฟรม) ยกตัวให้เท้าแตะพื้นพอดี
  - **ยืนยัน browser (จำลองโมเดลแบบ Tripo จริง 110 ชิ้น ชื่อ `tripo_part_N` วางตามกายวิภาค):** **จับเข้าข้อต่อถูก 110/110 ชิ้น** — หัว 10 · ลำตัว 26 · สะโพก 8 · แขนบน 7+7 · แขนล่าง 8+8 · ต้นขา 9+9 · แข้ง 9+9 ✓ **ไหล่ไม่ขยับตอนยกแขน** (จุดหมุนถูก) ✓ ปลายแขนยกขึ้น+ไปข้างหน้าจริงตอนเล็ง ✓ หมอบแล้วเตี้ยลง (1.70→1.53) ✓ **ทุกท่าเท้าแตะพื้นพอดี (min.y = 0.000)** ✓ draw call 429 ✓ ไม่มี error
  - **ค้าง:** ผู้ใช้ export `.glb` จาก Tripo (Smart Segment ได้เลย ไม่ต้องตั้งชื่อ/ไม่ต้อง rig) วาง `img/models/soldier_a.glb` + `soldier_b.glb` แล้วบอกให้ commit · **เงื่อนไขเดียวที่ต้องถูกคือ ท่า A-pose + หันหน้า −Z**
