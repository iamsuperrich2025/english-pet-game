...
- ✅ **รอบ 82: คนที่ invite กันเห็นคำเดียวกัน (party word sync) 🤝** — publish `cw="en|th"` ใน /world **เฉพาะตอนมีเพื่อนปาร์ตี้จริง** (คนทั่วไปไม่ส่ง → ไม่ผูก rules ใหม่) · หัวหน้า = key น้อยสุดในกลุ่ม `tinvLinked` (ทุกเครื่องคำนวณตรงกัน) · ลูกทีม `syncPartyWord()` (ใน tickPeers ทุกเฟรม) ดันคำหัวหน้าขึ้น `words[0]` → เห็นคำเดียวกัน เก็บตัวอักษรของตัวเอง · `lastSharedDone` กันลูกทีมที่ทำเสร็จก่อนวนซ้ำ (เล่นคำตัวเองรอหัวหน้าเปลี่ยน) · completeWord force `sendPos(true)` ดันคำใหม่ทันที · ⚠️ **letters ยังเป็น local (ตำแหน่งคนละที่)** — "ช่วยกันหา" = ระดับคำ/เป้าหมาย ยังไม่แชร์ตำแหน่งตัวอักษร (ต่อยอดได้: seed spawn ร่วม) · ยืนยัน 4 เคสด้วยการจำลอง peer · **⚠️ ต้อง publish rules ใหม่ (เพิ่ม field `cw` — RULES.md)** — version .58
