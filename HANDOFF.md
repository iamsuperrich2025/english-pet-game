...
- 🎯 **งานถัดไป (ผู้ใช้เลือกแล้ว 10 ก.ค.): ต่อยอด Lobby 3D** — (1) **turntable**: โมเดลหมุนช้าๆ อัตโนมัติตอน idle (หยุดเมื่อผู้ใช้แตะ/ลาก แล้วกลับมาหมุนต่อหลังปล่อยสักพัก) (2) **เช็ก animation clips** ในไฟล์ GLB ทั้ง 3 (`img/models/*.glb`) ว่ามี clip ไหม — `setupClips()` ใน `js/lobby3d.js` รองรับอยู่แล้วแต่เล่นเฉพาะชื่อ idle/breath/stand/rest ถ้ามี clip ชื่ออื่นพิจารณาใช้ · โค้ดสะอาดพร้อมต่อ (รอบ 105 เพิ่ง fix+commit) · ⚠️ อย่าลืม `cloneSkinned` มีผลกับ clip ด้วย (AnimationMixer ต้องผูกกับ root ที่ clone แล้ว — ตอนนี้ถูกอยู่)
