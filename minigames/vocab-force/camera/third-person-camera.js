...
(function(root){
  ...
  ThirdPersonCamera.prototype.tick = function(dt, player){
    ...
    const dist = (player.isPowerJumping && player.isPowerJumping())
      ? Math.max(this.dist, (VF.PowerJumpTune && VF.PowerJumpTune.CAMERA_DIST) || 11)
      : this.dist;
    const px = player.x, py = player.y + this.lookY, pz = player.z;
    ...
    const followK = (player.isDashing && player.isDashing())
      ...
      : ((player.isPowerJumping && player.isPowerJumping()) ? 12 : 9.5);
    ...
    this.fov += (this.baseFov - this.fov) * VF.clamp(dt * 9, 0, 1);
    if(Math.abs(this.fov - this.baseFov) > 0.05){
