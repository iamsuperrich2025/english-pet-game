...
(function(root){
  ...
  NexCharacterController.prototype.takeHit = function(amount, blocked, info){
    ...
    /* รอบ 1592: โดนวงระเบิดรถน้ำมัน / แนว SLAM / ลูกพลัง ATTACK → เล่นท่าล้ม "พอดีจังหวะที่พลังแตะตัว"
       (takeHit ถูกเรียกจากจุดที่ระเบิด/พลังงานถึงตัวพอดี จึงเล่นที่นี่เท่านั้น ไม่ก่อน ไม่หลัง · มือ/เตะ/ซอมบี้กัดไม่เข้าเงื่อนไข)
       · โดนจริงเท่านั้น (dmg ผ่าน + dash/invuln ไม่บล็อก) และยังไม่ตาย — ตายให้แอนิมตายทำงานแทน */
    if(dmg >= 0.5 && this.alive && (info.from === 'tanker' || info.from === 'gun' || info.from === 'slam')){
      this.playAction('knockDown');
    }
    return dmg;
  };
