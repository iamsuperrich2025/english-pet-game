...
(function(){
 ...
 function choose(preload){
  ...
  root.innerHTML=`<header class="ah-header"><button id="ah-back" aria-label="กลับไปหน้าหลัก">← กลับ</button><div><small>VOCAB ARENA · ADMIN</small><h1 id="ah-heading">เลือกเพื่อนร่วมผจญภัย</h1></div><button id="ah-motion" aria-pressed="true">✦ ภาพเคลื่อนไหว</button></header><div class="ah-layout"><div class="ah-stage"><div class="ah-orbit"></div><div class="ah-portrait"></div><div class="ah-stage-label"><span id="ah-tag"></span><strong id="ah-name"></strong></div></div><div class="ah-info"><div class="ah-roster" role="group" aria-label="ตัวละคร ชาย 4 หญิง 4">${roster.map(h=>`<button class="ah-card" data-hero="${h.id}" aria-pressed="false" aria-label="${h.name} ${h.title} ${h.gender}"><img src="${h.thumb}" alt="" width="160" height="240"><span>${h.name}</span><small>${h.gender}</small></button>`).join('')}</div><div class="ah-details" aria-live="polite"><div class="ah-detail-head"><h2 id="ah-title"></h2><span>HP 100</span></div><p id="ah-desc"></p><div id="ah-powers"></div><p class="ah-talent" id="ah-talent"></p></div><footer class="ah-footer"><div><span id="ah-ready" role="status">กำลังเตรียมสนาม…</span><small>ชุดพลังเริ่มต้น · เปลี่ยนพลังเพิ่มได้ในสนาม</small></div><button id="ah-play" disabled>เตรียมสนาม…</button></footer></div></div>`;
