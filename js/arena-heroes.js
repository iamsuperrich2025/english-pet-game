...
(function(){
 const rows=[
  ['fire','อัคนี','อัศวินเปลวไฟ','ชาย','🔥','#ff8852',0xee6541,0x864821,['fire','meteor'],'เผาพื้นที่ต่อเนื่อง แล้วปิดท้ายด้วยฝนดาวตก'],
  ['wind','วายุ','นักธนูสายลม','ชาย','🌪️','#7ee8c9',0x47ad94,0x488c87,['wind','arc'],'ดูดศัตรูรวมด้วยพายุ แล้วชิ่งสายฟ้าเป็นกลุ่ม'],
  ['ice','เหมันต์','จอมเวทน้ำแข็ง','ชาย','❄️','#99d8ff',0x668cd0,0xc9e5ff,['ice','water'],'แช่แข็งให้ศัตรูเดินช้า แล้วผลักออกด้วยคลื่น'],
  ['light','อรุณ','ผู้พิทักษ์แสง','ชาย','✨','#ffe296',0xeac65e,0xefc764,['light','earth'],'ฟื้น HP และเสริมโล่ พร้อมกระแทกศัตรูให้ถอย'],
  ['earth','พฤกษา','ผู้พิทักษ์พสุธา','หญิง','🪨','#c6d08c',0x8d9b57,0x754528,['earth','nova'],'แผ่นดินไหวผลักศัตรู แล้วระเบิดโนวารอบตัว'],
  ['meteor','ดารา','นักเดินทางดาวตก','หญิง','☄️','#ffb194',0xdd8b8b,0xef9696,['meteor','fire'],'ดาวตกสามลูกโจมตีวงกว้าง พร้อมวงเพลิงเผาต่อเนื่อง'],
  ['gravity','ลูน่า','จอมเวทแรงโน้มถ่วง','หญิง','🌌','#d4adff',0xa77bd8,0xbca5dd,['gravity','nova'],'หลุมดำดูดรวมก่อนระเบิด แล้วต่อด้วยโนวา'],
  ['water','ธารา','นักดาบสายน้ำ','หญิง','🌊','#8cdeff',0x559ec5,0x528fa4,['water','wind'],'คลื่นยักษ์ผลักศัตรู และพายุควบคุมทางเดิน']
 ];
 const motions={
 ...
 function choose(preload){
  ...
  root.innerHTML=`<header class="ah-header"><button id="ah-back" aria-label="กลับไปหน้าหลัก">← กลับ</button><div><small>VOCAB ARENA · ADMIN</small><h1 id="ah-heading">เลือกเพื่อนร่วมผจญภัย</h1></div><button id="ah-motion" aria-pressed="true">✦ ภาพเคลื่อนไหว</button></header><div class="ah-layout"><div class="ah-stage"><div class="ah-orbit"></div><div class="ah-portrait"></div><div class="ah-stage-label"><span id="ah-tag"></span><strong id="ah-name"></strong></div></div><div class="ah-info"><div class="ah-roster" role="group" aria-label="ตัวละคร ชาย 4 หญิง 4">${roster.map(h=>`<button class="ah-card" data-hero="${h.id}" aria-pressed="false" aria-label="${h.name} ${h.title} ${h.gender}"><img src="${h.thumb}" alt="" width="160" height="240"><span>${h.icon} ${h.name}</span><small>${h.gender}</small></button>`).join('')}</div><div class="ah-details" aria-live="polite"><div class="ah-detail-head"><h2 id="ah-title"></h2><span>HP 100</span></div><p id="ah-desc"></p><div id="ah-powers"></div><p class="ah-talent" id="ah-talent"></p></div><footer class="ah-footer"><div><span id="ah-ready" role="status">กำลังเตรียมสนาม…</span><small>ชุดพลังเริ่มต้น · เปลี่ยนพลังเพิ่มได้ในสนาม</small></div><button id="ah-play" disabled>เตรียมสนาม…</button></footer></div></div>`;
  document.body.append(root);const $=s=>root.querySelector(s),portrait=ArenaPortrait.create($('.ah-portrait'));let animation=!state.noAnim;
  function paint(){root.style.setProperty('--hero-color',selected.color);$('#ah-tag').textContent=selected.gender+' · '+selected.title;$('#ah-name').textContent=selected.name;$('#ah-title').textContent=selected.icon+' '+selected.title;$('#ah-desc').textContent=selected.desc;$('#ah-talent').textContent='พรธาตุ: '+ArenaElements.byId[selected.id].name+' ฟื้นพลังเร็วขึ้น 20%';$('#ah-powers').innerHTML=selected.slots.map(id=>{const p=ArenaElements.byId[id],cd=p.cd*(id===selected.id ? .8 : 1);return `<div class="ah-power"><b>${p.icon}</b><div><strong>${p.name} <em>${Number(cd.toFixed(1))} วิ</em></strong><span>${p.desc}</span></div></div>`;}).join('');root.querySelectorAll('[data-hero]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.hero===selected.id)));portrait.load(selected);}
  function toggle(){portrait.setEnabled(animation);$('#ah-motion').setAttribute('aria-pressed',String(animation));$('#ah-motion').textContent=animation?'✦ ภาพเคลื่อนไหว':'○ ภาพนิ่ง';}toggle();paint();
