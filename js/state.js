...
const DEFAULT_STATE = {
  ...
  lcAwardPaid:[],
  lcAwardLog:[],
  wshScore:0,                         // 🚢 กองเรือคำศัพท์ — คะแนนสะสม (กระดาน leaderboard)
  wshWords:0,
  wshIntro:0,
  ...
  adminAccess:false,                  // 🛡️ marker บัญชี Admin เดิม (Letter Cannon เปิดให้ผู้เล่นทุกคนแล้ว)
  letterCannonAnnouncementSeen:false, // 🔤💥 จำว่าผู้เล่นเห็น/ตอบป้ายเปิดตัวเกมแล้ว (sync ไปกับเซฟผู้เล่น)
...
function loadState(){
  try{
    ...
    if(raw){
      ...
      if(!Array.isArray(s.acPurchaseLog)) s.acPurchaseLog = [];
      s.acPurchaseLog = s.acPurchaseLog.filter(x=>x && x.home === 'medium' && x.price === 25000 && typeof x.at === 'number').slice(-20);
      if(!s.acDuplicateRefunds || typeof s.acDuplicateRefunds !== 'object' || Array.isArray(s.acDuplicateRefunds)) s.acDuplicateRefunds = {};
      if(!s.acDuplicateRefundNotice || typeof s.acDuplicateRefundNotice !== 'object' || Array.isArray(s.acDuplicateRefundNotice)) s.acDuplicateRefundNotice = null;
...
/* ข้อความเตือนมาตรฐาน — what = ชื่อของที่กำลังจะซื้อ (เช่น "เค้ก") */
function hungerSickMsg(what){
  const L = hungerSickLock();
  if(!L) return '';
  return `🤒 ${L.why} — ${L.fix} แล้วถึงจะซื้อ${what || 'ของกิน'}ได้นะ`;
}
