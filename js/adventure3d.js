/* ============================================================
   ...
   🛸 drone = โลกโดรน FPV Racing (รอบ 85): 50🪙/คำ · เร็ว/คล่องกว่าเฮลิฯ
              บินลอดหน้าต่างเข้าตึกร้าง เก็บตัวอักษรในห้องตามลำดับ (บินเฉียด ไม่ต้องจอด)
...
const HAUNT_ATTACKS = 10;       // 👻 ผีโจมตีครบ 10 ครั้ง = Game Over
const HAUNT_IFRAME = 1200;      // กันเหตุโจมตีซ้อนในเฟรมเดียวกัน (ms)
...
const MODES = {
  ...
  drone: {
    label:'โลกโดรน FPV', emoji:'🛸', reward:50, doneKey:'droneDone',
    shoot:false, ghost:false, drone:true,
    sky:0x9aa6b2, fogN:32, fogF:135, ground:0x45484d,
    intro:'🛸 <b>โลกโดรน FPV Racing!</b><br><small>บินเร็วสุดๆ ลอด<b>หน้าต่างตึกร้าง</b>เข้าไปในห้องต่างๆ<br>เก็บเฉพาะตัวอักษรของคำปัจจุบัน<b>ตามลำดับ</b> (ไม่ต้องจอด!) · ตัวละ 1🪙 · ครบคำ +50🪙<br>💨 เฉียดกำแพงหวุดหวิดแบบไม่ชน = เหรียญโบนัส (ยิ่งเฉียด+คอมโบ ยิ่งได้)!</small>',
  ...
  soccer: {
    ...
    sky:0x77c9f3, fogN:110, fogF:420, ground:0x3f9d43,
...
/* ==== ⚽🎵 รอบ 1339: Stadium Celebration — lazy stream + disk cache + exit fade ==== */
const SOCCER_BGM_BUILD_URL='__VW_SOCCER_BGM_URL__';
const SOCCER_BGM_URL=SOCCER_BGM_BUILD_URL.startsWith('__VW_')?'sound/football/Stadium_Celebration.mp3':SOCCER_BGM_BUILD_URL;
const SOCCER_BGM_VOLUME=.42, SOCCER_BGM_EXIT_FADE_MS=1100;
let soccerBgm=null, soccerBgmBtn=null, soccerBgmFadeTimer=0, soccerBgmFadeToken=0, soccerBgmPlayToken=0;
...
/* 🤖 รอบ 1482: วิถีกระสุนแบบกองเรือคำศัพท์ — G/มวล/ความเร็วปากกระบอกชุดเดียว */
const MECHA_G=8.4, MECHA_SHELL_MASS=1.2, MECHA_MUZZLE=62, MECHA_SHELL_HIT=2.2, MECHA_MAX_SHELLS=12;
/* 👾 รอบ 1511: ซอมบี้เลือดแดง (บอสยังต่างสีตามสายพันธุ์ · โครงร่างซอมบี้เหมือนกัน)
   geo() คืน geometry ใหม่ทุกครั้ง (dispose ได้อิสระ) */
const BOSS_SPECIES=[
  {key:'ember', name:'Ember', th:'อีมเบอร์ จอมเพลิง',   emoji:'🔥', geo:()=>new THREE.BoxGeometry(2.1,2.4,1.2), body:0x8a5a3a, emis:0x331108, eye:0xff4422, shot:0xff6a3a, scale:1.9,  shotSpd:1.0,  wordPick:8 },
  {key:'frost', name:'Frost', th:'ฟรอสต์ ราชันน้ำแข็ง', emoji:'❄️', geo:()=>new THREE.BoxGeometry(2.1,2.4,1.2), body:0x6a8a8a, emis:0x14384f, eye:0xff3344, shot:0x9fe6ff, scale:1.85, shotSpd:1.28, wordPick:8 },
  {key:'venom', name:'Venom', th:'เวน่อม พิษมรกต',      emoji:'🟢', geo:()=>new THREE.BoxGeometry(2.1,2.4,1.2), body:0x4a6a32, emis:0x1d4a12, eye:0xff2222, shot:0x9bff5a, scale:1.85, shotSpd:1.1,  wordPick:9 },
  {key:'volt',  name:'Volt',  th:'โวลต์ สายฟ้า',         emoji:'⚡', geo:()=>new THREE.BoxGeometry(2.1,2.4,1.2), body:0x7a7a3a, emis:0x5a4400, eye:0xff1a1a, shot:0xffe14d, scale:1.72, shotSpd:1.4,  wordPick:7 },
  {key:'titan', name:'Titan', th:'ไททัน เหล็กกล้า',      emoji:'🛡️', geo:()=>new THREE.BoxGeometry(2.4,2.7,1.4), body:0x6a7078, emis:0x2a3540, eye:0xff2a2a, shot:0xcfe0ff, scale:2.05, shotSpd:0.85, wordPick:10 },
];
...
function syncMechaFireBtns(){
  if(!overlayEl) return;
  ['#mecha-fire','#mecha-fire2'].forEach(sel=>{
    const el=overlayEl.querySelector(sel); if(!el) return;
    el.classList.toggle('on',!!mFireHeld);
    el.setAttribute('aria-pressed',mFireHeld?'true':'false');
  });
}
...
let mechaShells=[];   // 🤖 รอบ 1482: กระสุนวิถีโค้ง (physics ใน tickMechaShells)
let mechaSmoke=[], _mechaSmokeTex=null; // 💨 รอบ 1485: ควันหางมิสไซล์แบบยานแม่
...
let mCoinSaveT=0;   // 🤖 รอบ 1480: throttle เซฟเหรียญเข้ากระเป๋าหลัก/คลาวด์
...
let orderedWordProgress=0, orderedWrongAt=0; // 🚁🛸 โลกบินเก็บจากซ้ายไปขวาตามคำปัจจุบัน
let heliUsedRoofs=new Set();      // แยกตัวอักษรคนละดาดฟ้า กันป้ายซ้อนกันจนเห็นไม่ครบคำ
let droneUsedRooms=new Set();     // แยกตัวอักษรคนละห้องในตึกร้างเมื่อมีห้องเพียงพอ
function orderedLetterMode(){ return !!(M&&(M.heli||M.drone)); }
...
const SKY_IMG={ adv:'sky_day', heli:'sky_dawn', drone:'sky_storm', drive:'sky_dawn', soccer:'sky_soccer_day', mecha:'sky_day' };  // 🤖 รอบ 1480: หุ่นใช้ฟ้ากลางวัน
const SKY_EXT={sky_soccer_day:['avif','webp','jpg','png']};       // รอบ 1339: ภาพทึบใช้ AVIF; โลกเก่ายังคงยิง request แค่ jpg→png
...
function applySky(sc, mode){
  ...
  const set=img=>{ const tex=new THREE.CanvasTexture(seamlessSkyCanvas(img)); tex.needsUpdate=true;
    if(key==='sky_soccer_day'&&THREE.sRGBEncoding!==undefined) tex.encoding=THREE.sRGBEncoding;
    tex.mapping=THREE.EquirectangularReflectionMapping; skyTexCache[key]=tex; sc.background=tex; };
  const exts=SKY_EXT[key]||['jpg','png']; let i=0;
  const next=()=>{ if(i>=exts.length) return; const img=new Image();
    img.onload=()=>set(img); img.onerror=next; img.src='img/sky/'+key+'.'+exts[i++]; };
  next();
}
/* ============================================================
...
function applyTex(mat,key,rx,ry,tint,pngFirst,formats){ // formats = นามสกุลเรียงลำดับ เช่น ['.avif','.webp'] สำหรับ asset ใหม่
  if(!mat||!key) return;
  rx=rx||1; ry=ry||1;
  ...
  const use=(m,img)=>{
    ...
    if(key==='soccer_crowd_matchday'&&THREE.sRGBEncoding!==undefined) t.encoding=THREE.sRGBEncoding;
  ...
  const exts=formats&&formats.length?formats:[pngFirst?'.png':'.jpg',pngFirst?'.jpg':'.png']; let ei=0;
  const next=()=>{ if(ei>=exts.length){done(null);return;} const img=new Image();
    img.onload=()=>done(img); img.onerror=next; img.src='img/tex/'+key+exts[ei++]+assetRev; };
  next();
}
/* ============================================================
...
function buildScene(md){
  ...
  if(md==='adv'){
    ...
    sc.add(new THREE.HemisphereLight(0xf2fbff,0x56813d,.92));
    const sun=new THREE.DirectionalLight(0xfff4d0,1.08); sun.position.set(24,55,32); sc.add(sun);
    const fill=new THREE.DirectionalLight(0xdfe8ff,.30); fill.position.set(-30,40,-20); sc.add(fill);
...
function hotelSpot(self,floorReq){
  ...
  const byFloor=(floorReq===undefined)?sp:sp.filter(s=>HOTEL3D.floorOf(s.y)===floorReq);
  const free=byFloor.filter(s=>(cnt[s.room]||0)<HOTEL_PER_ROOM);
  const all=free.length?free:(byFloor.length?byFloor:sp); // เต็มทุกห้องจริง ๆ ค่อยยอมให้ล้น (ไม่ให้ตัวอักษรหาย)
...
function hotelQuestWordLetters(){
  const w=words[0];
  return w ? w.en.split('') : [];
}
function hotelStartQuestWord(){
...
function spawnLetter(ch,opt){
  ...
  if(M.drive){
    ...
    const free=rooms.filter(r=>!droneUsedRooms.has(r));
    const pool=free.length?free:rooms;
    const r=pool[Math.floor(Math.random()*pool.length)];
    droneUsedRooms.add(r);
    ...
    // 🏨 รอบ 1060: ตำแหน่งมาจากเส้นทางภารกิจชั้นที่กำหนด ไม่สุ่มข้ามชั้นแล้ว
    const p=opt.hotelPos||hotelSpot();
...
function pickUpLetter(i,fromPeer){
  const l=letters[i];
  if(!l) return;
  const ch=l.ch;
  ...
  if(orderedLetterMode()){
    const w=words[0], expected=w&&w.en[orderedWordProgress];
    if(!expected || ch!==expected){
      const now=performance.now();
      if(expected && now-orderedWrongAt>1200){
        orderedWrongAt=now;
  ...
  inv[ch]=(inv[ch]||0)+1;
  if(orderedLetterMode()) orderedWordProgress++;
  ...
  if(!M.hotel && !orderedLetterMode()) letterRespawns.push({ch,x:at.x,y:l.baseY,z:at.z,scale:l.spr.scale.x,room:l.room,at:performance.now()+LETTER_RESPAWN_MS});
  removeLetter(i);
...
function completeWord(i){
  ...
  if(M.hotel)return;
  if(orderedLetterMode()){ orderedWordProgress=0; heliUsedRoofs.clear(); droneUsedRooms.clear(); }
  const fresh=pickWords(1);                 // คำเดิมจบแล้ว → เติมคำใหม่ต่อท้ายคิว (8.4; โลกบินเรียงลำดับมีคิวเดียว)
...
function damagePlayer(n){
  ...
  renderHudTop();
  if(hp<=0){ if(M.hotel) hotelGameOver(); else knockedOut(); }
}
...
/* ============================================================
   ...
   ใกล้ที่สุด และข้ามผู้เล่นที่หลบอยู่ในห้องไม่เกิน 2 นาที
   ============================================================ */
...
function hotelGhostPlayers(){
  ...
  return context.players.map(player=>{
    ...
    return {id:player.id,x:player.x,y:player.y,z:player.z,
      ...
      roomBounds:R?{x0:R.x0,x1:R.x1,z0:Math.min(R.cz-HOTEL3D.CZ/2,R.cz+HOTEL3D.CZ/2),
        z1:Math.max(R.cz-HOTEL3D.CZ/2,R.cz+HOTEL3D.CZ/2)}:null};
  });
}
function hotelTurnScare(ms){
  ...
  overlayEl.classList.add('adv-jumpscare');
  ...
  HotelRuntime.later(()=>{
    ...
    if(overlayEl)overlayEl.classList.remove('adv-shake','adv-jumpscare');
  },duration);
}
function spawnGhost(){
  const fx=HauntedHotelGhost.create({
    THREE:THREE,scene:scene,camera:camera,
    onAttack:detail=>hotelGhostAttack(detail&&detail.kind||'chase'),
    onRoomIntrusion:()=>{
      HSound.wail();
      showBanner('👻 <b>ซ่อนเกิน 2 นาที!</b><br><small>ผีตามเข้ามาในห้องแล้ว — รีบหนีออกไป</small>',3200);
    },
    onBreathingStart:()=>{
...
function hotelGhostAttack(kind,force){
  ...
  const now=performance.now();
  if(!force&&hWardrobeTurn&&hWardrobeTurn.snapshot())return false;
  if(!force&&now<hurtUntil)return false;
  hurtUntil=now+HAUNT_IFRAME;
  hotelTurnScare(3000);
  hp=Math.max(0,hp-1);
  if(state.haptic!==false&&navigator.vibrate)navigator.vibrate([180,50,140]);
  dmgFlashEl.classList.remove('on');void dmgFlashEl.offsetWidth;dmgFlashEl.classList.add('on');
  renderHudTop();
  if(hp<=0)hotelGameOver();
  else showBanner(`💔 <b>ผีโจมตี ${HAUNT_ATTACKS-hp}/${HAUNT_ATTACKS} ครั้ง</b><br><small>เหลือพลังชีวิต ${hp} ช่อง</small>`,1800);
  return true;
}
...
/* เอฟเฟกต์ตกใจจากสภาพแวดล้อมเท่านั้น; ไม่หักพลังชีวิต (การโจมตีจริงผ่าน hotelGhostAttack) */
function hotelScare(){
  HSound.stinger();
...
let hPendingRoomId='';
let hWardrobeTurn=null;                   // เปิดตู้แล้วรอจนผู้เล่นหันไปทางอื่นจึง Jump Scare
let hotelGameOverShown=false;
...
function hotelReset(){
  ...
  hKnockAt=0; hActNow=null;hotelGameOverShown=false;
  hWardrobeTurn=HauntedHotelGhost.createWardrobeTurnTrigger();
  setTorch(false);
  if(hotel){
...
function tickHotelWorld(dt,now){
  ...
  const wardrobeScare=hWardrobeTurn&&hWardrobeTurn.update(yaw,now);
  if(wardrobeScare){
    hotelGhostAttack('wardrobe',true);
  }
...
function openWardrobe(W){
  if(!W || W.open) return;
  W.open=true; W.done=true; HSound.door();
  if(hWardrobeTurn)hWardrobeTurn.arm(W.slot||hotel.wardrobes.indexOf(W)+1,yaw,performance.now());
  if(!W.special) return;
  if(W.content==='letter'){
...
function sendPos(force){
  ...
  const hotelY=M.hotel?Math.round(hFootY*10)/10:null;
  if(!force && lastSent && lastSent.x===x && lastSent.z===z && lastSent.yaw===y && (!M.hotel||lastSent.hy===hotelY)) return;
  lastNetSend=now; lastSent={x,z,yaw:y,hy:hotelY};
  const payload={
  ...
  else if(M.hotel) payload.y=hotelY;                                    // โรงแรมหลายชั้น: เพื่อนต้องยืนชั้นจริง
...
function tickPeers(dt,now){
  ...
  Object.keys(peers).forEach(uid=>{
    ...
    if(p.bubble){
      ...
      else p.bubble.spr.position.set(p.cur.x,p.blk?3.65:p.mech?6.15:p.walk?((M.hotel?p.cur.y:0)+2.8):baseY+1.6,p.cur.z);   // ลอยตามหัว (โรงแรมบวกชั้นจริง)
    }
    if(p.micSpr) p.micSpr.position.set(p.cur.x,(p.blk?3.35:p.mech?5.75:p.walk?((M.hotel?p.cur.y:0)+2.55):baseY+1.22)+Math.sin(now/300)*.06,p.cur.z);
...
function syncPartyWord(){
  ...
  if(wordChanged){
    if(orderedLetterMode()){
      for(let i=letters.length-1;i>=0;i--) removeLetter(i);
      letterRespawns=[]; inv={}; orderedWordProgress=0; heliUsedRoofs.clear(); droneUsedRooms.clear();
...
function renderHudTop(){
  ...
  const hpBox=hudHpEl.parentElement;
  if(hpBox){
    if(mode==='haunt')hpBox.dataset.hp=`❤ พลังชีวิต ${hp}/${maxHp}`;
...
function renderHudInv(){
  ...
  hudInvEl.innerHTML=ks.length
    ...
    : `<span class="adv-inv-empty">${(M&&M.soccer)?'⚽ เตะบอลใส่ป้ายตัวอักษรเพื่อเก็บ':(M&&M.drone)?'🛸 บินเก็บตัวอักษรตามลำดับ ✨':'เดินชนตัวอักษรเพื่อเก็บ ✨'}</span>`;   // ⚽🛸 สนามบอลไม่ได้เดินเก็บ · โดรนต้องบอกลำดับ
}
...
function buildDom(){
  ...
  overlayEl.innerHTML=`
    ...
    <button id="adv-soccer-music" type="button" aria-pressed="true">🎵 เพลง เปิด</button>
  ...
  soccerBgmBtn=overlayEl.querySelector('#adv-soccer-music');
  soccerBgmBtn.addEventListener('click',e=>{ e.preventDefault(); e.stopPropagation(); soccerMusicToggle(); });
  document.addEventListener('visibilitychange',soccerMusicVisibilityChange);
  soccerMusicSyncButton();
  ...
  holdBtn('#mecha-fire',()=>{ mFireHeld=true; syncMechaFireBtns(); },()=>{ mFireHeld=false; syncMechaFireBtns(); });
  holdBtn('#mecha-fire2',()=>{ mFireHeld=true; syncMechaFireBtns(); },()=>{ mFireHeld=false; syncMechaFireBtns(); });   /* รอบ 223: ปุ่มยิงตัวที่ 2 (ใต้ minimap) ยิงเหมือนกัน */
  ...
  if(!IS_TOUCH){
    ...
    overlayEl.addEventListener('touchstart',e=>{
      ...
      for(const t of e.changedTouches){
        if(t.target.closest('#adv-shoot,#adv-horn,#adv-exit,#adv-help,#adv-intro,#adv-banner,#adv-hh-hint,#adv-hh-history,#adv-chat-btn,#adv-chat-box,.adv-vbtn,#adv-podium,#adv-reply,#adv-map,#adv-bigmap,#adv-aimpad,#adv-kick,#adv-scam,#adv-pk,#adv-fk,#adv-soccer-music,#adv-spinbtn,#adv-spinpad,#adv-soccerstart,.mecha-btn,#adv-wiper,#adv-seat,#adv-skipstart,#adv-visor,#adv-light,#adv-wing,#adv-tour,#adv-adshop,#adv-adshop-dlg,#adv-torch,#adv-use,#adv-act')) continue;   /* รอบ 684: +ไฟฉาย/ปุ่มใช้งานโรงแรมผีสิง */   /* รอบ 346: +ที่ปัดน้ำ/มุมนั่ง/ข้ามสตาร์ท — อยู่ครึ่งขวา ถ้าไม่กันไว้ นิ้วที่กดปุ่มจะกลายเป็นลากคันเร่ง · รอบ 350: +ม่านบังแดด(ตกหล่นจากรอบ 348!)/ไฟส่อง */  /* #adv-words เอาออก — เป็น pointer-events:none แล้ว นิ้วโดนคันบังคับได้ · รอบ 144: +map/bigmap · รอบ 196: +soccer · รอบ 199: +mecha */
...
function soccerTurfGrade(c,S){
  ...
  const SAT=.72, BRI=.84;                        // รอบ 1339 QA: เขียวสดแบบสนามจริง ไม่ซีดและไม่เรืองนีออน
...
function soccerTurfTexture(){
  ...
  t.repeat.set(28,38);                                      // รอบ 1339: ให้เนื้อหญ้าอ่านออกบนมือถือ ไม่ถี่จนกลายเป็นสีเรียบ
  const img=new Image();
...
function grassNormalTexture(){
  ...
  const t=new THREE.CanvasTexture(cv);
  t.wrapS=t.wrapT=THREE.RepeatWrapping; t.repeat.set(32,44);   // รอบ 1339: normal สอดคล้องความถี่ texture ใหม่
  return t;
}
...
/* ==== ⚽🏟️ รอบ 1339: Match-day stadium bowl — crowd façade + concourse + canopy + lattice floodlights ==== */
function soccerFloodTexture(){
  const cv=document.createElement('canvas'); cv.width=256; cv.height=128;
  const c=cv.getContext('2d'); c.fillStyle='#17222d'; c.fillRect(0,0,256,128);
  for(let r=0;r<3;r++) for(let k=0;k<6;k++){
    const x=24+k*42,y=23+r*41,g=c.createRadialGradient(x,y,1,x,y,16);
    g.addColorStop(0,'#ffffff'); g.addColorStop(.24,'#fffde0'); g.addColorStop(.52,'rgba(255,248,190,.65)'); g.addColorStop(1,'rgba(255,248,190,0)');
    c.fillStyle=g; c.fillRect(x-17,y-17,34,34);
  }
  return new THREE.CanvasTexture(cv);
}
function soccerScoreboardTexture(){
  const cv=document.createElement('canvas'); cv.width=512; cv.height=192;
  const c=cv.getContext('2d'),g=c.createLinearGradient(0,0,0,192);
  g.addColorStop(0,'#153d49'); g.addColorStop(1,'#061c24'); c.fillStyle=g; c.fillRect(0,0,512,192);
  c.strokeStyle='#72e0bd'; c.lineWidth=9; c.strokeRect(6,6,500,180);
  c.textAlign='center'; c.textBaseline='middle'; c.fillStyle='#ffffff'; c.font='900 48px Arial'; c.fillText('PLAY TOGETHER',256,78);
  c.fillStyle='#ffe16b'; c.font='900 30px Arial'; c.fillText('VOCAB WORLD  ⚽',256,137);
  return new THREE.CanvasTexture(cv);
}
function buildStands(sc,fw,fl){
  const wallM=new THREE.MeshLambertMaterial({color:0x193543});
  const concreteM=new THREE.MeshLambertMaterial({color:0xbac6ca});
  const roofM=new THREE.MeshLambertMaterial({color:0x18313e});
  const fasciaM=new THREE.MeshLambertMaterial({color:0x456f76});
  const flagM=new THREE.MeshBasicMaterial({color:0xffd631,side:THREE.DoubleSide});
  const crowdMat=repeat=>{
    const tex=soccerCrowdTexture(); tex.repeat.set(repeat,1);
    const m=new THREE.MeshBasicMaterial({map:tex,side:THREE.DoubleSide});
    applyTex(m,'soccer_crowd_matchday',repeat,1,null,false,['.avif','.webp']);
    return m;
  };
  const hw=fw/2+9, hl=fl/2+9;                  // รอบ 1339 QA: ถอยอัฒจันทร์ออก ให้กรอบสนามและท้องฟ้าหายใจแบบต้นแบบ
  const side=(len,px,pz,ry,repeat,score)=>{
    const g=new THREE.Group(); g.position.set(px,0,pz); g.rotation.y=ry;        // local +z = ถอยออกจากสนาม
    const back=new THREE.Mesh(new THREE.PlaneGeometry(len,13.8),wallM); back.position.set(0,6.9,7); g.add(back);
    const crowd=new THREE.Mesh(new THREE.PlaneGeometry(len,12.4),crowdMat(repeat));
    crowd.position.set(0,6.7,3.8); crowd.rotation.x=.10; g.add(crowd);          // รอบ 1339 QA: สูงพอดี 3 ชั้น ไม่กินพื้นที่ฟ้า
    const front=new THREE.Mesh(new THREE.BoxGeometry(len+1.5,1.35,.65),wallM); front.position.set(0,.68,-.05); g.add(front);
    const frontBand=new THREE.Mesh(new THREE.BoxGeometry(len+1.8,.38,.82),fasciaM); frontBand.position.set(0,1.42,-.12); g.add(frontBand);
    [[4.25,2.0],[8.55,3.45]].forEach(([y,z])=>{
      const slab=new THREE.Mesh(new THREE.BoxGeometry(len+1.2,.34,3.2),concreteM); slab.position.set(0,y,z); g.add(slab);
      const rail=new THREE.Mesh(new THREE.BoxGeometry(len+1.3,.32,.18),fasciaM); rail.position.set(0,y+.58,z-1.48); g.add(rail);
    });
    const roof=new THREE.Mesh(new THREE.BoxGeometry(len+3,.42,7.2),roofM); roof.position.set(0,13.65,6.05); roof.rotation.x=.035; g.add(roof);
    const roofEdge=new THREE.Mesh(new THREE.BoxGeometry(len+3,.68,.35),fasciaM); roofEdge.position.set(0,13.27,2.54); g.add(roofEdge);
    for(let i=0;i<6;i++){
      const x=-len*.42+i*len*.168;
      const pole=new THREE.Mesh(new THREE.BoxGeometry(.07,1.25,.07),concreteM); pole.position.set(x,14.55,2.5); g.add(pole);
      const flag=new THREE.Mesh(new THREE.PlaneGeometry(.72,.38),flagM); flag.position.set(x+.38,14.86,2.48); g.add(flag);
    }
    if(score){
      const sm=new THREE.MeshBasicMaterial({map:soccerScoreboardTexture(),side:THREE.DoubleSide});
      const board=new THREE.Mesh(new THREE.PlaneGeometry(9.2,3.45),sm); board.position.set(0,16.45,6.4); g.add(board);
      [-3.4,3.4].forEach(x=>{ const p=new THREE.Mesh(new THREE.BoxGeometry(.22,3,.22),wallM); p.position.set(x,14.85,6.48); g.add(p); });
    }
    sc.add(g);
  };
  side(fw+18,0,-hl,Math.PI,3,true); side(fw+18,0,hl,0,3,false);
  side(fl+18,-hw,0,-Math.PI/2,4,false); side(fl+18,hw,0,Math.PI/2,4,false);

  const towerM=new THREE.MeshLambertMaterial({color:0x8499a5});
  const barV=new THREE.BoxGeometry(.16,21,.16), barH=new THREE.BoxGeometry(1.5,.11,.11);
  const floodM=new THREE.MeshBasicMaterial({map:soccerFloodTexture(),side:THREE.DoubleSide});
  [[-hw-5,-hl-5],[hw+5,-hl-5],[-hw-5,hl+5],[hw+5,hl+5]].forEach(([x,z])=>{
    const mast=new THREE.Group(); mast.position.set(x,0,z); mast.rotation.y=Math.atan2(x,z);
    [-.58,.58].forEach(dx=>{ const b=new THREE.Mesh(barV,towerM); b.position.set(dx,10.5,0); mast.add(b); });
    for(let i=1;i<8;i++){ const b=new THREE.Mesh(barH,towerM); b.position.set(0,i*2.55,0); b.rotation.z=i%2?.11:-.11; mast.add(b); }
    sc.add(mast);
    const panel=new THREE.Mesh(new THREE.PlaneGeometry(5.1,2.8),floodM); panel.position.set(x,22.4,z); panel.lookAt(0,4,0); sc.add(panel);
  });
}
...
/* ==== ⚽🎵 รอบ 1339: BGM ฟุตบอล — สร้างเมื่อกดลงสนามเท่านั้น, metadata stream, disk cache, fade ตอนออก ==== */
function soccerMusicCanPlay(){
  return soccerMusicSession&&soccerMusicEnabled&&!document.hidden&&!(typeof state!=='undefined'&&state.sound===false);
}
function soccerMusicSyncButton(){
  if(!soccerBgmBtn)return;
  const masterOff=typeof state!=='undefined'&&state.sound===false;
  const on=soccerMusicEnabled&&!masterOff&&!soccerBgmBlocked;
  soccerBgmBtn.setAttribute('aria-pressed',on?'true':'false');
  soccerBgmBtn.classList.toggle('blocked',soccerBgmBlocked);
  soccerBgmBtn.textContent=soccerBgmBlocked?'⚠️ แตะเปิดเพลง':(on?'🎵 เพลง เปิด':'🔇 เพลง ปิด');
  soccerBgmBtn.title=masterOff?'เสียงหลักของเกมถูกปิดอยู่':'Stadium Celebration — เปิด/ปิดเพลงโลกฟุตบอล';
}
function soccerMusicEnsure(){
  if(soccerBgm)return soccerBgm;
  const a=new Audio(); a.preload='metadata'; a.loop=true; a.volume=SOCCER_BGM_VOLUME; a.src=SOCCER_BGM_URL;
  a.addEventListener('error',()=>{soccerBgmBlocked=true;soccerMusicSyncButton();}); soccerBgm=a; return a;
}
function soccerMusicCancelFade(){
  soccerBgmFadeToken++; if(soccerBgmFadeTimer){clearTimeout(soccerBgmFadeTimer);soccerBgmFadeTimer=0;}
}
function soccerMusicStart(){
  soccerMusicCancelFade(); soccerBgmBlocked=false; soccerMusicSyncButton();
  if(!soccerMusicCanPlay())return Promise.resolve(false);
  const a=soccerMusicEnsure(),token=++soccerBgmPlayToken; a.volume=SOCCER_BGM_VOLUME;
  const p=a.play(); if(!p||!p.then)return Promise.resolve(true);
  return p.then(()=>{if(token!==soccerBgmPlayToken||!soccerMusicSession){a.pause();return false;}soccerBgmBlocked=false;soccerMusicSyncButton();return true;})
    .catch(()=>{if(token===soccerBgmPlayToken){soccerBgmBlocked=true;soccerMusicSyncButton();}return false;});
}
function soccerMusicStop(fadeMs=0,reset=false,done){
  soccerMusicCancelFade(); soccerBgmPlayToken++; const a=soccerBgm;
  const finish=()=>{if(a){a.pause();a.volume=SOCCER_BGM_VOLUME;if(reset){try{a.currentTime=0;}catch(_e){}}}soccerBgmFadeTimer=0;if(done)done();};
  if(!a||a.paused||fadeMs<=0){finish();return;}
  const startAt=performance.now(),startVol=a.volume,fadeToken=++soccerBgmFadeToken;
  const step=()=>{if(fadeToken!==soccerBgmFadeToken)return;const k=Math.min(1,(performance.now()-startAt)/fadeMs);
    a.volume=Math.max(0,startVol*(1-k));if(k>=1)finish();else soccerBgmFadeTimer=setTimeout(step,40);}; step();
}
function soccerMusicSessionStart(){
  soccerMusicSession=true; soccerMusicEnabled=true; soccerBgmBlocked=false; soccerMusicSyncButton(); soccerMusicStart();
}
function soccerMusicToggle(){
  if(soccerBgmBlocked){soccerBgmBlocked=false;soccerMusicEnabled=true;soccerMusicSyncButton();soccerMusicStart();return;}
  soccerMusicEnabled=!soccerMusicEnabled; soccerMusicSyncButton();
  if(soccerMusicEnabled)soccerMusicStart();else soccerMusicStop(320,false);
}
function soccerMusicVisibilityChange(){
  if(!soccerMusicSession)return; if(document.hidden)soccerMusicStop(0,false);else soccerMusicStart();
}
...
function buildAura(sc){
  ...
  // 🔥 รอบ 941 (ผู้ใช้): เปลวฟ้า "ก้อนใหญ่ก้อนเดียว" — สไปรต์ใหญ่ 4 ชั้นซ้อนแกนเดียวกัน additive รวมเป็นเนื้อเดียว
  //    เปลือกนอกกว้าง 2.6m (ตัวกว้าง .58) → แกนในสว่างแคบ · ทุกชั้นเอนทางเดียวกัน = โยกทั้งก้อน
  //    📏 รอบ 947 (ผู้ใช้): เตี้ยลง — ยอดพ้นหัว (1.72m) ราว 2/4 ของความสูงตัว (≈.86) = ยอด ~2.6m (เดิมเสา 9.5m)
  [ {w:2.6,h:3.3,op:.6}, {w:2.1,h:3.05,op:.66}, {w:1.55,h:2.8,op:.72}, {w:1.05,h:2.5,op:.78} ]   // จูนจากภาพ: ปลายเทกซ์เจอร์อยู่ ~88% ของสไปรต์ → h 3.3 ให้ยอดจริง ~2.6m · op เพิ่มเพราะก้อนเตี้ยแสงรวมน้อยลง
  .forEach((L,i)=>{
...
function auraTick(dt,now){
  ...
  auraRings.forEach(fl=>{
    ...
    const h=fl.h*lick;
    fl.m.position.set(swayX*(h/2.8), h*.5+.05, swayZ*(h/2.8));   // ชั้นสูงกว่าเอนมากกว่า = ก้อนโค้งลู่ลมทั้งก้อน (รอบ 947: หาร 9→2.8 ตามก้อนที่เตี้ยลง)
    fl.m.scale.set(fl.w*(1.1-lick*.18), h, 1);
    fl.m.material.opacity=fl.op*(.8+.25*Math.sin(tS*9+fl.p2));
  });
  // 🌪️ รอบ 947 (ผู้ใช้): กลับทิศเป็น "หมุนลง" (บวก = แพตเทิร์นเกลียวเลื่อนลง เพราะ helix พันทวนเข็มตามความสูง)
  //    ความเร็วคง 12.6 rad/s (×3 จากรอบ 944)
  auraCoil.forEach((cl,i)=>{
    cl.m.rotation.y=tS*12.6;                                 // หมุนลงต่อเนื่องไม่มีหยุด (สายสองห่างครึ่งรอบใน geometry)
    cl.m.material.map.offset.x=(-tS*3.45)%1;                 // เปลวไหลตามแนวเกลียวขาลง (เร็วเท่ากัน)
    cl.m.material.opacity=.72+.2*Math.sin(tS*7+i*2.1);
    const p=1+.05*Math.sin(tS*5.3+i);                        // สูบ-พองเบา ๆ ให้มีชีวิต
    cl.m.scale.set(p,1,p);
  });
  // ✨ รอบ 946: ประกายไต่กรวย — r/y ใช้สูตรเดียวกับ auraCoilRibbon (CH=2.15, exp 1.4)
  //    มุมหมุนเท่าเกลียว (ติดผิวกรวยไปด้วยกัน) · t ไต่วนซ้ำ 0→1 คนละจังหวะ (ph สุ่ม)
  ...
  auraSparks.forEach(s=>{                                    // ember ฟ้า: ลอยขึ้นตามเสาไฟแล้วจางหาย วนใหม่
    ...
    s.m.position.set(Math.cos(a)*s.rr*(1-t*.35), .3+t*2.5, Math.sin(a)*s.rr*(1-t*.35));   // รอบ 947: ลอยถึง ~2.8m ตามก้อนไฟที่เตี้ยลง
...
function soccerCamera(dt,dx,dz){
  ...
  const camD = sbLive?8:5.2, camH = sbLive?3.4:1.0;          // รอบ 1339 QA: กล้องต่ำพอเห็นนักเตะเต็มตัว ขณะเส้นขอบฟ้ายังอยู่ระดับต้นแบบ
  ...
  // รอบ 1339 QA: มองเลยตัวผู้เล่นไปด้านหน้า ลดการก้มจ่อพื้น ให้เส้นขอบฟ้า/สนาม/นักเตะอยู่สัดส่วนเดียวกับภาพต้นแบบ
  camera.lookAt(foc.x+dx*5, foc.y+0.3, foc.z+dz*5);
}
...
function soccerKitGo(){
  ...
  soccerMusicSessionStart();                            // 🎵 รอบ 1339: เริ่มหลัง gesture กดลงสนาม จึงไม่ดึง MP3 ก่อนเข้าจริง
...
/* ============================================================
   ...
   ============================================================ */
/* 🔊 รอบ 1502: ตัด allroboteffect.mp3 → คลิปย่อยใน /sound/robot/mecha/ ใช้ร่วมทุกหุ่นตามสถานการณ์ */
const MechaAudio={
  ctx:null,
  _rr:{},
  _live:null,
  clips:{
    fire:{file:'fire.mp3',dir:'/sound/robot/',hash:'6cc1dcf3aaf465f9',vol:.62,gap:120,sliceMs:320},
    fireVarA:{file:'fire_var_a.mp3',dir:'/sound/robot/mecha/',hash:'adfec9fc4b85d5f6',vol:.58,gap:110},
    fireVarB:{file:'fire_var_b.mp3',dir:'/sound/robot/mecha/',hash:'6817ffed74d8bf73',vol:.58,gap:110},
    impactA:{file:'impact_a.mp3',dir:'/sound/robot/mecha/',hash:'73b3bb53b2820dde',vol:.6,gap:160},
    impactB:{file:'impact_b.mp3',dir:'/sound/robot/mecha/',hash:'77ab056a69bbdbb5',vol:.6,gap:160},
    explodeA:{file:'explode_a.mp3',dir:'/sound/robot/mecha/',hash:'e587405561f9fb7b',vol:.68,gap:280},
    explodeB:{file:'explode_b.mp3',dir:'/sound/robot/mecha/',hash:'326a8b6704d40f59',vol:.68,gap:280},
    stepA:{file:'step_a.mp3',dir:'/sound/robot/mecha/',hash:'a3a34d4711784692',vol:.42,gap:140},
    stepB:{file:'step_b.mp3',dir:'/sound/robot/mecha/',hash:'829f371071d3c7bc',vol:.42,gap:140},
    warnA:{file:'warn_a.mp3',dir:'/sound/robot/mecha/',hash:'fc64051afc686c67',vol:.55,gap:220},
    warnB:{file:'warn_b.mp3',dir:'/sound/robot/mecha/',hash:'71730083817c8338',vol:.55,gap:220},
    enemyA:{file:'enemy_a.mp3',dir:'/sound/robot/mecha/',hash:'d5da3b931161fb90',vol:.5,gap:140},
    enemyB:{file:'enemy_b.mp3',dir:'/sound/robot/mecha/',hash:'1d06f7d90af4516c',vol:.5,gap:140},
    pickupA:{file:'pickup_a.mp3',dir:'/sound/robot/mecha/',hash:'24432d40f6d5a31f',vol:.55,gap:180},
    pickupB:{file:'pickup_b.mp3',dir:'/sound/robot/mecha/',hash:'9727e252a110f29c',vol:.55,gap:180},
    shield:{file:'shield.mp3',dir:'/sound/robot/mecha/',hash:'bbdcfa9f3246d364',vol:.52,gap:200},
    boomArena:{file:'fire-a6fea31058694941.mp3',dir:'/sound/arena/',hash:'a6fea31058694941',vol:.55,gap:220}
  },
  ac(){ if(!this.ctx){ try{ this.ctx=new (window.AudioContext||window.webkitAudioContext)(); }catch(e){} } return this.ctx; },
  fireOn(){ return !!(typeof state!=='undefined' && state.sound!==false) && !(typeof document!=='undefined' && document.hidden); },
  _slot(name){
    const def=this.clips[name]; if(!def) return null;
    if(!this._live) this._live={};
    let L=this._live[name];
    if(!L){
      L=this._live[name]={file:def.file,dir:def.dir,hash:def.hash,vol:def.vol,gap:def.gap,sliceMs:def.sliceMs||0,
        sfx:null,blob:null,url:'',load:null,busy:false,at:-1e9,gen:0,stopT:0};
    }
    return L;
  },
  _pick(group){
    const key=group.join('|');
    const i=(this._rr[key]|0)%group.length; this._rr[key]=(i+1)%group.length; return group[i];
  },
  loadClip(name){
    const F=this._slot(name); if(!F) return Promise.resolve(null);
    if(F.blob) return Promise.resolve(F.blob); if(F.load) return F.load;
    const path=F.dir+F.file, key=(typeof location!=='undefined'?location.origin:'')+'/__vw_asset__'+path+'?v='+F.hash;
    F.load=Promise.resolve().then(()=>{
      if(typeof caches==='undefined') return null;
      return caches.open('vw-assets-content-v1').then(cache=>cache.match(key).then(hit=>hit?{cache,hit}:{cache})).catch(()=>({}));
    }).then(got=>{
      got=got||{}; if(got.hit) return got.hit.blob();
      if(typeof fetch!=='function') return null;
      return fetch(path).then(res=>{ if(!res.ok) throw 0; if(got.cache) got.cache.put(key,res.clone()).catch(()=>{}); return res.blob(); });
    }).then(blob=>blob?(F.blob=blob):null).catch(()=>null).finally(()=>{ F.load=null; });
    return F.load;
  },
  prepareFire(){
    if(!this.fireOn()) return;
    ['fire','fireVarA','fireVarB','impactA','impactB','explodeA','explodeB','stepA','stepB','warnA','warnB','enemyA','enemyB','pickupA','pickupB','shield','boomArena']
      .forEach(n=>void this.loadClip(n));
  },
  stopClip(name){
    const F=this._slot(name); if(!F) return;
    F.gen++; F.busy=false; F.at=-1e9;
  ...
  stopFireClip(){ Object.keys(this.clips).forEach(n=>this.stopClip(n)); },
  playClip(name){
    if(!this.fireOn()) return false;
    const F=this._slot(name); if(!F) return false;
    const now=(typeof performance!=='undefined'?performance.now():0);
    if(F.busy || now-F.at<(F.gap||250)) return false;
    const Ctor=typeof Audio!=='undefined'?Audio:(typeof window!=='undefined'?window.Audio:null);
    if(!Ctor) return false;
    try{
      if(!F.sfx){ F.sfx=new Ctor(); F.sfx.preload='none'; F.sfx.volume=F.vol!=null?F.vol:.55; }
      else F.sfx.volume=F.vol!=null?F.vol:.55;
      const el=F.sfx, at=F.gen; F.busy=true; F.at=now;
      ...
      return true;
    }catch(e){ F.busy=false; return false; }
  },
  playAny(names){ for(let i=0;i<names.length;i++){ if(this.playClip(names[i])) return true; } return false; },
  playFireClip(){ return this.playClip('boomArena'); },
  step(){
    if(this.playAny([this._pick(['stepA','stepB'])])) return;
  ...
  fire(color){
    /* FIRE: สลับ fire.mp3 ตัดสั้น + คลิปสั้นจากแพ็ก (ทุกหุ่นชุดเดียวกัน) */
    if(this.playAny([this._pick(['fire','fireVarA','fireVarB'])])) return;
    if(!state.sound) return; const c=this.ac(); if(!c) return; const t=c.currentTime;
  ...
  impactBoom(){
    if(this.playAny([this._pick(['impactA','impactB']),'boomArena'])) return;
    this.boomSynth();
  },
  boom(){
    if(this.playAny([this._pick(['explodeA','explodeB']),'boomArena'])) return;
    this.boomSynth();
  },
  boomSynth(){ if(!state.sound) return; const c=this.ac(); if(!c) return; const t=c.currentTime;
  ...
  warn(){
    if(this.playAny([this._pick(['warnA','warnB'])])) return;
    if(!state.sound) return; const c=this.ac(); if(!c) return; const t=c.currentTime;
  ...
  enemyShot(){
    if(this.playAny([this._pick(['enemyA','enemyB'])])) return;
    if(!state.sound) return; const c=this.ac(); if(!c) return; const t=c.currentTime;
  ...
  pickup(){
    if(this.playAny([this._pick(['pickupA','pickupB'])])) return;
    if(!state.sound) return; const c=this.ac(); if(!c) return; const t=c.currentTime;
  ...
  shield(){
    if(this.playClip('shield')) return;
    this.pickup();
  },
  /* 🎵 รอบ 1517: BGM หุ่น — โหลดตอนเข้าโลกเท่านั้น · Cache ตาม content-hash · เล่นจาก Blob (ไม่ decode PCM) */
  bgmMeta:{file:'bgm.mp3',dir:'/sound/robot/',hash:'55c48aa88a519fcb',vol:.36},
  _bgm:null,
  _bgmEnsure(){
    if(this._bgm) return this._bgm;
    const self=this;
    this._bgm={sfx:null,blob:null,url:'',load:null,session:false,fadeT:0,playTok:0,visBound:false};
    if(typeof document!=='undefined' && !this._bgm.visBound){
      document.addEventListener('visibilitychange',()=>{ if(self._bgm&&self._bgm.session) self.syncBgm(); });
      this._bgm.visBound=true;
    }
    return this._bgm;
  },
  loadBgm(){
    const B=this._bgmEnsure(), M=this.bgmMeta;
    if(B.blob) return Promise.resolve(B.blob); if(B.load) return B.load;
    const path=M.dir+M.file, key=(typeof location!=='undefined'?location.origin:'')+'/__vw_asset__'+path+'?v='+M.hash;
    B.load=Promise.resolve().then(()=>{
      if(typeof caches==='undefined') return null;
      return caches.open('vw-assets-content-v1').then(cache=>cache.match(key).then(hit=>hit?{cache,hit}:{cache})).catch(()=>({}));
    }).then(got=>{
      got=got||{}; if(got.hit) return got.hit.blob();
      if(typeof fetch!=='function') return null;
      return fetch(path).then(res=>{ if(!res.ok) throw 0; if(got.cache) got.cache.put(key,res.clone()).catch(()=>{}); return res.blob(); });
    }).then(blob=>blob?(B.blob=blob):null).catch(()=>null).finally(()=>{ B.load=null; });
    return B.load;
  },
  syncBgm(){
    const B=this._bgm; if(!B||!B.session) return;
    if(!this.fireOn()){ if(B.sfx&&!B.sfx.paused) try{ B.sfx.pause(); }catch(e){} return; }
    this.startBgm();
  },
  startBgm(){
    if(!(typeof state!=='undefined' && state.sound!==false)) return false;
    if(typeof document!=='undefined' && document.hidden) return false;
    const B=this._bgmEnsure(), M=this.bgmMeta, Ctor=typeof Audio!=='undefined'?Audio:(typeof window!=='undefined'?window.Audio:null);
    if(!Ctor) return false;
    B.session=true;
    if(B.fadeT){ try{ clearTimeout(B.fadeT); }catch(e){} B.fadeT=0; }
    try{
      if(!B.sfx){ B.sfx=new Ctor(); B.sfx.preload='none'; B.sfx.loop=true; B.sfx.volume=M.vol; }
      else{ B.sfx.loop=true; B.sfx.volume=M.vol; }
      const el=B.sfx, tok=++B.playTok;
      if(B.blob&&B.url&&!el.paused) return true;
      void this.loadBgm().then(blob=>{
        if(!blob||el!==B.sfx||tok!==B.playTok||!B.session||!this.fireOn()) return;
        if(!B.url){ B.url=URL.createObjectURL(blob); el.src=B.url; }
  ...
  stopBgm(fadeMs=800){
    const B=this._bgm; if(!B) return;
    B.session=false; B.playTok++;
    if(B.fadeT){ try{ clearTimeout(B.fadeT); }catch(e){} B.fadeT=0; }
    const a=B.sfx;
    const finish=()=>{ if(a){ try{ a.pause(); a.volume=this.bgmMeta.vol; }catch(e){} } B.fadeT=0; };
    if(!a||a.paused||!(fadeMs>0)){ finish(); return; }
    const startAt=(typeof performance!=='undefined'?performance.now():Date.now()), startVol=a.volume;
    const step=()=>{
      if(B.session) return;
      const k=Math.min(1,(((typeof performance!=='undefined'?performance.now():Date.now())-startAt)/fadeMs));
      try{ a.volume=Math.max(0,startVol*(1-k)); }catch(e){}
      if(k>=1) finish(); else B.fadeT=setTimeout(step,40);
    };
    step();
  },
};
...
function makeAlien(bossArg){
  ...
  word.en.split('').forEach((ch,i)=>{
    ...
    const off={x:(i-(n-1)/2)*2.3, y:3.5, z:0};
  ...
  grp.position.set(p.x,1.55,p.z); if(boss) grp.scale.setScalar(scl); scene.add(grp);
...
function mechaBankCoins(n){
  const amt=Math.max(0,Number(n)||0); if(!amt) return 0;
  addCoins(amt); sessionCoins+=amt;
  saveState();
  if(typeof authPushSave==='function'){
    if(mCoinSaveT) clearTimeout(mCoinSaveT);
    mCoinSaveT=setTimeout(()=>{ mCoinSaveT=0; try{ authPushSave(false); }catch(e){} },700);
  }
  return amt;
}
...
function setMechaHudSkin(rid){
  ...
  if(mhUI.fireBtns) mhUI.fireBtns.forEach(b=>{ if(!b) return; b.style.borderColor=hex; b.style.color=hex; b.style.textShadow='0 0 6px '+hex; });
...
function mechaHitByAlien(a){
  if(mechaShielded(performance.now())){ MechaAudio.shield(); if(a&&a.grp){ const gs=a.gs||1; a.grp.scale.setScalar(gs*.85); setTimeout(()=>{ if(a.grp) a.grp.scale.setScalar(gs); },140);} return; }  // 🛡️ โล่กัน = ไม่เสียหาย (ดันเอเลี่ยนหด)
...
function spawnAlienShot(a){
  const from=a.grp.position.clone(); from.y=(a.baseY||1.55)+1.4;
  ...
  const m=new THREE.Mesh(new THREE.SphereGeometry(a.boss?.6:.4,10,8),
    new THREE.MeshBasicMaterial({color:a.boss?(a.species?a.species.shot:0xff3b6b):0xc41e1e}));
  m.position.copy(from); scene.add(m);
  const spd=ALIEN_SHOT_SPD*(a.boss?(a.species?a.species.shotSpd:1.1):1)*(mWaveSpd||1);
...
function tickAlienShots(dt,now){
  for(let i=alienShots.length-1;i>=0;i--){
    ...
    if(s.mesh.position.distanceTo(camera.position)<2.2){          // โดนหุ่น
      if(mechaShielded(now)){ MechaAudio.shield(); removeAlienShot(i); continue; }   // 🛡️ โล่กันกระสุน (ไม่เสียหาย)
...
function mechaTracer(wx,wy,wz,hit){
  /* คง API เดิมให้ preview/test — เกมจริงใช้ spawnMechaShell วิถีโค้ง */
  const dir=new THREE.Vector3(); camera.getWorldDirection(dir);
...
function mechaMuzzlePoint(side=0){
  const dir=new THREE.Vector3(); camera.getWorldDirection(dir);
  const from=camera.position.clone().addScaledVector(dir,1.55);
  from.y-=.42;                                          // ต่ำกว่าตาเล็กน้อย = ปากกระบอก
  if(side){
    const right=new THREE.Vector3().crossVectors(dir,new THREE.Vector3(0,1,0));
    if(right.lengthSq()>1e-6){ right.normalize(); from.addScaledVector(right,side); }
  }
  return {from, dir};
}
function spawnMechaShell(side=0){
  if(mechaShells.filter(s=>s.alive).length>=MECHA_MAX_SHELLS) return null;
  const {from, dir}=mechaMuzzlePoint(side);
  const vel=dir.clone().multiplyScalar(MECHA_MUZZLE);
  const fx=mechaFX&&mechaFX.launch?mechaFX.launch(mechaRobotId,from,vel,performance.now()):0;
  const o={alive:true,x:from.x,y:from.y,z:from.z,vx:vel.x,vy:vel.y,vz:vel.z,life:6,fx};
  mechaShells.push(o); return o;
}
function mechaKillShell(o,hit,now){
  if(!o||!o.alive) return;
  o.alive=false;
  if(hit) MechaAudio.impactBoom();                 // 🤖 รอบ 1485: กระทบเป้า = เสียงระเบิด (คลิป fire เดิม)
  if(mechaFX){
    if(o.fx && mechaFX.impact) mechaFX.impact(o.fx,!!hit,now||performance.now());
    else if(o.fx && mechaFX.kill) mechaFX.kill(o.fx);
  }
}
function mechaApplyLetterHit(a,l){
  if(!a||!l||l.done) return false;
  mShotsHit++;
  if(l.idx===a.nextIdx){
    l.done=true; l.spr.visible=false; a.nextIdx++;
    mCombo++; if(mCombo>mComboMax) mComboMax=mCombo;
    const mult=mCombo>=COMBO_X3?3:(mCombo>=COMBO_X2?2:1), gain=MECHA_LETTER_COIN*mult;
    mechaBankCoins(gain);
    if(mult>1) mechaComboPop(mult);
    sfx.coin(); speakLetter(l.ch); renderHudTop();
    if(mFocusAlien===a) mechaHudWord(a);
    if(a.nextIdx>=a.word.en.length) explodeAlien(a);
  }else{
    mCombo=0;
    if(l.spr.material){ l.spr.material.color.setHex(0xff3030);
      setTimeout(()=>{ if(l.spr&&l.spr.material) l.spr.material.color.setHex(0xffffff); },220); }
    sfx.wrong();
  }
  return true;
}
/* ==== 💨 รอบ 1485: ควันหางมิสไซล์หุ่นรบ (แบบยานแม่) ==== */
function mechaSmokeTex(){
  if(_mechaSmokeTex) return _mechaSmokeTex;
  ...
  _mechaSmokeTex=new THREE.CanvasTexture(c); return _mechaSmokeTex;
}
function spawnMechaSmoke(x,y,z){
  if(!scene || mechaSmoke.length>120) return;
  const mat=new THREE.SpriteMaterial({map:mechaSmokeTex(),transparent:true,opacity:.58,
    depthWrite:false,color:0xe8e2d4});
  const s=new THREE.Sprite(mat); s.position.set(x,y,z);
  const g0=1.05+Math.random()*.35; s.scale.setScalar(g0); scene.add(s);
  mechaSmoke.push({s,born:performance.now(),life:1000+Math.random()*280,g0,op:.58,
    vx:(Math.random()-.5)*.35, vy:.55+Math.random()*.45, vz:(Math.random()-.5)*.35, grow:2.2});
}
function tickMechaSmoke(dt,now){
  for(let i=mechaSmoke.length-1;i>=0;i--){
    const p=mechaSmoke[i], age=(now-p.born)/p.life;
    if(age>=1){ scene.remove(p.s); p.s.material.dispose(); mechaSmoke.splice(i,1); continue; }
    p.s.material.opacity=p.op*(1-age);
    p.s.scale.setScalar(p.g0*(1+age*p.grow));
    p.s.position.x+=p.vx*dt; p.s.position.y+=p.vy*dt; p.s.position.z+=p.vz*dt;
  }
}
function clearMechaSmoke(){
  mechaSmoke.forEach(p=>{ if(p.s){ scene&&scene.remove(p.s); p.s.material&&p.s.material.dispose(); } });
  mechaSmoke=[];
}
function tickMechaShells(dt,now){
  for(let i=mechaShells.length-1;i>=0;i--){
    const o=mechaShells[i]; if(!o.alive){ mechaShells.splice(i,1); continue; }
    o.vy-=MECHA_G*MECHA_SHELL_MASS*dt;                 // แรงโน้มถ่วงชุดเดียวกับกองเรือ
    o.x+=o.vx*dt; o.y+=o.vy*dt; o.z+=o.vz*dt; o.life-=dt;
    if(mechaFX&&o.fx&&mechaFX.sync) mechaFX.sync(o.fx,o.x,o.y,o.z,o.vx,o.vy,o.vz);
    /* 💨 ควันต่อท้ายจรวด ~22 ครั้ง/วิ — วางหลังทิศความเร็วเหมือนยานแม่ */
    if(!o.smokeAt || now>=o.smokeAt){
      o.smokeAt=now+45;
      const sp=Math.hypot(o.vx,o.vy,o.vz)||1;
      spawnMechaSmoke(o.x-o.vx/sp*1.05, o.y-o.vy/sp*1.05, o.z-o.vz/sp*1.05);
    }
    let hitLetter=false;
    for(let ai=0;ai<aliens.length&&!hitLetter;ai++){
      const a=aliens[ai], gs=a.gs||1;
      for(let li=0;li<a.letters.length;li++){
        const l=a.letters[li]; if(l.done) continue;
        const wx=a.grp.position.x+l.off.x*gs, wy=a.grp.position.y+l.off.y*gs, wz=a.grp.position.z+l.off.z*gs;
        if(Math.hypot(o.x-wx,o.y-wy,o.z-wz)<=MECHA_SHELL_HIT*gs){
          mechaKillShell(o,true,now); mechaApplyLetterHit(a,l); hitLetter=true; break;
        }
      }
    }
    if(hitLetter){ mechaShells.splice(i,1); continue; }
    if(o.life<=0 || o.y<-2 || Math.abs(o.x)>HALF+40 || Math.abs(o.z)>HALF+40){
      mechaKillShell(o,false,now); mechaShells.splice(i,1); continue;
    }
    if(o.y<=0 && o.vy<=0){                             // ตกพื้น = ระเบิดเล็ก (เหมือนน้ำทะเลกองเรือ)
      mechaKillShell(o,false,now); mechaShells.splice(i,1);
    }
  }
}
function mechaFire(now){
  if(mOverheat){ if(now-mLastFire>140){ mLastFire=now; sfx.wrong(); } return; }
  mLastFire=now;
  mShotsFired++;
  mHeat=Math.min(100,mHeat+13);
  if(mHeat>=100){ mOverheat=true; MechaAudio.warn(); }
  MechaAudio.fire(mechaWeapon.color);
  camera.updateMatrixWorld();
  // หุ่นคู่/กระจาย = ยิงหลายนัดพร้อมกัน · ทุกนัดวิถีโค้งจากปากกระบอก
  const sides = mechaWeapon.twin ? [-.32,.32]
    : (mechaWeapon.spread ? [-.4,0,.4] : [0]);
  let n=0; for(let s=0;s<sides.length;s++) if(spawnMechaShell(sides[s])) n++;
  if(!n && mechaFX) mechaTracer(0,0,0,false);         // fallback ถ้าเต็มพูล
}
function explodeAlien(a){
  MechaAudio.boom();
  /* 🩸 รอบ 1511: ระเบิดเลือดแดง */
  const BLOOD=0xc41e1e, BLOOD2=0x8b0000, BLOOD3=0xff4444;
  for(let i=0;i<26;i++){
    const col=i%3===0?BLOOD3:(i%2?BLOOD:BLOOD2);
    const pc=new THREE.Mesh(new THREE.SphereGeometry(.28+.2*Math.random(),6,5),new THREE.MeshBasicMaterial({color:col,transparent:true}));
    pc.position.copy(a.grp.position); pc.position.y+=(a.baseY||1.55)+.8; scene.add(pc);
    const dir=new THREE.Vector3(Math.random()*2-1,Math.random()*1.6+.2,Math.random()*2-1).normalize();
    mechaTracers.push({line:pc,until:performance.now()+720,vel:dir.multiplyScalar(6+Math.random()*10),particle:true});
  }
  const reward=M.reward+(a.boss?BOSS_BONUS:0);
  mechaBankCoins(reward); sessionWords++;
  if(a.boss){ mBossKills++; state.mechaBoss=(state.mechaBoss||0)+1;
  ...
  if(typeof vbRecord==='function') vbRecord(a.word.en,a.word.th,true);
  ...
  mWaveKilled++;
  if(mWaveKilled>=mWaveGoal) waveComplete();
  else waveSpawnFill();
  updateWaveHud();
  renderHudTop(); renderBoard();
  if(netUp()) sendPos(true);
}
function tickMecha(dt,now){
  ...
  // 🧟 รอบ 1511: ซอมบี้เดิน + ตอนโจมตีหันหน้าหาผู้เล่นเสมอ
  aliens.forEach(a=>{
    const g=a.grp.position, baseY=a.baseY||1.55;
    const dc=Math.hypot(g.x-camera.position.x,g.z-camera.position.z);
    const engaging=running && dc<75;
    if(engaging){
      a.tgt={x:camera.position.x,z:camera.position.z};
      a.grp.rotation.y=Math.atan2(g.x-camera.position.x, g.z-camera.position.z); // หน้า -Z หันหาผู้เล่น
    }else if(now>a.wanderAt){
      a.tgt={x:(Math.random()*2-1)*(HALF-12),z:(Math.random()*2-1)*(HALF-12)}; a.wanderAt=now+2600+Math.random()*3200;
    }
    const dx=a.tgt.x-g.x, dz=a.tgt.z-g.z, d=Math.hypot(dx,dz)||1;
    g.x+=dx/d*ALIEN_SPEED*(engaging?1.15:1)*dt; g.z+=dz/d*ALIEN_SPEED*(engaging?1.15:1)*dt;
    g.y=baseY+Math.sin(now/380+g.x)*.1;
    if(!engaging) a.grp.rotation.y=Math.atan2(-dx, -dz);
    if(running){
      if(dc<MECHA_ATK_RANGE && now>mHitAt && now>(a.atkAt||0)){ a.atkAt=now+2200; mHitAt=now+900; a.grp.rotation.y=Math.atan2(g.x-camera.position.x, g.z-camera.position.z); mechaHitByAlien(a); }
      else if(dc>=MECHA_ATK_RANGE && dc<75 && now>(a.shotAt||0)){ a.shotAt=now+ALIEN_SHOT_GAP+Math.random()*1600; a.grp.rotation.y=Math.atan2(g.x-camera.position.x, g.z-camera.position.z); spawnAlienShot(a); } }
  ...
  tickMechaShells(dt,now);                           // 🤖 รอบ 1482: กระสุนวิถีโค้ง + ชนตัวอักษร
  tickMechaSmoke(dt,now);                            // 💨 รอบ 1485: ควันหางมิสไซล์
...
function start(md,opt){
  ...
  if(M.heli){
    ...
    syncMechaMoveBtns(); syncMechaFireBtns();
    ...
    MechaAudio.prepareFire();                      // 🤖 รอบ 1502: พรีโหลดคลิปหุ่นจาก allroboteffect + fire.mp3
    MechaAudio.startBgm();                         // 🎵 รอบ 1517: BGM โหลด lazy + Cache hash (ไม่พรีโหลดตอนเปิดเกม)
    clearMechaSmoke();
  ...
  words=M.hotel?[]:pickWords(orderedLetterMode()?1:GUIDE_WORDS);
  orderedWordProgress=0; orderedWrongAt=0; heliUsedRoofs.clear(); droneUsedRooms.clear();
  ...
  /* 🏨 รอบ 778 (ผู้ใช้สั่งข้อ 5): โรงแรมผีสิงวางเฉพาะตัวอักษรของคำที่กำลังหาอยู่ — ไม่มีตัวหลอก ไม่มีของคำอื่น */
    ...
    if(!orderedLetterMode()) for(let i=0;i<8;i++) spawnLetter('abcdefghijklmnopqrstuvwxyz'[Math.floor(Math.random()*26)]);
  ...
  if(mode==='soccer'){
    soccerMusicSession=false; soccerMusicEnabled=true; soccerBgmBlocked=false;
    soccerMusicStop(0,true); soccerMusicSyncButton();
...
function exitWorld(){
  ...
  running=false;
  if(exitSoccer){
    soccerMusicSession=false;
    soccerMusicStop(SOCCER_BGM_EXIT_FADE_MS,true,()=>setTimeout(()=>{
      if(!soccerMusicSession&&(!overlayEl||!overlayEl.classList.contains('on'))&&typeof Music!=='undefined') Music.resumeBg();
    },0));
  }
  ...
  ATC.reset();
  if(typeof MechaAudio!=='undefined'){
    if(MechaAudio.stopBgm) MechaAudio.stopBgm(900);
    if(MechaAudio.stopFireClip) MechaAudio.stopFireClip();
  }
  toggleChatBox(false);
  selfMsgEl.classList.remove('on');
  myChat=null;
  overlayEl.classList.remove('on','adv-hunted','adv-shake','adv-neck-breath','adv-jumpscare');
  if(introEl) introEl.classList.remove('on');
  scareEl.classList.remove('on','turn-scare');
  banEl.classList.remove('show','stay'); banEl.innerHTML='';
  if(!exitSoccer&&typeof Music!=='undefined') Music.resumeBg();   // 🎵 ฟุตบอลคืน Lobby หลัง fade 1.1 วิใน callback ด้านบน
...
function mechaRecapLine(){
  ...
  return `🌊 ถึงเวฟ ${mWave||1} · ⭐ คอมโบสูงสุด ×${mComboMax} · 👾 ล้มบอส ${mBossKills} · 🎯 แม่นยำ ${acc}% · 📖 ${sessionWords} คำ · +${fmtNum(sessionCoins)} 🪙 · รวม ${fmtNum(state.coins||0)}`;
}
...
window.Adventure3D={
  ...
  _t:{
    ...
    hotel:{ get h(){return hotel}, get footY(){return hFootY}, get dark(){return blackedOut},
            get torch(){return torchOn}, get quest(){ return hQuest&&{
