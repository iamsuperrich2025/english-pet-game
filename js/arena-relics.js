...
(function(){
  ...
  function render(root,api){
    ...
    if(!v){v={category:'all',query:'',page:0,api};views.set(panel,v);const tools=document.createElement('div');tools.className='va-grimoire-tools';tools.innerHTML='<label><select aria-label="หมวดพลังอักษร">'+categories.map(([id,n])=>'<option value="'+id+'">'+n+'</option>').join('')+'</select></label><label class="va-grimoire-search"><input type="search" aria-label="ค้นหาพลังอักษร" placeholder="ค้นหาชื่อหรือประโยชน์…"></label><b>50 RELICS</b>';panel.querySelector('.va-panel-head').after(tools);
      tools.querySelector('select').addEventListener('change',e=>{v.category=e.target.value;v.page=0;render(root,v.api);});tools.querySelector('input').addEventListener('input',e=>{v.query=e.target.value;v.page=0;render(root,v.api);});
      const nav=document.createElement('div');nav.className='va-grimoire-nav';nav.innerHTML='<button data-relic-page="-1" aria-label="พลังอักษรหน้าก่อน">←</button><span></span><button data-relic-page="1" aria-label="พลังอักษรหน้าถัดไป">→</button><small>ซื้อครั้งเดียว · ใช้ถาวร · โบนัสมีขีดจำกัด</small>';panel.append(nav);nav.addEventListener('click',e=>{const b=e.target.closest('[data-relic-page]');if(b){v.page+=Number(b.dataset.relicPage);render(root,v.api);}});
    }
    ...
    const size=innerWidth<=500?10:innerWidth<=700?8:innerHeight>500?15:10,pages=Math.max(1,Math.ceil(list.length/size));v.page=Math.max(0,Math.min(pages-1,v.page));
    root.querySelector('#va-store-grid').innerHTML=list.slice(v.page*size,(v.page+1)*size).map(i=>'<button class="va-store-item'+(api.own(i.id)?' owned':'')+'" data-buy="'+i.id+'" aria-label="'+i.name+' '+i.desc+' '+(api.own(i.id)?'มีแล้ว':i.price+' เหรียญ')+'" '+(api.own(i.id)?'disabled':'')+'><span class="va-store-ico">'+icon(i)+'</span><div class="va-store-name">'+i.name+'</div><div class="va-store-desc">'+i.desc+'</div><div class="va-store-price">'+(api.own(i.id)?(typeof isAdmin==='function'&&isAdmin()?'✦ ADMIN · ใช้ได้':'✓ มีแล้ว'):'◈ '+api.fmt(i.price))+'</div></button>').join('')||'<p class="va-spell-empty">ไม่พบพลังอักษรที่ค้นหา</p>';
    const nav=panel.querySelector('.va-grimoire-nav');nav.querySelector('span').textContent=list.length?(v.page*size+1)+'–'+Math.min(list.length,(v.page+1)*size)+' / '+list.length+' ชิ้น':'0 / 50 ชิ้น';nav.querySelector('[data-relic-page="-1"]').disabled=v.page===0;nav.querySelector('[data-relic-page="1"]').disabled=v.page===pages-1;
  }
