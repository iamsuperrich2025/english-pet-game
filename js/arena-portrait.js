 ...
 function create(host){
  ...
  try{
   ...
   const fs=shader(gl.FRAGMENT_SHADER,`precision mediump float;varying vec2 uv;uniform sampler2D pic;uniform float t;uniform vec4 hair,capeA,capeB,face,magic;uniform float flame;
    ...
    void main(){float fixedFace=smoothstep(.85,1.2,length((uv-face.xy)/face.zw));float h=mask(hair)*fixedFace;float c=max(mask(capeA),mask(capeB))*fixedFace;float m=mask(magic)*fixedFace;vec2 d=vec2(h*.009*sin(t*1.65+uv.y*17.)+c*.015*sin(t*1.35+uv.y*14.),h*.0025*sin(t*1.35+uv.x*11.)+c*.005*sin(t*1.7+uv.x*12.));d+=m*vec2(.009*sin(t*3.2+uv.y*45.),.007*sin(t*2.6+uv.x*21.));vec4 col=texture2D(pic,clamp(uv+d,0.,1.));col.rgb*=1.+m*(.08+flame*.12)*sin(t*7.+uv.y*28.);gl_FragColor=col;}`);
  ...
  function particles(t){ctx.clearRect(0,0,sparks.width,sparks.height);const [mx,my,rx,ry]=hero.motion.magic,w=sparks.width,h=sparks.height;ctx.fillStyle=hero.color;ctx.strokeStyle=hero.color;
   for(let i=0;i<24;i++){const q=(t*.34+i/24)%1,a=i*2.4+t*1.05,orbit=hero.id==='gravity'||hero.id==='water'||hero.id==='wind';const x=(mx+(orbit?Math.cos(a)*rx*.9:Math.sin(i*12.4+q*2)*rx*.88))*w,y=(my+(orbit?Math.sin(a)*ry*.46:-q*ry*1.9+ry*.72))*h;ctx.globalAlpha=Math.sin(q*Math.PI)*.88;ctx.beginPath();if(hero.id==='ice'||hero.id==='earth'){const r=2.8+q*3;ctx.moveTo(x,y-r);ctx.lineTo(x+r*.6,y);ctx.lineTo(x,y+r);ctx.lineTo(x-r*.6,y);ctx.closePath();}else ctx.arc(x,y,(hero.id==='fire'?3.2:2.1)+q*2.2,0,Math.PI*2);ctx.fill();}ctx.globalAlpha=1;
  }
