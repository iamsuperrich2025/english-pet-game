...
/* 🎯 กฎที่ใช้ตัดสินท่าปืน (ผู้ใช้: "ของเค้าวางปืนเป็นมุมที่เล็งจากจุดเล็งได้ ของเราจุดเล็งโคตรห่าง"):
...
/* 🖼️ รอบ 453 (ผู้ใช้ส่งภาพหน้าจอ Tripo: "อยากเห็นปืนมุมนี้ในเกม" = เห็นทั้งกระบอก มีกล้องส่อง
   มองจากด้านหลัง-เยื้องบน) — ของรอบ 451 ดึงเข้ามาใกล้เกินจนโครงปืน/กล้อง/พานท้ายหลุดขอบขวา
   เหลือให้เห็นแค่ "ก้อนดำ" มุมล่างขวา · เทียบภาพจริง 8 ท่าในเกมแล้วเลือกท่านี้:
   ถอยออก + ย้ายไปขวา-ล่างอีกนิด + ย่อลง = เห็นลำกล้อง–กล้องส่อง–โครงปืน–พานท้ายครบทั้งแนว
   ⚠️ ยิ่งดึงปืนเข้าใกล้ตา ยิ่งเห็นปืนน้อยลง (ส่วนที่ใกล้จะบานหลุดขอบจอ) — ใกล้ ≠ เห็นชัด */
/* 📐 รอบ 454 (ผู้ใช้สั่งเป็นตัวเลขจากภาพจริง): "ปืนเล็กเลยดูไกลตัว → ขยายใหญ่ขึ้น
   และกดปากกระบอกลงจนแนวปืนบนจอเป็นมุม 15° (ของเดิม ~70°)"
   วัดมุมเงาปืนบนจอด้วย PCA ของจุดที่อยู่ในเฟรม → ของเดิม 65° · ค่าใหม่ = 15° เป๊ะ · ปืนใหญ่ขึ้น 1.5 เท่า
   ⚠️ แลกมาด้วย: ท่าถือสะโพก **ลำกล้องไม่ได้ชี้เข้ากากบาทแล้ว** (กดลง 30°) — กฎรอบ 452 ใช้ไม่ได้กับท่านี้
   แต่ **กระสุนยังไปที่กากบาทเสมอ** (ยิงตามแนวเล็งของกล้อง ไม่ได้ยิงตามทิศโมเดลปืน)
   และตอนส่องกล้อง (ADS) ปืนยกกลับเข้าแนวสายตาตาม ADS_ROT เหมือนเดิม */
/* 📐 รอบ 455 (ผู้ใช้สั่งต่อ): "ใหญ่กว่านี้ + ยกพานท้ายขึ้นอีก ~5°"
   → ขยาย .85 → 1.15 (ใหญ่ขึ้นอีก 35%) · เงาปืนบนจอจาก 15° → **10°** (ยกท้ายขึ้น 5° ตามสั่ง)
   วิธีหาค่า: bisection หา GUN_ROT[0] ที่ทำให้มุมแกนหลักของเงาปืน (PCA) = เป้าหมายพอดี */
/* 📐 รอบ 456 (ผู้ใช้: "ขยับปืนไปข้างหน้าให้เห็น scope · ยกพานท้ายขึ้นอีก 5°"):
   ดันปืนออกไปข้างหน้า z −.66 → −.95 (กล้องส่องกลับเข้ามาอยู่ในเฟรม จากเดิมตกขอบล่าง)
   + ขยาย 1.15 → 1.35 ชดเชยระยะที่ไกลขึ้น (ขนาดบนจอเท่าเดิมราว 96%) · เงาปืน 10° → **5°** */
const GUN_POS=[.22,-.17,-.95];      // ตำแหน่งปืนเทียบกล้อง (x ขวา · y ล่าง · z ระยะหน้ากล้อง)
/* 🎯 รอบ 452 (ผู้ใช้: "ลากเส้นตรงออกจากปลายกระบอก ปลายเส้นต้องเป็นจุดกลางกากบาท"):
   = ลำกล้องต้อง **เล็งไปที่จุดเล็งจริง** ไม่ใช่แค่ "เอียงเข้าใน" สวย ๆ
   คำนวณตรง ๆ: ปากกระบอกอยู่ที่ M (พิกัดกล้อง) · เป้าอยู่บนแกนสายตาที่ระยะ ZERO_DIST
   ทิศลำกล้องที่ถูกต้อง d = normalize(เป้า − M) → แปลงกลับเป็นมุม Euler ของกลุ่มปืน
   ได้ค่า **[ก้ม-เงย .002 · หันเข้าใน .003]** = แทบขนานแกนสายตา (ของเดิมหันเข้าใน .17 = 9.7°
   ซึ่งชี้ออกนอกทางกระสุนไปไกลมาก) · การ "เอียงทแยงให้เห็นตัวปืน" ย้ายไปพึ่ง roll + ตำแหน่ง x แทน
   ⚠️ กฎถาวร: ห้ามเพิ่ม pitch/yaw ให้ปืนอีกเพื่อความสวย — มันคือการทำให้ปืนเล็งผิดทางทันที */
const ZERO_DIST=50;                 // ระยะที่ลำกล้อง "ตัดเข้าแนวเล็ง" (ม.) — เหมือนการ zero ปืนจริง
const GUN_ROT=[-.645,.004,.09];     // มุมปืน: [ก้ม-เงย, หันเข้าใน, เอียงทแยง] — รอบ 456: เงาปืนบนจอ 5° (ยกท้ายจาก 10°)
const GUN_SCALE=1.35;               // ขนาดปืนในจอ (รอบ 456: 1.15 → 1.35 ชดเชยที่ดันปืนไปข้างหน้า)
