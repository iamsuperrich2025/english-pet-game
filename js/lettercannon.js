...
(function(){
  ...
  function playerLimits(){const pad=playerSize()*.42;return{minX:pad,maxX:W-pad,minY:Math.max(H*.34,pad+8),maxY:H-pad*.55};}
  ...
  function resetMission(){
    ...
    letters=[];bullets=[];particles=[];shockwaves=[];particleCursor=0;rewardedLetters.clear();rewardedWords.clear();activePower=null;powerLeft=powerTotal=0;missiles=MAX_MISSILES;shotSeq=0;currentAmmo=AMMO[0];barrelCycle=flashSide=0;paused=false;fireAt=0;nextWord();renderHud();
  }
  ...
  function completeWord(){
    ...
    toast('🌟 '+word.en+' · '+word.th+'  ครบคำ! +'+WORD_BONUS+' เหรียญ','#ffe85c');celebrate();
    later(()=>{if(running&&!missionEnded)nextWord();},820);
  }
  ...
  function update(dt){
    ...
    const lim=playerLimits(),kbdX=(keyRight?1:0)-(keyLeft?1:0),kbdY=(keyDown?1:0)-(keyUp?1:0),mag=Math.hypot(kbdX,kbdY)||1,maxSpeed=Math.min(MOVE_SPEED_CAP,Math.max(W,H)*MOVE_SPEED_FACTOR);
    if(dragPointer!==null){const k=1-Math.exp(-dt*MOVE_FOLLOW),nx=playerX+(dragX-playerX)*k,ny=playerY+(dragY-playerY)*k;playerVX=(nx-playerX)/Math.max(dt,.0004);playerVY=(ny-playerY)/Math.max(dt,.0004);playerX=nx;playerY=ny;}
    else{const targetVX=kbdX/mag*maxSpeed,targetVY=kbdY/mag*maxSpeed,moving=kbdX||kbdY,response=moving?MOVE_ACCEL:MOVE_BRAKE;playerVX+=(targetVX-playerVX)*Math.min(1,dt*response);playerVY+=(targetVY-playerVY)*Math.min(1,dt*response);if(!moving&&Math.hypot(playerVX,playerVY)<4)playerVX=playerVY=0;playerX+=playerVX*dt;playerY+=playerVY*dt;}
    if(playerX<lim.minX){playerX=lim.minX;}else if(playerX>lim.maxX){playerX=lim.maxX;}
    if(playerY<lim.minY){playerY=lim.minY;}else if(playerY>lim.maxY){playerY=lim.maxY;}
  ...
  function frame(now){if(!running)return;raf=requestAnimationFrame(frame);if(!frameAt){frameAt=now;last=now;}if(paused||counting){if(now-frameAt<100)return;frameAt=now;last=now;draw(now/1000);return;}const dt=Math.min(.05,Math.max(0,(now-last)/1000||.016));last=now;frameAt=now;update(dt);draw(now/1000);if(((now/250)|0)!==(((now-dt*1000)/250)|0))renderHud();}
  function bind(){
  ...
  async function requestPortrait(){
    if(document.fullscreenElement&&document.exitFullscreen){try{await document.exitFullscreen();}catch(_e){}}
    gameFullscreen=false;
    await lockPortrait();
  }
  function releasePortrait(){try{if(orientationLocked&&screen.orientation&&screen.orientation.unlock)screen.orientation.unlock();}catch(_e){}orientationLocked=false;if(document.fullscreenElement&&document.exitFullscreen)document.exitFullscreen().catch(()=>{});gameFullscreen=false;}
