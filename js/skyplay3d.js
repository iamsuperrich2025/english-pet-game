...
(function(){
  ...
  let classOffer=null,classWire='',classFinished=false,classView='setup',classRoster={},classHostUid='',classReportSaved=false,teacherData=null,characterResumePaused=false;
  ...
  const playerCharacter=id=>PLAYER_CHARACTERS.find(c=>c.id===String(id||''))||PLAYER_CHARACTERS.find(c=>c.id===DEFAULT_CHARACTER_ID);
  const selectedCharacter=()=>playerCharacter(state.skyCharacter).id;
  ...
  function ensureState(){
    ...
    state.skyCharacter=selectedCharacter();
  ...
  function createDom(){
    ...
    root=document.createElement('div');root.id='sp-root';root.innerHTML=`
      ...
      <div class="sp-top">
        ...
        <button class="sp-pill sp-character-btn" id="sp-character">🧑 ${esc(playerCharacter(state.skyCharacter).name)}</button>
      ...
      <div class="sp-character-picker" id="sp-character-picker" aria-hidden="true"><div class="sp-character-card">
        <button class="sp-close" id="sp-character-close" aria-label="Close">×</button><small>🧸 SOFT CUBOID CHIBI 3D</small>
        <h2>เลือกตัวละคร Vocab Sky</h2><div class="sp-character-grid" id="sp-character-grid"></div>
        <p>ตัวละครที่เลือกจะแสดงเหมือนกันทั้งเครื่องนี้และเพื่อนออนไลน์</p>
      </div></div>
  ...
  function chooseCharacter(id){
    const character=playerCharacter(id);state.skyCharacter=character.id;if(player&&player.fig){setPlayerSpriteCharacter(player.fig,character.id);updatePlayerSprite(player.fig,player.yaw,performance.now(),false,false);}const button=root&&root.querySelector('#sp-character');if(button)button.textContent=`🧑 ${character.name}`;saveState();if(typeof authPushSave==='function')authPushSave(false);netSend(true);showToast(`เลือก ${character.name} แล้ว`);closeCharacterPicker();return character.id;
  }
  ...
  function buildVocabularyTower(){
    ...
    const core=meshSoft(2.1,20,2.1,0xf3e8ff,.5);core.position.set(cx,10.5,cz);scene.add(core);cameraMeshes.push(core);addSolidObject('tower-core',core);
    ...
    const crown=new THREE.Mesh(new THREE.ConeGeometry(6.1,4.2,6),mat(0xffd968,'standard'));crown.position.set(cx,21.2,cz);crown.rotation.y=Math.PI/6;scene.add(crown);addSolidObject('tower-crown',crown);
  ...
  function buildWorld(){
    ...
    [[-52,20,-35,10],[49,15,-41,8],[-45,30,35,7],[48,36,29,9]].forEach(([x,y,z,r],i)=>{const m=new THREE.Mesh(new THREE.CylinderGeometry(r,r+2,3,24),mat([0x8bdc78,0xffc5dc,0x80d8ff,0xb9a1ff][i]));m.position.set(x,y,z);scene.add(m);addSolidCircle('floating-island-'+i,x,z,r,y-1.5,y+1.5,{support:true});});
  ...
  function buildPlayer(){
    const g=new THREE.Group(),fig=makePlayerSprite(selectedCharacter()),sh=new THREE.Mesh(new THREE.CircleGeometry(.72,20),new THREE.MeshBasicMaterial({color:0x17326a,transparent:true,opacity:.18,depthWrite:false}));g.add(fig);sh.rotation.x=-Math.PI/2;sh.position.y=.02;sh.name='player-contact-shadow';g.add(sh);scene.add(g);
  ...
  function bindInput(){
    ...
    addListener(root.querySelector('#sp-character'),'click',openCharacterPicker);addListener(root.querySelector('#sp-character-close'),'click',closeCharacterPicker);addListener(ui.characterGrid,'click',e=>{const button=e.target.closest('[data-character]');if(button)chooseCharacter(button.dataset.character);});
    ...
    addListener(window,'keyup',e=>keys.delete(e.code));addListener(window,'resize',resize);addListener(document,'visibilitychange',()=>{if(document.hidden){keys.clear();joy.id=null;joy.x=joy.z=0;if(ui.joyKnob)ui.joyKnob.style.transform='translate(-50%,-50%)';}});
    const j=ui.joy,placeJoy=e=>{const rr=root.getBoundingClientRect(),jr=j.getBoundingClientRect(),half=jr.width*.5+4,x=clamp(e.clientX-rr.left,half,rr.width*.5-half),y=clamp(e.clientY-rr.top,half,rr.height-half);j.style.left=`${x}px`;j.style.top=`${y}px`;j.style.bottom='auto';j.style.transform='translate(-50%,-50%)';},moveJoy=e=>{if(joy.id!==e.pointerId)return;e.preventDefault();const r=j.getBoundingClientRect(),dx=e.clientX-(r.left+r.width/2),dy=e.clientY-(r.top+r.height/2),m=Math.max(1,Math.hypot(dx,dy)),lim=r.width*.34,k=Math.min(lim,m)/m;joy.x=clamp(dx/lim,-1,1);joy.z=clamp(dy/lim,-1,1);ui.joyKnob.style.transform=`translate(calc(-50% + ${dx*k}px),calc(-50% + ${dy*k}px))`;};
    const joyZone=e=>{const rr=root.getBoundingClientRect();return e.pointerType!=='mouse'&&e.clientX<=rr.left+rr.width*.5&&!(e.target.closest&&e.target.closest('.sp-top,.sp-daily,.sp-activity.on,.sp-character-picker.on,.sp-gate.on,.sp-tower.on,.sp-classroom.on,button,input,select,textarea,label,a'));},beginJoy=(e,force=false)=>{if(joy.id!==null||paused||(!force&&!joyZone(e)))return false;e.preventDefault();placeJoy(e);joy.id=e.pointerId;if(look.id===e.pointerId)look.id=null;const owner=e.currentTarget||root;owner.setPointerCapture&&owner.setPointerCapture(e.pointerId);moveJoy(e);return true;};
    const endJoy=e=>{if(joy.id!==e.pointerId)return;joy.id=null;joy.x=joy.z=0;ui.joyKnob.style.transform='translate(-50%,-50%)';};
    addListener(root,'pointerdown',e=>{if(!joyZone(e))return;if(joy.id===null)beginJoy(e);e.stopPropagation();},true);addListener(j,'pointerdown',e=>{if(e.pointerType==='mouse'&&beginJoy(e,true))e.stopPropagation();});addListener(window,'pointermove',moveJoy,{passive:false});addListener(window,'pointerup',endJoy);addListener(window,'pointercancel',endJoy);
  ...
  function netSend(force){if(!room||!player)return;const elapsed=Math.min(99999,Math.round(activityElapsed()/100));let hp=`S3:${activityCode()}:${activityRound?activityRound.score:0}:${activity==='obby'?currentCheckpoint:(activityRound?activityRound.progress:0)}:${elapsed}:${activityRound?activityRound.seed:'plaza'}`,cw=classWire||'';if(activity==='classroom'&&activityRound){if(activityRound.version==='C5'){if(activityRound.role==='host')hp=`S5:H:${phaseNumber(activityRound.phase)}:${activityRound.step||0}:${elapsed}:${activityRound.eventId}`;else{const mode=activityRound.phase==='ready'?'R':activityRound.phase==='finished'?'F':'C',score=mode==='R'?(activityRound.ready?1:0):(activityRound.score||0);hp=`S5:${mode}:${score}:${activityRound.progress||0}:${elapsed}:${activityRound.eventId}`;cw=studentReportWire();}}else hp=`S4:${activityRound.role==='host'?'H':'C'}:${activityRound.score||0}:${activityRound.progress||0}:${elapsed}:${activityRound.eventId}`;}room.send({n:String(state.profileName||'นักสำรวจ').slice(0,40),x:+player.pos.x.toFixed(2),y:+player.pos.y.toFixed(2),z:+player.pos.z.toFixed(2),yaw:+player.group.rotation.y.toFixed(3),av:profileAvatar(),sc:selectedCharacter(),m:performance.now()<emoteUntil?1:0,w:sessionStars,cw,hp},!!force);}
  function onPeer(uid,d){peers[uid]=d||{};if(!peerActors[uid])buildPeer(uid,d||{});const a=peerActors[uid];if(a){setPlayerSpriteCharacter(a.fig,d.sc);a.target.set(Number(d.x)||0,Number(d.y)||0,Number(d.z)||0);a.yaw=Number(d.yaw)||0;a.data=d||{};}considerJoinOffer(uid,d||{});considerClassOffer(uid,d||{});rememberClassPeer(uid,d||{});updateOnline();updateActivityHud(performance.now(),true);if(activity==='classroom')renderClassroom(true);}
