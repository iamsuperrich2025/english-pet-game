"use strict";
/* ============================================================
   ☁️📚 รอบ 1229 — VOCAB SKY PLAYGROUND · PHASE 1
   ...
   - plaza, obby, checkpoints, vocabulary gate/stars, main coin reward
   ============================================================ */
(function(){
  ...
  function createDom(){
    ...
    Object.assign(ui,{online:root.querySelector('#sp-online'),challenge:root.querySelector('#sp-challenge'),progress:root.querySelector('#sp-progress'),coins:root.querySelector('#sp-coins'),hint:root.querySelector('#sp-hint'),word:root.querySelector('#sp-word'),pop:root.querySelector('#sp-pop'),toast:root.querySelector('#sp-toast'),joy:root.querySelector('#sp-joy'),joyKnob:root.querySelector('#sp-joy i'),gate:root.querySelector('#sp-gate'),gateQ:root.querySelector('#sp-gate-q'),gateOptions:root.querySelector('#sp-gate-options'),gateFeedback:root.querySelector('#sp-gate-feedback')});
  }
  ...
  function buildWorld(){
    supports=[];cameraMeshes=[];moving=[];rotators=[];checkpoints=[];stars=[];effects=[];
  ...
  function wordPool(){let pool=(typeof vocabForStudent==='function'?vocabForStudent():[]).filter(x=>x&&/^[a-z]{3,10}$/i.test(x[0])&&x[1]);if(pool.length<6)pool=pool.concat([['cat','แมว'],['dog','สุนัข'],['book','หนังสือ'],['star','ดาว'],['apple','แอปเปิล'],['cloud','เมฆ']]);return pool.sort(()=>Math.random()-.5).slice(0,8);}
  ...
  function updateWorld(dt,t){
    ...
    if(portal){portal.rotation.y+=dt*.45;if(!routeFinished&&player.pos.distanceTo(portal.position.clone().add(new THREE.Vector3(0,1,0)))<2.8)finishRoute();}
  ...
  function netSend(force){if(!room||!player)return;room.send({n:String(state.profileName||'นักสำรวจ').slice(0,40),x:+player.pos.x.toFixed(2),y:+player.pos.y.toFixed(2),z:+player.pos.z.toFixed(2),yaw:+player.group.rotation.y.toFixed(3),av:profileAvatar(),m:performance.now()<emoteUntil?1:0,w:sessionStars,hp:`S1:${currentCheckpoint}:${routeFinished?1:0}`},!!force);}
  function onPeer(uid,d){peers[uid]=d||{};if(!peerActors[uid])buildPeer(uid,d||{});const a=peerActors[uid];if(a){a.target.set(Number(d.x)||0,Number(d.y)||0,Number(d.z)||0);a.yaw=Number(d.yaw)||0;a.data=d||{};}updateOnline();}
  function onPeerGone(uid){delete peers[uid];removePeer(uid);updateOnline();}
  ...
  function updateHud(){if(!root)return;ui.coins.textContent=fmt(state.coins||0);ui.progress.textContent=`⭐ ${sessionStars}/5 · 💎 CP ${currentCheckpoint+1}/3`;}
