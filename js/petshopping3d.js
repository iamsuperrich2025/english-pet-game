...
window.PetShopping3D=(()=>{
  ...
  function buildHUD(systemCar){
    ...
    root.insertAdjacentHTML('beforeend',`
      ...
      <div class="ps3-warning"></div><button class="ps3-store">เข้าร้าน</button><div class="ps3-help">W เร่ง · Space เบรก · A/D เลี้ยว · R เกียร์ · Z/C ไฟเลี้ยว · V กล้อง · H แตร · M เพลง${systemCar?' · รถระบบฟรีรอบนี้':''}</div>
  ...
  function start(opt={}){if(running||!window.THREE||!document.body)return false;target=opt.target==='fashion'?'fashion':'food';carId=/^car_\d+$/.test(opt.carId||'')?opt.carId:'car_01';car={x:0,z:18,yaw:Math.PI,speed:0,steer:0};keys={};steerCtl=0;padThr=padBr=gearR=kBack=false;dSpeed=dSteer=dVelX=dVelZ=dRoll=dRollV=dPitch=dPitchV=dHeave=dHeaveV=0;radioBars.fill(0);radioPeaks.fill(0);dCamYaw=car.yaw;carRevBeepAt=collisionAt=0;carEngineOn=carBelted=carOverSpeed=false;carStartOpen=true;turnSet(0);cam3=false;seatLevel=0;root=document.createElement('div');root.className='ps3-root';document.body.appendChild(root);try{buildScene();carMesh=makeDriverCar();buildHUD(!!(opt.systemCar||opt.rental));bind();if(typeof Music!=='undefined'){Music.suspendBg();Music.carRadio(true);radioState();}running=true;last=performance.now();raf=requestAnimationFrame(tick);return true;}catch(e){console.error(e);cleanup();return false;}}
