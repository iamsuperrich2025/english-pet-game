...
window.PetShopping3D=(()=>{
  ...
  let carEngineOn=false,carBelted=false,carStartOpen=true,carBeltFined=false,carOverSpeed=false,carFines=[];
  ...
  function showLaw(warn,done){if(!lawEl){done&&done();return;}lawEl.innerHTML=`<h3>🛡️ SAFETY BRIEFING · กฎหมายจราจร</h3>${warn?'<div class="li-warn">⚠️ ยังไม่คาดเข็มขัด ออกรถได้แต่จะถูกปรับเมื่อรถวิ่ง</div>':''}<div class="li-grid"><div>🔒 <b>เข็มขัดนิรภัย · ม.123</b><br><small>ในเกม: ไม่คาดแล้วขับ = ปรับ 🪙${CAR_FINE_BELT}</small></div><div>🚨 <b>ความเร็ว · ม.67</b><br><small>เกิน ${CAR_LEGAL_KMH} กม./ชม. = ใบสั่งครั้งละ 🪙${CAR_FINE_SPEED}</small></div><div>🔧 <b>ขับชนแรง</b><br><small>ค่าซ่อมครั้งละ 🪙${CAR_REPAIR_FEE}</small></div></div><button type="button">🫡 รับทราบ</button>`;lawEl.style.display='block';lawEl.querySelector('button').onclick=()=>{lawEl.style.display='none';done&&done();};}
