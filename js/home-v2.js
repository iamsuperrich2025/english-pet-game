...
(function(){
  ...
  function navButton(actionName, iconName, label, sourceSelector=''){
    const source = sourceSelector ? ` data-vw2-source="${htmlEscape(sourceSelector)}"` : '';
    return `<button class="vw2-rail-btn" data-vw2-action="${htmlEscape(actionName)}"${source}><span>${icon(iconName)}</span><b>${htmlEscape(label)}</b></button>`;
  }

  function bottomButton(actionName, iconName, label, tone='violet'){
    return `<button class="vw2-mode ${htmlEscape(tone)}" data-vw2-action="${htmlEscape(actionName)}"><span>${icon(iconName)}</span><b>${htmlEscape(label)}</b></button>`;
  }

  function toolButton(actionName, iconName, label, extra=''){
    return `<button class="vw2-tool-btn ${htmlEscape(extra)}" data-vw2-action="${htmlEscape(actionName)}" title="${htmlEscape(label)}"><span>${icon(iconName)}</span><b>${htmlEscape(label)}</b></button>`;
  }
...
`;
  ...
  function action(name){
    switch(name){
      ...
      case 'cure': clickExisting('#btn-rail-cure'); break;
      ...
      case 'examstd': clickExisting('#btn-rail-examstd'); break;
      case 'onet': clickExisting('#btn-rail-onet'); break;
      case 'stats': clickExisting('#btn-stats'); break;
      case 'trophy': clickExisting('#btn-rail-trophy'); break;
  ...
  function build(){
    ...
    const railButtons = [
      ...
      ['wordsearch','search','ค้นหาคำ'],
      ['typing','typing','พิมพ์คำ'],
      ['bubble','bubble','ฟอง'],
      ['shoot','target','ยิงเป้าคำ'],
      ['cannon','cannon','Letter Cannon'],
      ['examstd','exam','ข้อสอบจริง'],
      ['onet','flag','O-NET'],
      ['rank','crown','อันดับ'],
      ['market','market','ตลาด'],
      ...
      ['gifts','gift','ของขวัญ'],
      ['stats','stats','สถิติ'],
      ['trophy','trophy','ตู้เข็ม'],
    ].map(x=>navButton(x[0],x[1],x[2],x[3]||'')).join('');

    const modeButtons = [
      ['vocabbook','bookgold','สมุดคำศัพท์','book'],
      ['ielts','book','IELTS','blue'],
      ['toeic','book','TOEIC','green'],
      ['toefl','book','TOEFL','orange'],
      ['onetp6','star','O-NET ป.6','gold'],
      ['onetm3','leaf','O-NET ม.3','lime'],
      ['onetm6','crown','O-NET ม.6','violet'],
      ['cats','sparkle','หมวดคำศัพท์','pink'],
      ['play','controller','จับคู่คำศัพท์','game'],
      ['picmatch','picture','จับคู่ภาพ','blue'],
      ['picdict','bookgold','Picture Dictionary','green'],
      ['picquiz','headphones','ครูถามศัพท์','orange'],
      ['bandexam','clipboard','สอบเลื่อนขั้น','violet'],
    ].map(x=>bottomButton(x[0],x[1],x[2],x[3])).join('');
      ...
      <div class="vw2-shell">
        <header class="vw2-top">
          ...
          <section class="vw2-wallet" aria-label="ข้อมูลรายได้และทรัพย์สิน">
            <button class="vw2-wallet-pill coin" data-vw2-action="rank" title="เหรียญที่มีอยู่"><small>เหรียญของฉัน</small>${icon('coin')}<b id="vw2-coins">0</b><em>+</em></button>
          ...
          <section class="vw2-top-actions" aria-label="เครื่องมือ — ทุกปุ่มมีข้อความกำกับ">
            ${toolButton('chat','chat','ข้อความ')}
            ${toolButton('music','music','เพลง')}
            ${toolButton('night','moon','กลางคืน')}
            ${toolButton('settings','settings','ตั้งค่า')}
            ${toolButton('logout','logout','ออกระบบ')}
        ...
        <div class="vw2-preview-mark">ADMIN PREVIEW · HOME V2 PET WELCOME + COMPLETE METRICS</div>
      </div>`;
  ...
  function sync(){
    ...
    setText('vw2-grade', grade ? `ระดับชั้น ${cleanText(grade,20)}` : 'ระดับชั้น —');
    ...
    root.querySelectorAll('[data-vw2-source]').forEach(btn=>{
      const source = document.querySelector(btn.dataset.vw2Source);
