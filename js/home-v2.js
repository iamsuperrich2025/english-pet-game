...
(function(){
  ...
  Object.assign(ICON_ART, {
    ...
    ship:`<path d="M8 38l8-10h32l8 10 4 4v8H6v-8z" class="i-blue"/><path d="M18 30h26l4 8H14z" class="i-white"/><rect x="28" y="16" width="8" height="14" class="i-purple"/><path d="M36 12l14 6-14 5z" class="i-peach"/><circle cx="18" cy="50" r="5" class="i-coin"/><circle cx="46" cy="50" r="5" class="i-coin"/><path d="M10 54c12 6 32 6 44 0" class="i-mint"/>`,
  ...
  function syncRuntimeActionParity(){
    ...
    if(kart){const allowed=true;kart.hidden=!allowed;kart.style.display=allowed?'':'none';kart.disabled=!allowed;kart.setAttribute('aria-hidden',String(!allowed));}
  ...
  const VW2_FEED_STEP_MS = 6000;
  const VW2_FEED_RESUME_MS = 10000;
  function bindV2FeedAutoFlow(host){
    if(!host || host.dataset.vw2AutoFlowBound === '1') return;
    host.dataset.vw2AutoFlowBound = '1';
    const hold = ()=>{
      const now = Date.now();
      host.__vw2FeedHoldUntil = now + VW2_FEED_RESUME_MS;
      host.__vw2FeedNextAt = host.__vw2FeedHoldUntil + VW2_FEED_STEP_MS;
    };
    ...
    host.addEventListener('scroll', ()=>{
      if(Date.now() > (host.__vw2FeedProgrammaticUntil || 0)) hold();
    }, {passive:true});
  }
  function advanceV2FeedAutoFlow(host){
    if(!host) return;
    bindV2FeedAutoFlow(host);
    const now = Date.now();
    if(root?.hidden || !dashboardActive() || now < (host.__vw2FeedHoldUntil || 0)) return;
    if(!host.__vw2FeedNextAt){ host.__vw2FeedNextAt = now + VW2_FEED_STEP_MS; return; }
    if(now < host.__vw2FeedNextAt) return;
    host.__vw2FeedNextAt = now + VW2_FEED_STEP_MS;
    const maxTop = Math.max(0,host.scrollHeight-host.clientHeight);
    if(maxTop < 4) return;
    const current = host.scrollTop;
    const stops = Array.from(host.children).map(el=>el.offsetTop).filter((top,index,list)=>top > 0 && (index === 0 || top !== list[index-1]));
    const next = stops.find(top=>top > current + 8);
    const target = next == null || current >= maxTop - 4 ? 0 : Math.min(next,maxTop);
    host.__vw2FeedProgrammaticUntil = now + 1400;
    try{ host.scrollTo({top:target,behavior:'smooth'}); }
    catch(_){ host.scrollTop = target; }
  }
  function feedCardsFromAuthoritativeSource(){
    ...
    try{
      ...
      if(host.dataset.vw2Html !== html){
        host.innerHTML = html;
        host.dataset.vw2Html = html;
        host.__vw2FeedNextAt = Date.now() + VW2_FEED_STEP_MS;
      }
      advanceV2FeedAutoFlow(host);
    }catch(_){ }
  }
