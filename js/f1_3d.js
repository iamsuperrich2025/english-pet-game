...
/* 🛞 ยางสึก + พิทสต็อป (รอบ 900) — จูนที่นี่ที่เดียว */
...
function surfAt(x,z,hint){
  ...
  else if(inPitLane(x,z,lat)) s='pit';                 // 🛞 รอบ 900 — เลนพิทมีผิวของตัวเอง
...
function buildTrackScene(){
  ...
  if(F1_MAP.pit&&F1_MAP.pit.length>2){
    ...
    for(let i=0;i<p.length;i++){
      ...
      const L=Math.hypot(dx,dz)||1e-6; dx/=L; dz/=L;
      /* 🛞 รอบ 900: กว้างเท่า PIT_HALF_W พอดี — ที่เห็น = ที่ลิมิตเตอร์จับ */
    ...
    buildPitBox();                                     // 🛞 รอบ 900 — ช่องจอดเปลี่ยนยาง + เสาป้าย
  }
...
#f1-cockpit{position:absolute;inset:0;z-index:5;pointer-events:none;display:none;
  background:url('img/f1/cockpit.webp') center bottom/cover no-repeat}
...
function build(){
  ...
  buildPitLine();          // 🛞 รอบ 900 — ต้องมาหลัง buildLine (ใช้ nearIdx หาฝั่งโรงรถ) และก่อน buildTrackScene
  ...
  /* 🛞 รอบ 900: เลนพิท (เส้นประเทา) + ช่องเปลี่ยนยาง (จุดฟ้า) */
  if(PITL){
...
function drsTick(dt,braking){
  ...
  drsInZone=!!drsZoneAt(i)&&fwd>2&&surfNow!=='sand';   // ต้องวิ่งตามทาง ไม่ใช่จมทราย
...
function physTick(dt){
  ...
  pitLaneNow=(surf==='pit');                           // 🛞 รอบ 900 (surfAt คำนวณให้แล้ว ไม่ต้องหาซ้ำ)
  ...
  /* 🚧 รอบ 900: ลิมิตเตอร์เลนพิท 80 กม./ชม. (อัตโนมัติเหมือนของจริง — เด็กไม่ต้องกดเอง) */
  pitLimited=false;
...
function progressTick(dt){
  ...
  if(lapStartAt&&prog<120&&lastProg>TOTAL-160&&cpFlags[0]&&cpFlags[1]&&cpFlags[2]){
    ...
    /* 🛞 รอบ 900: รอบที่แวะเลนพิท = ไม่นับสถิติ (กติกาจริง) แต่ยังได้เหรียญครบรอบ */
    if(lapPitted){
...
/* ============================================================
   🛞🔧 รอบ 900: ยางสึก + พิทสต็อปเปลี่ยนยาง
   ─────────────────────────────────────────────
...
function hudTick(){
  ...
  /* 🚧 รอบ 900: ลิมิตเตอร์ทำงาน = เลขความเร็วเป็นสีเหลือง (บอกว่าไม่ใช่รถเสีย) */
...
function start(){
  ...
  tyreReset();              // 🛞 รอบ 900 — ยางใหม่ทุกครั้งที่เข้าสนาม
