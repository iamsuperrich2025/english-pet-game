...
const CFG={
  ...
  runtimeVersion:'P1.2.6M-20260904-AIM-FIRE-EDGE-ZERO-DEADZONE',
...
class GlobalMobileTouchRouter{
  ...
  classifyRoleAt(x,y,target=null){const r=this.regions();if(this.protectedPointAt(x,y)||isProtectedFrontlineTarget(target))return {role:null,reason:'protected'};if(pointInRect(r.fire,x,y))return {role:'fire',reason:'actual-fire'};if(pointInRect(r.fireZone,x,y))return {role:'fire',reason:'fire-slop'};if(pointInRect(this.rect('aim'),x,y))return {role:'aim',reason:'actual-aim'};if(pointInRect(this.rect('drive'),x,y))return {role:'drive',reason:'actual-drive'};if(pointInRect(r.drive,x,y))return {role:'drive',reason:'drive-zone'};if(pointInRect(r.aim,x,y))return {role:'aim',reason:'aim-zone'};if(pointInRect(r.gameplay,x,y))return {role:null,reason:'safe-gameplay'};return {role:null,reason:'dead-zone'};}  roleAt(x,y,target){return this.classifyRoleAt(x,y,target).role;}
  ...
  begin(id,x,y,target,transport){this.observe(x,y,target,transport);if(id==null)return null;if(this.owners.has(id)){const existing=this.owners.get(id);if(existing==='aim')this.releaseAimPointer(id);else this.end(id,null,null,target,transport,true);}const decision=this.classifyRoleAt(x,y,target),role=decision.role;this.lastRoleReason=decision.reason;this.lastRoleDown=role?String(role).toUpperCase():(decision.reason==='safe-gameplay'?'TAP':'NONE');if(role==='aim'&&!this.recoverAimForBegin(id)){this.lastRoleDown='NONE';this.lastRoleReason='aim-busy';this.onDiagnostic(this.snapshot());return null;}if(!role||this.rolePointers[role]!=null){const tapped=this.beginTapCandidate(id,x,y,target,transport);this.lastRoleDown=tapped?'TAP':'NONE';if(role)this.lastRoleReason=String(role)+'-busy';this.onDiagnostic(this.snapshot());return null;}this.owners.set(id,role);this.rolePointers[role]=id;if(role==='drive'){this.recenter(role,x,y);this.driveState.id=id;this.applyStick(role,x,y,transport);}else if(role==='aim'){this.recenter(role,x,y);this.aimState.id=id;this.startAimGesture(id,x,y,transport);this.applyStick(role,x,y,transport);}else this.startFireGesture(id,x,y,transport);this.onDiagnostic(this.snapshot());return role;}
