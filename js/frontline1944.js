 ...
 * Phase 1.2.5 Mobile Aim Latch + Floating Controls: canonical tank controller, streamed battlefield sectors, logical world coordinates,
...
const CFG={
  ...
  cameraHeight:86,
  cameraOffsetZ:72,
  runtimeVersion:'P1.2.5-20260831-AIM-LATCH-FLOATING-CONTROLS',
...
const G={
  ...
  fireEventSerial:0,resources:null,raycaster:null,groundPlane:null,progressHydrated:false,lastControlInputAt:0,desktopFireQueued:false,runtimeIdentity:null,lastTouchLikeInputAt:0,mobileAimLatch:{valid:false,heading:null,lastUpdateAt:0},
};
...
function claimTankRuntimeOwnership(){
  const identity={kind:'tank',phase:'1.2.5',version:CFG.runtimeVersion,sharedRuntime:true,active:true};G.runtimeIdentity=identity;
...
function runtimeIdentity(){return G.runtimeIdentity||{kind:'tank',phase:'1.2.5',version:CFG.runtimeVersion,sharedRuntime:true,active:false};}
...
function makeDom(){
  ...
  <div class="fl44-hud"><div class="fl44-top">
    ...
    <div class="fl44-panel fl44-coins"><span>เหรียญรวม</span><strong id="fl44-coins">0</strong><span id="fl44-grade"></span><small id="fl44-sector">Sector 0</small><small id="fl44-runtime">P1.2.5</small><small id="fl44-input">GLOBAL INPUT READY</small></div>
  </div>
...
class MobileTankInputAdapter{
  sample(){const aimWorld=G.aim.active&&Math.hypot(G.aim.x,G.aim.y)>.12?screenAimDirection(G.aim.x,G.aim.y):null;if(aimWorld)latchMobileAimWorldDirection(aimWorld);return mobileCommandFromState(G.joy,G.aim,G.firing,aimWorld,mobileAimLatchedHeading());}
}
...
function mobileControlRegions(root,driveEl,aimEl,fireEl){
  const v=viewportRectFor(root),safe=safeAreaInsetsFor(root),usableLeft=v.left+safe.left,usableRight=v.right-safe.right,usableTop=v.top+safe.top,usableBottom=v.bottom-safe.bottom,w=Math.max(1,usableRight-usableLeft),h=Math.max(1,usableBottom-usableTop),driveRect=driveEl&&driveEl.getBoundingClientRect?driveEl.getBoundingClientRect():{width:96,height:96},aimRect=aimEl&&aimEl.getBoundingClientRect?aimEl.getBoundingClientRect():{width:82,height:82},fireRect=fireEl&&fireEl.getBoundingClientRect?fireEl.getBoundingClientRect():{width:68,height:68},margin=clamp(Math.min(w,h)*.028,8,22),split=usableLeft+w*.5,top=usableTop+Math.max(h*.43,Math.min(h-80,62)),bottom=usableBottom-margin,fireW=Math.max(Number(fireRect.width||68)*2.05,w*.18),fireH=Math.max(Number(fireRect.height||68)*2.05,h*.36),fireLeft=Math.max(split+margin+Number(aimRect.width||82)*.9,usableRight-margin-fireW),fireTop=Math.max(top,bottom-fireH);
  const drive=rectFromEdges(usableLeft+margin,top,split-margin,bottom),fire=rectFromEdges(fireLeft,fireTop,usableRight-margin,bottom),aim=rectFromEdges(split+margin,top,Math.max(split+margin,fire.left-margin),bottom);
  return {viewport:v,safeArea:safe,drive,aim,fire};
}
function elementUsableRect(el){if(!el)return null;try{if(el.hidden||el.getAttribute&&el.getAttribute('aria-hidden')==='true')return null;const r=el.getBoundingClientRect&&el.getBoundingClientRect();return r&&Number(r.width)>0&&Number(r.height)>0?r:null;}catch(_){return null;}}
function pointOverProtectedFrontlineUI(x,y){if(!G.root)return false;for(const sel of ['#fl44-exit','.fl44-top','.fl44-objective','.fl44-boss','.fl44-state']){let el=null;try{el=G.root.querySelector(sel);}catch(_){}const r=elementUsableRect(el);if(r&&pointInRect(r,x,y))return true;}return false;}
...
function isProtectedFrontlineTarget(target){
  let n=target;while(n&&n!==G.root){const tag=String(n.tagName||'').toUpperCase();if(n.id==='fl44-exit')return true;if((tag==='BUTTON'||tag==='A'||tag==='INPUT'||tag==='SELECT'||tag==='TEXTAREA')&&n.id!=='fl44-fire')return true;n=n.parentElement;}return false;
}
class GlobalMobileTouchRouter{
  constructor(opts={}){
    this.root=opts.root||null;this.driveEl=opts.driveEl||null;this.driveKnob=opts.driveKnob||null;this.aimEl=opts.aimEl||null;this.aimKnob=opts.aimKnob||null;this.fireEl=opts.fireEl||null;this.driveState=opts.driveState||{x:0,y:0,id:null,active:false};this.aimState=opts.aimState||{x:0,y:0,id:null,active:false};this.onFireChange=typeof opts.onFireChange==='function'?opts.onFireChange:()=>{};this.onAimVector=typeof opts.onAimVector==='function'?opts.onAimVector:()=>{};this.onDiagnostic=typeof opts.onDiagnostic==='function'?opts.onDiagnostic:()=>{};this.hitTargetAt=typeof opts.hitTargetAt==='function'?opts.hitTargetAt:null;this.protectedPointAt=typeof opts.protectedPointAt==='function'?opts.protectedPointAt:()=>false;this.owners=new Map();this.rolePointers={drive:null,aim:null,fire:null};this.gestureRects={drive:null,aim:null,fire:null};this.lastTransport='none';this.lastEventTarget='none';this.lastHitTarget='none';this.lastPoint={x:0,y:0};
  }
  rect(role){const el=role==='drive'?this.driveEl:role==='aim'?this.aimEl:this.fireEl;return el&&typeof el.getBoundingClientRect==='function'?el.getBoundingClientRect():null;}
  ...
  recenter(role,x,y){const el=role==='drive'?this.driveEl:role==='aim'?this.aimEl:this.fireEl,zone=this.regions()[role],base=this.rect(role)||{width:role==='drive'?96:role==='aim'?82:68,height:role==='drive'?96:role==='aim'?82:68},w=Math.max(1,Number(base.width)||1),h=Math.max(1,Number(base.height)||1),minX=zone.left+w/2,maxX=zone.left+zone.width-w/2,minY=zone.top+h/2,maxY=zone.top+zone.height-h/2,cx=maxX>=minX?clamp(Number(x)||0,minX,maxX):zone.left+zone.width/2,cy=maxY>=minY?clamp(Number(y)||0,minY,maxY):zone.top+zone.height/2,rect={left:cx-w/2,top:cy-h/2,width:w,height:h,right:cx+w/2,bottom:cy+h/2};this.gestureRects[role]=rect;if(el&&el.style){el.style.left=rect.left+'px';el.style.top=rect.top+'px';el.style.right='auto';el.style.bottom='auto';if(el.dataset)el.dataset.floating='1';}return rect;}
  ...
  begin(id,x,y,target,transport){
    this.observe(x,y,target,transport);if(id==null)return null;if(this.owners.has(id)){this.move(id,x,y,target,transport);return this.owners.get(id)||null;}const role=this.roleAt(x,y);if(!role||this.rolePointers[role]!=null)return null;this.owners.set(id,role);this.rolePointers[role]=id;this.recenter(role,x,y);
    if(role==='drive'){this.driveState.id=id;this.applyStick(role,x,y,transport);}else if(role==='aim'){this.aimState.id=id;this.applyStick(role,x,y,transport);}else{this.onFireChange(true);G.lastControlInputAt=inputNow();}
    this.onDiagnostic(this.snapshot());return role;
  }
  move(id,x,y,target,transport){
    this.observe(x,y,target,transport);const role=this.owners.get(id);if(!role)return null;if(role==='drive'||role==='aim')this.applyStick(role,x,y,transport);this.onDiagnostic(this.snapshot());return role;
  }
  end(id,x,y,target,transport){
    if(x!=null&&y!=null)this.observe(x,y,target,transport);const role=this.owners.get(id);if(!role)return null;if((role==='drive'||role==='aim')&&x!=null&&y!=null)this.applyStick(role,x,y,transport);this.owners.delete(id);if(this.rolePointers[role]===id)this.rolePointers[role]=null;if(role==='drive')resetStickState(this.driveState,this.driveKnob);else if(role==='aim')resetStickState(this.aimState,this.aimKnob);else this.onFireChange(false);this.gestureRects[role]=null;this.onDiagnostic(this.snapshot());return role;
  }
  ...
  cancelAll(){for(const id of [...this.owners.keys()])this.end(id,null,null,null,this.lastTransport);this.owners.clear();this.rolePointers={drive:null,aim:null,fire:null};this.gestureRects={drive:null,aim:null,fire:null};resetStickState(this.driveState,this.driveKnob);resetStickState(this.aimState,this.aimKnob);this.onFireChange(false);this.onDiagnostic(this.snapshot());}
  activeCount(){return this.owners.size;}
  snapshot(){const regions=this.regions();return {transport:this.lastTransport,pointers:this.owners.size,driveId:this.rolePointers.drive,aimId:this.rolePointers.aim,fireId:this.rolePointers.fire,eventTarget:this.lastEventTarget,hitTarget:this.lastHitTarget,point:{x:this.lastPoint.x,y:this.lastPoint.y},regions:{drive:regions.drive,aim:regions.aim,fire:regions.fire}};}
}
...
function bindGlobalMobileTouchRouter(){
  ...
  const router=new GlobalMobileTouchRouter({root:G.root,driveEl:drive,driveKnob,aimEl:aim,aimKnob,fireEl:fire,driveState:G.joy,aimState:G.aim,onFireChange:on=>{G.firing=!!on;fire.classList.toggle('on',!!on);},onAimVector:v=>latchMobileAimVector(v.x,v.y),onDiagnostic:updateInputDiagnostics,hitTargetAt:(x,y)=>document.elementFromPoint?document.elementFromPoint(x,y):null,protectedPointAt:pointOverProtectedFrontlineUI});
  ...
  if('PointerEvent' in window){
    ...
    const move=e=>{if(!touchPointer(e))return;markTouchLikeInput();claim(router.move(e.pointerId,e.clientX,e.clientY,e.target,'pointer'),e);},end=e=>{if(!touchPointer(e))return;markTouchLikeInput();claim(router.end(e.pointerId,e.clientX,e.clientY,e.target,'pointer'),e);};
    listen(window,'pointerdown',down,{passive:false,capture:true});listen(window,'pointermove',move,{passive:false,capture:true});listen(window,'pointerup',end,{passive:false,capture:true});listen(window,'pointercancel',end,{passive:false,capture:true});listen(window,'lostpointercapture',e=>{if(touchPointer(e))router.noteLostCapture(e.pointerId,e.target,'pointer');},{capture:true});
  }else{
    ...
    const end=e=>{markTouchLikeInput();let owned=false;for(const t of changed(e))if(router.end(t.identifier,t.clientX,t.clientY,t.target||e.target,'touch'))owned=true;if(owned)preventControlDefault(e);};
    listen(window,'touchstart',down,{passive:false,capture:true});listen(window,'touchmove',move,{passive:false,capture:true});listen(window,'touchend',end,{passive:false,capture:true});listen(window,'touchcancel',end,{passive:false,capture:true});
  }
  updateInputDiagnostics(router.snapshot());return router;
}
