...
const CFG={
  ...
  runtimeVersion:'P1.2.6I-20260904-AIM-OWNER-IMMEDIATE',
...
class GlobalMobileTouchRouter{
  ...
  startAimGesture(id,x,y,transport){const startedAt=inputNow(),g={id,startX:Number(x)||0,startY:Number(y)||0,lastX:Number(x)||0,lastY:Number(y)||0,startedAt,maxMove:0,transport};this.aimGesture=g;G.lastControlInputAt=startedAt;return g;}




  moveAimGesture(id,x,y,transport){const g=this.aimGesture;if(!g||g.id!==id)return this.applyStick('aim',x,y,transport);g.lastX=Number(x)||0;g.lastY=Number(y)||0;g.maxMove=Math.max(g.maxMove,Math.hypot(g.lastX-g.startX,g.lastY-g.startY));return this.applyStick('aim',x,y,transport);}
