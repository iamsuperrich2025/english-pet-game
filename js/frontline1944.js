...
const CFG={
  ...
  runtimeVersion:'P1.2.6K-20260904-TRUE-MOVABLE-AIM-ONE-TAP-ENTRY',
...
function applyAimRect(aimEl,rect){if(!aimEl||!rect||!aimEl.style)return null;aimEl.style.left=rect.left+'px';aimEl.style.top=rect.top+'px';aimEl.style.right='auto';aimEl.style.bottom='auto';if(aimEl.dataset)aimEl.dataset.floating='1';return rect;}
...
class GlobalMobileTouchRouter{
  ...
  recenter(role,x,y){if(role==='aim'){const rect=findSafeAimRect(this.root,this.driveEl,this.aimEl,this.fireEl,x,y);if(rect){this.gestureRects.aim=rect;applyAimRect(this.aimEl,rect);return rect;}}const el=this.driveEl,zone=this.regions().drive,base=this.rect('drive')||{width:96,height:96},w=Math.max(1,Number(base.width)||1),h=Math.max(1,Number(base.height)||1),minX=zone.left+w/2,maxX=zone.right-w/2,minY=zone.top+h/2,maxY=zone.bottom-h/2,cx=maxX>=minX?clamp(Number(x)||0,minX,maxX):zone.left+zone.width/2,cy=maxY>=minY?clamp(Number(y)||0,minY,maxY):zone.top+zone.height/2,rect={left:cx-w/2,top:cy-h/2,width:w,height:h,right:cx+w/2,bottom:cy+h/2};this.gestureRects.drive=rect;if(el&&el.style){el.style.left=rect.left+'px';el.style.top=rect.top+'px';el.style.right='auto';el.style.bottom='auto';if(el.dataset)el.dataset.floating='1';}return rect;}
  ...
  startAimGesture(id,x,y,transport){const startedAt=inputNow(),r=this.gestureRects.aim||this.rect('aim'),c=r?rectCenter(r):{x:Number(x)||0,y:Number(y)||0},g={id,startX:Number(x)||0,startY:Number(y)||0,lastX:Number(x)||0,lastY:Number(y)||0,baseX:c.x,baseY:c.y,startedAt,maxMove:0,transport};this.aimGesture=g;G.lastControlInputAt=startedAt;return g;}




  moveAimGesture(id,x,y,transport){const g=this.aimGesture;if(!g||g.id!==id)return this.applyStick('aim',x,y,transport);g.lastX=Number(x)||0;g.lastY=Number(y)||0;const dx=g.lastX-g.startX,dy=g.lastY-g.startY;g.maxMove=Math.max(g.maxMove,Math.hypot(dx,dy));const r=this.gestureRects.aim||this.rect('aim'),m=Math.max(1,Math.min(Number(r&&r.width)||82,Number(r&&r.height)||82)*.36),aimShare=.60;let vx=dx*aimShare,vy=dy*aimShare,L=Math.hypot(vx,vy);if(L>m){const k=m/L;vx*=k;vy*=k;}const next=findSafeAimRect(this.root,this.driveEl,this.aimEl,this.fireEl,g.lastX-vx,g.lastY-vy);if(next){this.gestureRects.aim=next;applyAimRect(this.aimEl,next);const c=rectCenter(next);g.baseX=c.x;g.baseY=c.y;}return this.applyStick('aim',x,y,transport);}
  ...
  move(id,x,y,target,transport){
    ...
    if(x!=null&&y!=null)this.observe(x,y,target,transport);let role=this.owners.get(id);if(!role&&(this.rolePointers.aim===id||(this.aimGesture&&this.aimGesture.id===id)||this.aimState.id===id))role='aim';if(!role){const fired=this.finishTapCandidate(id,x,y,target,cancelled);this.onDiagnostic(this.snapshot());return fired?'double-fire':null;}if(role==='drive'&&x!=null&&y!=null)this.applyStick(role,x,y,transport);else if(role==='aim'&&x!=null&&y!=null)this.moveAimGesture(id,x,y,transport);if(role==='aim')this.releaseAimPointer(id);else{this.owners.delete(id);if(this.rolePointers[role]===id)this.rolePointers[role]=null;if(role==='drive')resetStickState(this.driveState,this.driveKnob);else this.finishFireGesture(id,x,y,cancelled);this.gestureRects[role]=null;}this.onDiagnostic(this.snapshot());return role;
  ...
  noteLostCapture(id,target,transport){let role=this.owners.get(id);if(!role&&(this.rolePointers.aim===id||(this.aimGesture&&this.aimGesture.id===id)||this.aimState.id===id))role='aim';if(role==='drive')this.driveState.captured=false;else if(role==='aim')this.releaseAimPointer(id);if(target)this.lastEventTarget=eventTargetLabel(target);if(transport)this.lastTransport=transport;this.onDiagnostic(this.snapshot());return role||null;}
