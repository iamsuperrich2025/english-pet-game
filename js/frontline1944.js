...
class GlobalMobileTouchRouter{
  ...
  roleAt(x,y){if(this.protectedPointAt(x,y))return null;const r=this.regions();if(pointInRect(r.fire,x,y))return 'fire';if(pointInRect(r.drive,x,y))return 'drive';if(pointInRect(r.aim,x,y))return 'aim';return null;}
  ...
  begin(id,x,y,target,transport){
    ...
    if(role==='drive'){this.recenter(role,x,y);this.driveState.id=id;this.applyStick(role,x,y,transport);}else if(role==='aim'){this.recenter(role,x,y);this.aimState.id=id;this.applyStick(role,x,y,transport);}else this.startFireGesture(id,x,y,transport);
