...
function targetLockTargetValid(target){if(!target||!target.ref)return false;if(target.kind==='enemy'){const e=target.ref;if(!e||e.dead||!e.group||e.hp<=0||!G.enemies.includes(e))return false;return !G.sectorStreamer||G.sectorStreamer.isActive(WorldSpace.sectorIndexAtZ(e.world.z));}if(target.kind==='fortress'){const f=target.ref;if(!f||f!==G.fortress||!f.group||f.state!=='core'||f.coreHP<=0)return false;return !G.sectorStreamer||G.sectorStreamer.isActive(f.sectorIndex);}return false;}
...
function targetLockLabel(target){if(!target||!target.ref)return 'TARGET';if(target.kind==='fortress')return 'FORTRESS CORE';return target.ref.boss?'FORTRESS COMMANDER':'ENEMY '+String(target.ref.id||'').split(':').pop().toUpperCase();}
function targetLockCandidates(){const out=[];for(const e of G.enemies){if(!e||e.dead||!e.group||e.hp<=0)continue;const t={kind:'enemy',id:e.id,ref:e};if(targetLockTargetValid(t))out.push(t);}const f=G.fortress;if(f&&f.state==='core'&&f.coreHP>0){const t={kind:'fortress',id:f.id,ref:f};if(targetLockTargetValid(t))out.push(t);}return out;}
function pickTargetLockAtScreen(x,y){if(!G.canvas)return null;const ground=pointerToGround({clientX:Number(x)||0,clientY:Number(y)||0});if(!ground)return null;let unitsPerPx=1;try{const r=G.canvas.getBoundingClientRect();unitsPerPx=CFG.viewW/Math.max(1,Number(r.width)||1);}catch(_){}const touchWorld=Math.max(2.2,30*unitsPerPx);let best=null,bestScore=Infinity;for(const t of targetLockCandidates()){const w=targetLockWorld(t);if(!w)continue;const base=t.kind==='fortress'?5.0:Math.max(1,Number(t.ref.radius)||1),hitRadius=base+touchWorld,d=Math.hypot(ground.x-w.x,ground.z-w.z),score=d/hitRadius;if(d<=hitRadius&&score<bestScore){best=t;bestScore=score;}}return best;}
function selectTargetLockAtScreen(x,y){const s=specialControlState();if(!s.targetLockMode)return false;const target=pickTargetLockAtScreen(x,y);if(!target){if(!s.lockedTarget)showToast('TARGET LOCK READY','แตะให้ใกล้ตัวศัตรู / บอส / Fortress Core');return false;}s.lockedTarget=target;syncSpecialControlUI();showToast('TARGET LOCKED',targetLockLabel(target));return true;}
...
startSpecialGesture(role,id,x,y,transport){const g={role,id,startX:Number(x)||0,startY:Number(y)||0,lastX:Number(x)||0,lastY:Number(y)||0,maxMove:0,transport};this.specialGestures.set(id,g);G.lastControlInputAt=inputNow();return g;}
  ...
  begin(id,x,y,target,transport){this.observe(x,y,target,transport);if(id==null)return null;if(this.owners.has(id)){const existing=this.owners.get(id);if(existing==='aim')this.releaseAimPointer(id);else this.end(id,null,null,target,transport,true);}const decision=this.classifyRoleAt(x,y,target),role=decision.role;this.lastRoleReason=decision.reason;this.lastRoleDown=role?String(role).toUpperCase():(decision.reason==='safe-gameplay'?'TAP':'NONE');if(role==='aim'&&!this.recoverAimForBegin(id)){this.lastRoleDown='NONE';this.lastRoleReason='aim-busy';this.onDiagnostic(this.snapshot());return null;}if(!role||this.rolePointers[role]!=null){if(!role&&decision.reason==='safe-gameplay')this.onGameplayTapStart(x,y,target);const tapped=this.beginTapCandidate(id,x,y,target,transport);this.lastRoleDown=tapped?'TAP':'NONE';if(role)this.lastRoleReason=String(role)+'-busy';this.onDiagnostic(this.snapshot());return null;}this.owners.set(id,role);this.rolePointers[role]=id;if(role==='drive'){this.recenter(role,x,y);this.driveState.id=id;this.applyStick(role,x,y,transport);}else if(role==='aim'){this.recenter(role,x,y);this.aimState.id=id;this.startAimGesture(id,x,y,transport);this.applyStick(role,x,y,transport);}else if(role==='fire')this.startFireGesture(id,x,y,transport);else this.startSpecialGesture(role,id,x,y,transport);this.onDiagnostic(this.snapshot());return role;}
...
function bindGlobalMobileTouchRouter(){
  ...
  const router=new GlobalMobileTouchRouter({root:G.root,driveEl:drive,driveKnob,aimEl:aim,aimKnob,fireEl:fire,specialEls,driveState:G.joy,aimState:G.aim,onFireChange:on=>{G.firing=!!on;fire.classList.toggle('on',!!on);},onFirePulse:source=>queueMobileFirePulse(source),onAimVector:v=>latchMobileAimVector(v.x,v.y),onSpecialToggle:toggleSpecialControl,onGameplayTapStart:(x,y)=>{if(specialControlState().targetLockMode)selectTargetLockAtScreen(x,y);},onDiagnostic:updateInputDiagnostics,hitTargetAt:(x,y)=>document.elementFromPoint?document.elementFromPoint(x,y):null,protectedPointAt:pointOverProtectedFrontlineUI});
