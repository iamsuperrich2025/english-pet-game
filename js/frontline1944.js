...
function makeDom(){
  ...
  <button class="fl44-exit" id="fl44-exit">ออก</button><pre class="fl44-input-diag" id="fl44-input-diag" aria-hidden="true"></pre><div class="fl44-toast" id="fl44-toast"><b></b><span></span></div>`;
...
function inputDiagnosticsEnabled(){return !!(CFG.mobileInputDiagnostics&&adminAllowed()&&(explicitInputDiagnosticsRequested()||coarseInputEnvironment()));}
...
function isProtectedFrontlineTarget(target){
  let n=target;while(n&&n!==G.root){const tag=String(n.tagName||'').toUpperCase(),id=String(n.id||''),cls=typeof n.className==='string'?n.className:'',role=n.getAttribute&&String(n.getAttribute('role')||'').toLowerCase();if(id==='fl44-exit')return true;if(/\b(fl44-top|fl44-objective|fl44-boss|fl44-state|fl44-toast|fl44-loading|fl44-input-diag)\b/.test(cls))return true;if((tag==='BUTTON'||tag==='A'||tag==='INPUT'||tag==='SELECT'||tag==='TEXTAREA')&&id!=='fl44-fire')return true;if(role==='dialog'||role==='menu'||role==='menuitem'||role==='button')return id!=='fl44-fire';if(n.getAttribute&&n.getAttribute('aria-modal')==='true')return true;if(n.isContentEditable)return true;n=n.parentElement;}return false;
}
