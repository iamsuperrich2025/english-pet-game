...
const CFG={
  ...
  runtimeVersion:'P1.2.6L-20260904-AIM-DOM-POSITION-PROOF',
...
function aimPlacementBlockedRects(root,driveEl,fireEl){const out=protectedFrontlineRects(root).map(r=>expandRect(r,6));for(const el of [driveEl,fireEl]){const r=elementUsableRect(el);if(r)out.push(expandRect(r,10));}return out;}

function findSafeAimRect(root,driveEl,aimEl,fireEl,cx,cy){const safe=safeGameplayRect(root),base=elementUsableRect(aimEl)||{width:82,height:82},w=Math.max(1,Number(base.width)||82),h=Math.max(1,Number(base.height)||82),blocked=aimPlacementBlockedRects(root,driveEl,fireEl),desired={x:Number(cx)||safe.right-w/2,y:Number(cy)||safe.bottom-h/2},candidates=[],seen=new Set(),add=(x,y)=>{const r=fireRectAtCenter(x,y,w,h,safe),c=rectCenter(r),k=Math.round(c.x*10)+':'+Math.round(c.y*10);if(!seen.has(k)){seen.add(k);candidates.push(r);}};add(desired.x,desired.y);const gap=8;for(const b of blocked){add(b.left-w/2-gap,desired.y);add(b.right+w/2+gap,desired.y);add(desired.x,b.top-h/2-gap);add(desired.x,b.bottom+h/2+gap);}candidates.sort((a,b)=>{const ca=rectCenter(a),cb=rectCenter(b),da=(ca.x-desired.x)**2+(ca.y-desired.y)**2,db=(cb.x-desired.x)**2+(cb.y-desired.y)**2;return da-db;});for(const r of candidates)if(fireRectIsValid(r,safe,blocked))return r;const step=Math.max(14,Math.min(w,h)*.35);let best=null,bestD=Infinity;for(let y=safe.top+h/2;y<=safe.bottom-h/2+1;y+=step)for(let x=safe.left+w/2;x<=safe.right-w/2+1;x+=step){const r=fireRectAtCenter(x,y,w,h,safe);if(!fireRectIsValid(r,safe,blocked))continue;const c=rectCenter(r),d=(c.x-desired.x)**2+(c.y-desired.y)**2;if(d<bestD){best=r;bestD=d;}}return best;}
...
class GlobalMobileTouchRouter{
  ...
  recoverAimForBegin(id){const stale=new Set();if(this.rolePointers.aim!=null)stale.add(this.rolePointers.aim);for(const [ownerId,role] of this.owners)if(role==='aim')stale.add(ownerId);if(this.aimGesture&&this.aimGesture.id!=null)stale.add(this.aimGesture.id);if(this.aimState.id!=null)stale.add(this.aimState.id);for(const staleId of stale)this.releaseAimPointer(staleId);this.rolePointers.aim=null;this.aimGesture=null;this.gestureRects.aim=null;resetStickState(this.aimState,this.aimKnob);return true;}
  ...
  begin(id,x,y,target,transport){
    ...
    this.observe(x,y,target,transport);if(id==null)return null;if(this.owners.has(id)){const existing=this.owners.get(id);if(existing==='aim')this.releaseAimPointer(id);else this.end(id,null,null,target,transport,true);}const role=this.roleAt(x,y);if(role==='aim')this.recoverAimForBegin(id);if(!role||this.rolePointers[role]!=null){this.beginTapCandidate(id,x,y,target,transport);return null;}this.owners.set(id,role);this.rolePointers[role]=id;




    if(role==='drive'){this.recenter(role,x,y);this.driveState.id=id;this.applyStick(role,x,y,transport);}else if(role==='aim'){/* True floating AIM: each new AIM pointerdown owns AIM, recenters the base immediately to this touch, and begins normal turret aiming from that center. There is no delayed AIM reposition intent while the pointer remains down. */this.recenter(role,x,y);this.aimState.id=id;this.startAimGesture(id,x,y,transport);this.applyStick(role,x,y,transport);}else this.startFireGesture(id,x,y,transport);
  ...
  noteLostCapture(id,target,transport){let role=this.owners.get(id);if(!role&&(this.rolePointers.aim===id||(this.aimGesture&&this.aimGesture.id===id)||this.aimState.id===id))role='aim';if(role==='drive')this.driveState.captured=false;else if(role==='aim'){this.aimDiag.lostCount++;this.releaseAimPointer(id);}if(target)this.lastEventTarget=eventTargetLabel(target);if(transport)this.lastTransport=transport;this.onDiagnostic(this.snapshot());return role||null;}
...
function updateInputDiagnostics(snapshot){
  ...
  const delivery=renderRuntimeIdentity(),ad=r.aimDiag||{},dom=r.aimDom||{},inline=dom.inline||{},computed=dom.computed||{},op=dom.offsetParent||{},sp=ad.startPointer||{},lp=ad.lastPointer||r.point||{},req=ad.requestedBase||{},act=ad.actualBase||r.aimBase||{};
