 ...
 * Phase 1.2.6D Runtime Freshness Rebase: accepted Phase 1.2.6 controls plus deterministic delivery identity, logical world coordinates,
...
const CFG={
  ...
  runtimeVersion:'P1.2.6D-20260902-f02fff',
...
function mobileControlRegions(root,driveEl,aimEl,fireEl){
  ...
  const drive=rectFromEdges(usableLeft+margin,top,split-margin,bottom),fireZone=rectFromEdges(fireLeft,fireTop,usableRight-margin,bottom),aim=rectFromEdges(split+margin,top,Math.max(split+margin,fireZone.left-margin),bottom),actualFire=elementUsableRect(fireEl)||rectFromEdges(Number(fireRect.left)||fireZone.left,Number(fireRect.top)||fireZone.top,(Number(fireRect.left)||fireZone.left)+Math.max(1,Number(fireRect.width)||68),(Number(fireRect.top)||fireZone.top)+Math.max(1,Number(fireRect.height)||68));
...
class GlobalMobileTouchRouter{
  ...
  begin(id,x,y,target,transport){
    ...
    if(role==='drive'){this.recenter(role,x,y);this.driveState.id=id;this.applyStick(role,x,y,transport);}else if(role==='aim'){const directAim=role==='aim'&&pointInRect(this.rect('aim'),x,y);if(directAim)this.gestureRects[role]=this.rect('aim');else this.recenter(role,x,y);this.aimState.id=id;this.applyStick(role,x,y,transport);}else this.startFireGesture(id,x,y,transport);
  ...
  move(id,x,y,target,transport){
    ...
    this.observe(x,y,target,transport);const role=this.owners.get(id);if(!role){this.moveTapCandidate(id,x,y);return null;}if(role==='drive'||role==='aim')this.applyStick(role,x,y,transport);else this.moveFireGesture(id,x,y);this.onDiagnostic(this.snapshot());return role;
    ...
    if(x!=null&&y!=null)this.observe(x,y,target,transport);const role=this.owners.get(id);if(!role){const fired=this.finishTapCandidate(id,x,y,target,cancelled);this.onDiagnostic(this.snapshot());return fired?'double-fire':null;}if((role==='drive'||role==='aim')&&x!=null&&y!=null)this.applyStick(role,x,y,transport);this.owners.delete(id);if(this.rolePointers[role]===id)this.rolePointers[role]=null;if(role==='drive')resetStickState(this.driveState,this.driveKnob);else if(role==='aim')resetStickState(this.aimState,this.aimKnob);else this.finishFireGesture(id,x,y,cancelled);this.gestureRects[role]=null;this.onDiagnostic(this.snapshot());return role;
  ...
  cancelAll(){for(const id of [...this.owners.keys()])this.cancel(id,null,null,null,this.lastTransport);this.owners.clear();this.tapCandidates.clear();this.lastTap=null;this.rolePointers={drive:null,aim:null,fire:null};this.gestureRects={drive:null,aim:null,fire:null};resetStickState(this.driveState,this.driveKnob);resetStickState(this.aimState,this.aimKnob);this.onFireChange(false);if(this.fireEl&&this.fireEl.classList)this.fireEl.classList.remove('repositioning');this.onDiagnostic(this.snapshot());}
