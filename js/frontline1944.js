 ...
 * Phase 1.2.4 Global Mobile Touch Router: canonical tank controller, streamed battlefield sectors, logical world coordinates,
...
const CFG={
  ...
  cameraHeight:86,
  cameraOffsetZ:72,
  runtimeVersion:'P1.2.4-20260831-GLOBAL-TOUCH-ROUTER',
...
const G={
  ...
  layers:{},keys:new Set(),listeners:[],joy:{x:0,y:0,id:null,active:false,transport:'idle',lastTransport:'none',captured:false,moves:0,lastEventAt:0},aim:{x:0,y:-1,id:null,active:false,transport:'idle',lastTransport:'none',captured:false,moves:0,lastEventAt:0},mobileRouter:null,
...
function claimTankRuntimeOwnership(){
  const identity={kind:'tank',phase:'1.2.4',version:CFG.runtimeVersion,sharedRuntime:true,active:true};G.runtimeIdentity=identity;
...
function runtimeIdentity(){return G.runtimeIdentity||{kind:'tank',phase:'1.2.4',version:CFG.runtimeVersion,sharedRuntime:true,active:false};}
...
function makeDom(){
  ...
  <div class="fl44-hud"><div class="fl44-top">
    ...
    <div class="fl44-panel fl44-coins"><span>เหรียญรวม</span><strong id="fl44-coins">0</strong><span id="fl44-grade"></span><small id="fl44-sector">Sector 0</small><small id="fl44-runtime">P1.2.4</small><small id="fl44-input">GLOBAL INPUT READY</small></div>
  </div>
...
class GlobalMobileTouchRouter{
  ...
  roleAt(x,y){if(pointInRect(this.rect('fire'),x,y))return 'fire';if(pointInRect(this.rect('aim'),x,y))return 'aim';if(pointInRect(this.rect('drive'),x,y))return 'drive';return null;}
  ...
  applyStick(role,x,y,transport){const state=role==='drive'?this.driveState:this.aimState,el=role==='drive'?this.driveEl:this.aimEl,knob=role==='drive'?this.driveKnob:this.aimKnob,rect=el&&el.getBoundingClientRect?el.getBoundingClientRect():null,v=stickVectorFromRect(rect,x,y);state.x=v.x;state.y=v.y;state.active=true;state.transport=transport;state.lastTransport=transport;state.lastEventAt=inputNow();state.moves=(Number(state.moves)||0)+1;state.captured=false;if(knob&&knob.style)knob.style.transform='translate('+v.dx+'px,'+v.dy+'px)';G.lastControlInputAt=state.lastEventAt;return v;}
  begin(id,x,y,target,transport){
    this.observe(x,y,target,transport);if(id==null)return null;if(this.owners.has(id)){this.move(id,x,y,target,transport);return this.owners.get(id)||null;}const role=this.roleAt(x,y);if(!role||this.rolePointers[role]!=null)return null;this.owners.set(id,role);this.rolePointers[role]=id;
  ...
  move(id,x,y,target,transport){
    ...
    if(x!=null&&y!=null)this.observe(x,y,target,transport);const role=this.owners.get(id);if(!role)return null;this.owners.delete(id);if(this.rolePointers[role]===id)this.rolePointers[role]=null;if(role==='drive')resetStickState(this.driveState,this.driveKnob);else if(role==='aim')resetStickState(this.aimState,this.aimKnob);else this.onFireChange(false);this.onDiagnostic(this.snapshot());return role;
  }
  ...
  cancelAll(){for(const id of [...this.owners.keys()])this.end(id,null,null,null,this.lastTransport);this.owners.clear();this.rolePointers={drive:null,aim:null,fire:null};resetStickState(this.driveState,this.driveKnob);resetStickState(this.aimState,this.aimKnob);this.onFireChange(false);this.onDiagnostic(this.snapshot());}
  activeCount(){return this.owners.size;}
  snapshot(){return {transport:this.lastTransport,pointers:this.owners.size,driveId:this.rolePointers.drive,aimId:this.rolePointers.aim,fireId:this.rolePointers.fire,eventTarget:this.lastEventTarget,hitTarget:this.lastHitTarget,point:{x:this.lastPoint.x,y:this.lastPoint.y}};}
}
function updateInputDiagnostics(snapshot){
  if(!CFG.mobileInputDiagnostics||!G.root)return;const el=$('#fl44-input-diag');if(!el)return;const r=snapshot||(G.mobileRouter&&G.mobileRouter.snapshot())||{transport:('PointerEvent' in window)?'POINTER':'TOUCH',pointers:0,eventTarget:'none',hitTarget:'none'},cmd=mobileCommandFromState(G.joy,G.aim,G.firing,null),f=n=>Number.isFinite(Number(n))?Number(n).toFixed(2):'0.00';
  el.textContent='INPUT: '+String(r.transport||'NONE').toUpperCase()+'\nPOINTERS: '+Number(r.pointers||0)+'\nDRIVE: DOWN='+(G.joy.active?1:0)+' ID='+(G.joy.id==null?'—':G.joy.id)+' X='+f(G.joy.x)+' Y='+f(G.joy.y)+' THROTTLE='+f(cmd.throttle)+' STEER='+f(cmd.steering)+'\nAIM: DOWN='+(G.aim.active?1:0)+' ID='+(G.aim.id==null?'—':G.aim.id)+' X='+f(G.aim.x)+' Y='+f(G.aim.y)+'\nFIRE: DOWN='+(G.firing?1:0)+' ID='+(r.fireId==null?'—':r.fireId)+'\nEVENT TARGET: '+String(r.eventTarget||'none')+'\nHIT TEST: '+String(r.hitTarget||'none');
}
function bindGlobalMobileTouchRouter(){
  ...
  const router=new GlobalMobileTouchRouter({root:G.root,driveEl:drive,driveKnob,aimEl:aim,aimKnob,fireEl:fire,driveState:G.joy,aimState:G.aim,onFireChange:on=>{G.firing=!!on;fire.classList.toggle('on',!!on);},onDiagnostic:updateInputDiagnostics,hitTargetAt:(x,y)=>document.elementFromPoint?document.elementFromPoint(x,y):null});
  ...
  if('PointerEvent' in window){
    const down=e=>{if(protectedTarget(e.target)){router.observe(e.clientX,e.clientY,e.target,'pointer');return;}claim(router.begin(e.pointerId,e.clientX,e.clientY,e.target,'pointer'),e);};
    const move=e=>claim(router.move(e.pointerId,e.clientX,e.clientY,e.target,'pointer'),e),end=e=>claim(router.end(e.pointerId,e.clientX,e.clientY,e.target,'pointer'),e);
    listen(window,'pointerdown',down,{passive:false,capture:true});listen(window,'pointermove',move,{passive:false,capture:true});listen(window,'pointerup',end,{passive:false,capture:true});listen(window,'pointercancel',end,{passive:false,capture:true});listen(window,'lostpointercapture',e=>router.noteLostCapture(e.pointerId,e.target,'pointer'),{capture:true});
  }else{
    const changed=e=>[...(e.changedTouches||[])],moved=e=>{const c=changed(e);return c.length?c:[...(e.touches||[])];};
    const down=e=>{let owned=false;for(const t of changed(e)){const target=t.target||e.target;if(protectedTarget(target)){router.observe(t.clientX,t.clientY,target,'touch');continue;}if(router.begin(t.identifier,t.clientX,t.clientY,target,'touch'))owned=true;}if(owned)preventControlDefault(e);};
    const move=e=>{let owned=false;for(const t of moved(e))if(router.move(t.identifier,t.clientX,t.clientY,t.target||e.target,'touch'))owned=true;if(owned)preventControlDefault(e);};
    const end=e=>{let owned=false;for(const t of changed(e))if(router.end(t.identifier,t.clientX,t.clientY,t.target||e.target,'touch'))owned=true;if(owned)preventControlDefault(e);};
