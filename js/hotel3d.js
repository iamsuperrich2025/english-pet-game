...
/* 🩹 รอบ 778: ช่วง "ไต่" ของบันไดต้องหยุดก่อนถึงผนัง CORE_E แล้วปูชานพักราบต่อจนถึงผนัง
   (ไม่งั้นขั้นสุดท้ายจ่อกำแพงเป๊ะ = อาการ "บันไดชนกำแพง" ที่ผู้ใช้แจ้ง) · surfaceY() ใช้ค่าชุดเดียวกันนี้ */
const STAIR_TOP_D=1.8;           // ความลึกชานพักบนสุดของแต่ละช่วงบันได
const STAIR_BOT_D=.5;            // ชานพักล่างสั้น ๆ ก่อนเริ่มไต่ (ช่องก้าวขึ้นบันไดจะได้กว้างพอ)
const RAMP_X0=WEST+STAIR_BOT_D;  // x ที่เริ่มไต่
const RAMP_X1=CORE_E-STAIR_TOP_D;// x ที่ไต่ถึงระดับชั้นบนแล้ว (จากนี้ไปเป็นพื้นราบ)
const RAMP_RUN=RAMP_X1-RAMP_X0;  // ระยะแนวราบของช่วงไต่
...
function build(THREE_,opt){
  ...
  /* ---------- 🪜 บันได: ทางลาดมีลูกนอน + ราวกันตกไต่ตามความชัน ----------
     🩹 รอบ 778 (ผู้ใช้ส่งภาพ: "บันไดทางขึ้นชนกับกำแพง"): เดิมทางลาดไต่เต็ม 7 ม. ไปจบที่ x=CORE_E พอดี
     ซึ่งเป็นผนังกั้นแกนบันไดกับห้อง N1 → ขั้นสุดท้ายจ่อกำแพงเป๊ะ เดินขึ้นไปแล้วหน้าชนผนัง ดูเหมือนบันไดสร้างชนกำแพง
     → หด "ช่วงไต่" ให้จบก่อนถึงผนัง แล้วปูชานพักบนราบ ๆ ต่อจนถึงผนัง (STAIR_TOP_D)
       เดินขึ้นมาถึงจะได้ยืนบนพื้นราบก่อน แล้วเลี้ยวขวา (ทิศ +Z) ออกไปชานพัก/ทางเดินได้เหมือนบันไดจริง */
  const RUN=RAMP_RUN;                                      // ระยะแนวราบของช่วงไต่ (ค่าคงที่บนหัวไฟล์)
  const rampY=x=>Math.max(0,Math.min(1,(x-RAMP_X0)/RUN))*FLOOR_H;
  for(let f=0;f<FLOORS-1;f++){
    const y=floorY(f), ang=Math.atan2(FLOOR_H,RUN), len=Math.hypot(RUN,FLOOR_H);
    accBox(A.stone,(RAMP_X0+RAMP_X1)/2,y+FLOOR_H/2-.16,(RZ0+RZ1)/2,len,.3,RZ1-RZ0,2.4,{z:ang});
    for(let s=0;s<12;s++){                                  // ลูกนอนให้เห็นเป็นขั้นบันไดจริง
      const t=(s+.5)/12, sx=RAMP_X0+t*RUN;
      accBox(A.stone,sx,y+rampY(sx)+.06,(RZ0+RZ1)/2,RUN/12*.94,.12,RZ1-RZ0-.2,2.4);
    }
    // ชานพักล่าง/บน (ราบ) หัวท้ายทางลาด — ล่างไว้ก้าวขึ้นสบาย ๆ · บนไว้ยืนพักก่อนเลี้ยวออกทางเดิน
    accBox(A.stone,(WEST+RAMP_X0)/2,y-.15,(RZ0+RZ1)/2,STAIR_BOT_D,.3,RZ1-RZ0,2.4);
    accBox(A.stone,(RAMP_X1+CORE_E)/2,y+FLOOR_H-.15,(RZ0+RZ1)/2,STAIR_TOP_D,.3,RZ1-RZ0,2.4);
    /* ราวกันตกด้านชานพัก — คลุมเฉพาะ "ช่วงที่พื้นต่างระดับจริง"
       เริ่มที่ WEST+2.0: ก่อนหน้านั้นทางลาดยังสูงไม่ถึงเข่า (<STEP_MAX) = ช่องก้าวขึ้นบันไดจากชานพักล่าง
       จบที่ RAMP_X1: จากตรงนั้นไปถึงผนังเป็นชานพักบนระดับเดียวกับพื้นชั้น = ทางออกไปทางเดิน ห้ามกั้น */
    for(let s=0;s<5;s++){
      const x0=WEST+2.0+s*.72, x1=Math.min(x0+.72,RAMP_X1), ry=y+rampY((x0+x1)/2);
      if(x1<=x0) break;
  ...
  const PORTRAIT_X_N=[-10.2,-1.5,2.9,11.2];        // ผนังทางเดินฝั่งเหนือ (ชั้น 1-4)
  const PORTRAIT_X_S=[-4.5,7.0];                   // ผนังทางเดินฝั่งใต้ (ชั้น 1-4)
  const LOBBY_X_N=[-8,-2,3,16], LOBBY_X_S=[-6,8];  // ล็อบบี้: ผนังนอกเหนือ/ใต้ (เลี่ยงเคาน์เตอร์+กระถาง)
  for(let f=0;f<FLOORS;f++){
  ...
  [[-2,-5.3],[-2,5.3],[8,5.3],[15,-5.3],[2.5,8],[9,-4],[16.5,3.5],[16.5,-3.5],[5,0],[17,0]]
    .forEach(p=>H.spots.push({x:p[0],z:p[1],y:0,room:'lobby'}));
  return H;
}
...
function surfaceY(H,x,z,curY){
  ...
  if(inRect(x,z,WEST,CORE_E,RZ0,RZ1)){                            // อยู่บนทางลาดบันได/ชานพักบน
    /* t=0 ก่อน RAMP_X0 (ชานพักล่าง) · t=1 ตั้งแต่ RAMP_X1 ถึงผนัง (ชานพักบน) — ราบทั้งสองหัว (รอบ 778) */
    const t=Math.max(0,Math.min(1,(x-RAMP_X0)/RAMP_RUN));
    let best=0,bd=1e9;
    for(let f=0;f<FLOORS-1;f++){
