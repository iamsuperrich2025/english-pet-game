...
assert(a.includes("file:'MissileLaunch.mp3'"),'launch sfx file');
assert(a.includes("dir:'/sound/robot/'"),'launch sfx path');
assert(a.includes("playClip('launch')"),'FIRE uses MissileLaunch');
...
assert(fs.existsSync('sound/robot/MissileLaunch.mp3'),'MissileLaunch asset on disk');

assert(fx.includes('hitBoom'),'fx fire-ring hit branch');
assert(fx.includes('0xff812e'),'fx fire orange');
assert(fx.includes('shot.hit?920:260'),'longer hit impact duration');
