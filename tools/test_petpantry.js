...
const ctx={console,window:{},structuredClone,saveState(){},renderDashboard(){},syncHeader(){},state:{coins:20000,petPantry:{shelfId:null,stock:{}},owned:[]},sfx:{buy(){}},activePet(){return {type:'cat',sick:true}},foodSafeForPetMenu:f=>!!f&&!f.human&&!(Array.isArray(f.badFor)&&f.badFor.length),PETS:{dog:{favFood:{emoji:'🦴',name:'ขนมสุนัข',price:300}},cat:{favFood:{emoji:'🐟',name:'ปลา',price:300}},dragon:{favFood:{emoji:'🌶️',name:'พริก',price:300}}},FOODS:[{id:'apple',name:'แอปเปิ้ล',emoji:'🍎',price:150},{id:'feast',name:'ชุดอาหาร',emoji:'🍱',price:1000},{id:'choco',name:'ช็อกโกแลต',emoji:'🍫',price:200,human:true,badFor:['dog','cat','dragon']}],ITEMS:[]};
...
r=p.buyFood('apple',5);assert(r.ok&&r.cost===750&&p.qty('apple')===5,'sick pet must still be able to buy food stock');const coins=ctx.state.coins;assert(p.take('apple',1)&&p.qty('apple')===4&&ctx.state.coins===coins,'feeding take must debit stock only');
