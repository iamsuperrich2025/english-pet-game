...
(async()=>{recipes();await new Promise(r=>server.listen(0,'127.0.0.1',r));const browser=await chromium.launch({channel:'msedge',headless:true}),page=await browser.newPage({viewport:{width:1366,height:768}}),errors=[],requests=[];page.on('pageerror',e=>errors.push(e.message));page.on('console',m=>{if(m.type()==='error'&&/Shader Error|VALIDATE_STATUS|WebGLProgram/.test(m.text()))errors.push(m.text());});page.on('request',r=>requests.push(new URL(r.url()).pathname));
 try{
  ...
  const initialCoins=await page.evaluate(()=>state.coins);await page.click('[data-equip-spell="dragon_breath"]');
  ok('unconfirmed spell stays unpaid and keeps the grimoire open',await page.evaluate(n=>state.coins===n&&!state.arenaItems.spell_dragon_breath&&document.querySelector('#va-spellbook').classList.contains('on')&&document.querySelector('#va-buy-confirm').classList.contains('on'),initialCoins));
  ok('confirm dialog fits the short HUD',await page.evaluate(()=>{const c=document.querySelector('#va-buy-confirm .va-buy-card'),r=c.getBoundingClientRect();return r.left>=0&&r.top>=0&&r.right<=innerWidth+1&&r.bottom<=innerHeight+1&&c.scrollHeight<=c.clientHeight+1;}));
  await page.click('#va-buy-cancel');
  ok('cancel returns to the open grimoire without charging',await page.evaluate(n=>state.coins===n&&!state.arenaItems.spell_dragon_breath&&document.querySelector('#va-spellbook').classList.contains('on')&&!document.querySelector('#va-buy-confirm').classList.contains('on'),initialCoins));
  await page.click('[data-equip-spell="dragon_breath"]');await page.click('#va-buy-ok');await page.waitForFunction(()=>VocabArena3D._t.slots[1]==='dragon_breath');
  ...
  await page.evaluate(()=>VocabArena3D._t.spellbook(true));await page.selectOption('[aria-label="หมวดธาตุ"]','ice');await page.fill('[aria-label="ค้นหาเวทมนตร์"]','');await page.route('**/js/arena-spells/ice.js',r=>r.abort());await page.click('[data-equip-spell="frost_lotus"]');await page.click('#va-buy-ok');await page.waitForFunction(()=>document.querySelector('#va-spell-load').textContent.includes('ไม่สำเร็จ'));
  ok('load failure preserves equipped spell and offers retry',await page.evaluate(()=>VocabArena3D._t.slots[1]==='dragon_breath'));await page.unroute('**/js/arena-spells/ice.js');await page.click('[data-equip-spell="frost_lotus"]');await page.click('#va-buy-ok');await page.waitForFunction(()=>VocabArena3D._t.slots[1]==='frost_lotus');
