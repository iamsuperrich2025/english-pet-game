...
Audio uses `frontline-synth.js` (24 transient voices maximum and one reusable noise buffer), `frontline-score.js` as a quiet pentatonic fallback if the file cannot play, and `frontline-audio.js` for cues plus looping background music. After JOIN the browser downloads **one** hashed track (`sound/Frontline/bgmusic-9295a3b2636fb961.ogg` Opus VBR ~80 kbps, or `bgmusic-a88cbf606d0bd932.mp3` 128 kbps if Opus is unavailable), plays it from a compressed Blob (`preload='none'`), and reuses Cache Storage `vw-assets-content-v1` so loops, mute toggles, EXIT/re-join and reloads do not fetch again. The original `Arcade_Adventure_bgm.mp3` is not a runtime URL. Mute is remembered, hidden pages pause music, reward cues duck the loop, and EXIT disposes the player without clearing the cache. The Lobby never loads these scripts. See `SOUND_LICENSES.md`.
...
The orange DROP button leaves the carried card behind the hull, with safe placement around map edges and intact vaults. Empty/dead/offline/pending players cannot press it. An unneeded-letter hint accounts for duplicate letters already banked, and the coach says a cute Thai “หยิบตัวอักษรผิดแล้ว · กด DROP วางลงนะ”. Tall landscape screens place DROP above BOMB; short screens place it beside BOMB to keep all controls accessible. The owner has a 1,250 ms re-pickup lock, while rivals may pick it up immediately. Manual cards remain until collected, separately from forced combat drops. Up to 32 manual cards may be on the field; at capacity the card stays carried and the HUD asks players to collect loose cards first. DROP does not change banks or coins.
...
Focused browser-global modules divide config, movement, input, combat, bombs, A-Z pickups, vaults, words, bots, neutral guards, networking, room simulation, recycled map chunks, shapes, scene rendering, effects, synthesized audio, UI, and economy integration. The painted meadow is one world-shaded sheet that follows the camera every frame, so the grass does not hitch when tiles recycle. Fifteen procedural chunks still hold bushes, crates and trees; only the column or row that left the window is reassigned. Instanced grass/daisies keep three draw calls and rewrite matrices only for those recycled cells. The toy garden has continuous scalloped dirt lanes, rounded flowering bushes, crates, fences, cube trees and instanced grass/daisies in three draw calls. The battlefield loads no image textures or icon fonts. A single 512px shadow target recenters on texel-aligned steps so received ground shadows stay put between moves; effects do not cast shadows or create lights. The HUD uses small inline SVG illustrations and glossy CSS controls. Needed letters in the target word keep a gold card on the field; letters still missing from your bank that sit off-screen get a gold edge arrow with the letter on it. The shared tank GLB is 130,640 bytes, 3,754 vertices and 5,124 triangles.
...
- `node --test tools/frontline-v1/frontline-audio.test.mjs` for hashed Opus/MP3 sizes, runtime URLs and preview CSP
- `node tools/frontline-v1/audio.browser.test.mjs` for one-format download after JOIN, loop without a second fetch, reload from cache, and MP3 fallback; set `FRONTLINE_PLAYWRIGHT` if Playwright is not locally resolvable
...
## Smooth render reconciliation (round 1402)

`frontline-scene.js` owns `makeRenderPose`: normal driving renders immediately; server position corrections and bump revisions settle through a 120 ms exponential visual offset. Respawn/re-entry and large teleports reset the view. Tank, camera, terrain selection, labels, lighting and local muzzle feedback share the displayed pose; collision, controls, server state and network cadence are unchanged. Labels project once per frame after the camera update, and stable label/HUD text is written only when changed.

`node --test tools/frontline-v1/frontline-scroll.test.mjs` covers forward/reverse turns at 30/60/120 Hz, corrections, repeated bumps, reset and state preservation. `node tools/frontline-v1/scroll.browser.test.mjs` uses the isolated preview to measure camera jumps, DOM mutations and render budgets. Baseline 3-unit correction: first frame 3 → 0.389 units; unchanged text writes over 60 frames 2,100 → 0; same 420 draw calls/80,819 triangles in the controlled fixture. Desktop p95 remained about 17 ms; these figures do not establish frame rates on the user's physical phone.
