/* Fixed instanced grass and daisies: three draw calls, repositioned only when chunks recycle. */
(function(){
  'use strict';
  ...
  F.makeFlora=function(root){
    ...
    const grass=new T.InstancedMesh(leafGeo,leafMat,15*40*3),petals=new T.InstancedMesh(flowerGeo,flowerMat,15*10),pollen=new T.InstancedMesh(pollenGeo,pollenMat,15*10);
    ...
    return{update(cx,cz,hash){
      let blades=0,blooms=0;const size=F.C.chunkSize;
      for(let dz=-1;dz<=1;dz++)for(let dx=-2;dx<=2;dx++)for(let i=0;i<40;i++){
        const tx=cx+dx,tz=cz+dz,x=(tx+hash(tx,tz,400+i*7)-.5)*size,z=(tz+hash(tx,tz,401+i*7)-.5)*size;
        if(!allowed(x,z))continue;
        const yaw=hash(tx,tz,402+i*7)*Math.PI*2,scale=1.05+hash(tx,tz,403+i*7)*.55;
        for(let n=0;n<3;n++){
          dummy.position.set(x+(n-1)*.20,-.04,z);dummy.rotation.set(-.8,Math.sin(yaw)*.3,(n-1)*-.48);dummy.scale.set(scale*1.4,scale,scale);
          dummy.updateMatrix();grass.setMatrixAt(blades,dummy.matrix);grass.setColorAt(blades,leaves[n]);blades++;
        }
        if(i%4===0){
          dummy.position.set(x+.35,.045,z+.25);dummy.rotation.set(-Math.PI/2,0,yaw);dummy.scale.setScalar(.8+scale*.3);dummy.updateMatrix();
          petals.setMatrixAt(blooms,dummy.matrix);dummy.position.y=.055;dummy.updateMatrix();pollen.setMatrixAt(blooms,dummy.matrix);blooms++;
        }
      }
      grass.count=blades;petals.count=pollen.count=blooms;
      for(const mesh of [grass,petals,pollen])mesh.instanceMatrix.needsUpdate=true;
      if(grass.instanceColor)grass.instanceColor.needsUpdate=true;
    },dispose(){for(const mesh of [grass,petals,pollen]){root.remove(mesh);if(mesh.dispose)mesh.dispose();}
