...
assert.ok(html.includes('Frontline')&&html.includes('id="fl-lobby"')&&html.includes('/index_classic.html')&&html.includes('กลับ Lobby'));
...
assert.equal(await fs.access(path.join(root,'sound/Frontline/Arcade_Adventure_bgm.mp3')).then(()=>true,()=>false),false);
const audio=await fs.readFile(path.join(root,'frontline/frontline-audio.js'),'utf8');
...
assert.ok(await fs.access(path.join(root,'sound/Frontline/bgmusic-9295a3b2636fb961.ogg')).then(()=>true,()=>false));
assert.ok(await fs.access(path.join(root,'sound/Frontline/bgmusic-a88cbf606d0bd932.mp3')).then(()=>true,()=>false));
