...
must(indexClassic.includes("css/home-v2.css?v=1510") && indexClassic.includes("js/home-v2.js?v=1510"), "R44 Home theme cache-bust missing from index_classic.html");
...
must(css.includes(".vw2-word-kicker:before{content:\"✦\"") && css.includes("justify-self:end;min-width:54px") && css.includes("font-size:clamp(20px,1.62vw,27px)!important"), "R32 balanced badge/word/reward treatments missing");
must(css.includes(".vw2-feature-title,.vw2-word-ribbon{left:20%!important;right:20%!important;top:10px!important;height:48px!important}") && css.includes(".vw2-feature-title,.vw2-word-ribbon{left:17%!important;right:17%!important;height:45px!important}"), "R32 responsive New Vocab geometry missing");
must(!home.includes("ADMIN PREVIEW · R32 NEW VOCAB · ROYAL BALANCE"), "stale R32 preview badge remains");
must(home.includes("R33 / รอบ 1314") && home.includes("--vw2-r1314-runtime-ready:1") && css.includes("R33 / รอบ 1314") && css.includes("--vw2-r1314-ready:1"), "R33 Classic left-rail lineage markers missing");
must(home.includes("CLASSIC_RAIL_GLYPHS") && home.includes("function classicRailGlyph(actionName, sourceSelector)") && home.includes("querySelector('.rail-ico')"), "R33 does not reuse original Classic rail glyphs");
['💊','🏙️','📈','🏭','🔎','⌨️','🏎️'].forEach(glyph=>must(home.includes(glyph), `R33 original rail glyph fallback missing: ${glyph}`));
...
const adminOnlyWorlds = ["worldSky","worldDrive","worldMoto","worldInvasion","wordship","skirmish","vocabforce"];
...
must(indexClassic.includes('<link rel="preload" as="script" href="js/home-v2.js?v=1510">'), "R43 Primary Home script is not preloaded ahead of dashboard activation");
...
must(css.includes(".vw2-word-ribbon{top:52px!important}") && css.includes(".vw2-word-ribbon{top:43px!important}") && css.includes(".vw2-word-ribbon{top:48px!important}"), "R35 New Word responsive safe-gap geometry missing");
const r35SafeGaps = [
  116 + 5 - 49 + 52 - (63 + 56),  // desktop
