...
/* R6 premium visual fidelity correction guard. */
must(home.includes("Home V2 R6 Premium Visual Fidelity Correction"), "R6 premium presentation stylesheet missing from home-v2.js");
must(css.includes("Home V2 R6 Premium Visual Fidelity Correction"), "R6 premium presentation stylesheet missing from css/home-v2.css");
must(!home.includes("Visual Fidelity Pass R4"), "R4 override tail still present; R6 must remain a coherent presentation stylesheet");
...
must(home.includes(".i-pink{fill:#ff82ba") && home.includes(".i-blue{fill:#65c6f7") && home.includes(".i-line{fill:none;stroke:var(--vw2-line)"), "R6 explicit pastel SVG palette missing; black fallback icons could return");
must(css.includes("Explicit styling prevents browser-default black silhouettes"), "R6 black-icon regression guard missing from CSS");
must(home.includes("grid-template-columns:clamp(210px,31vw,255px)"), "R6 mobile profile width/breathing-room correction missing");
must(home.includes("font-size:15px") && home.includes("font-size:6.9px"), "R6 mobile profile typography upgrade missing");
...
console.log("PASS Home V2 R6 premium visual fidelity + pastel icon system + profile readability + authoritative bindings + complete marker parity + fixed mobile landscape static checks");
