...
/* R11.3 DIRECT FIDELITY CORRECTION + READABILITY RECOVERY + authoritative behavior guard. */
must(home.includes("R11.3 Direct Fidelity Correction + Readability Recovery"), "R11.3 Home V2 JS marker missing");
must(css.includes("R11.3 Direct Fidelity Correction + Readability Recovery") && css.includes("--vw2-r111-ready:1") && css.includes("--vw2-r112-ready:1") && css.includes("--vw2-r113-ready:1"), "R11.3 stylesheet lineage/marker missing");
must(home.includes("--vw2-r111-runtime-ready:1;--vw2-r112-runtime-ready:1;--vw2-r113-runtime-ready:1"), "R11.3 runtime marker missing");
must(home.includes("ADMIN PREVIEW · R11.3 DIRECT FIDELITY + READABILITY"), "R11.3 visible ADMIN PREVIEW marker missing");
must(indexClassic.includes("css/home-v2.css?v=1219") && indexClassic.includes("js/home-v2.js?v=1219"), "R11.3 cache-bust missing from index_classic.html");
...
must(css.includes('r111_kanok_corner.svg') && css.includes('r111_kanok_band.svg'), "R11.3 subtle integrated Kanok panel accents missing");
must(!home.includes('r111_screen_frame.svg') && css.includes('.vw2-screen-frame{display:none!important}'), "R11.3 rejected yellow outer Thai frame still mounted/visible");
...
/* R11.3 top-bar recovery: controlled geometry, readable typography, no number ellipsis. */
must(css.includes('grid-template-rows:clamp(96px,26.5vh,110px)') && css.includes('grid-template-columns:clamp(188px,29vw,270px) minmax(0,1fr) clamp(150px,22vw,220px)'), "R11.3 controlled top-row geometry missing");
must(css.includes('grid-template-columns:minmax(68px,1.25fr) repeat(4,minmax(50px,1fr))') && css.includes('font-variant-numeric:tabular-nums lining-nums'), "R11.3 statistic card width/value hierarchy missing");
must(css.includes('grid-template-columns:repeat(2,minmax(66px,1fr))') && css.includes('.vw2-tool-btn.vw2-classic{grid-column:1/-1') && css.includes('.vw2-tool-btn{min-width:66px'), "R11.3 utility buttons can still collapse into narrow strips");
must(css.includes('.vw2-stat-copy small{font-size:clamp(8.5px') && css.includes('.vw2-stat-copy b{font-size:clamp(11.5px') && css.includes('.vw2-tool-btn b{min-width:0;flex:1;font-size:clamp(8.5px'), "R11.3 top typography floor missing");
must(css.includes('.vw2-feed-copy p{margin:2px 0 0;font-size:clamp(8.5px') && css.includes('.vw2-qbody b{font-size:clamp(8.5px') && css.includes('.vw2-online-name-line>b{min-width:0;max-width:70%;font-size:clamp(9px') && css.includes('.vw2-mode{position:relative') && css.includes('font-size:clamp(7.5px,.60vw,10px)'), "R11.3 panel/bottom readability floor missing");
must(home.includes('function fmtTopValue(v)') && home.includes("setTopNumber('vw2-coins', coins)") && home.includes("setTopNumber('vw2-worth', worth)"), "R11.3 safe compact top-number formatting missing");
must(css.includes('text-overflow:clip;font-variant-numeric:tabular-nums') && !/\.vw2-stat-copy b\{[^}]*text-overflow:ellipsis/.test(css), "R11.3 important top values can still ellipsize");
must(!/font-size:(?:3|4)(?:\.\d+)?px/.test(css), "R11.3 stylesheet reintroduced 3–4px typography collapse");
...
must(css.includes("@media (max-width:1180px),(max-height:520px)") && css.includes("@media (max-width:760px)") && css.includes("@media (max-height:390px)"), "R11.3 mobile landscape stability breakpoints missing");
...
must(css.includes('body.vw2-home-active .toast') && css.includes('top:clamp(110px,29vh,126px)') && css.includes('bottom:auto'), "R11.3 compact safe-zone toast presentation missing");
...
if(fail.length){
  console.error("Home V2 R11.3 validation FAILED:\n- " + fail.join("\n- "));
  process.exit(1);
}
console.log("Home V2 R11.3 validation PASS");
console.log(`Checked ${expectedRail.length} left destinations, ${expectedBottom.length} bottom actions, ${r111Assets.length} retained optimized visual assets, authoritative online/feed/pet/house/profile bindings, and mobile landscape guards.`);
