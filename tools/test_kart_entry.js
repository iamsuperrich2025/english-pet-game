...
const entry=ui.slice(ui.indexOf('async function enterKart3D(){'),ui.indexOf('async function enterPickup3D(){'));
(async()=>{
 ...
 const source=fs.readFileSync('js/f1_3d.js','utf8');assert(source.includes("map:IS_KART?(P.keys&&P.keys.map||NS):'f1'"));assert(source.includes("IS_KART?(P.keys&&P.keys.best||'kartBest'):'f1Best'"));assert(source.includes("IS_KART?(P.keys&&P.keys.ghost||'vwKartGhost'):GHOST_KEY"));checks+=3;
