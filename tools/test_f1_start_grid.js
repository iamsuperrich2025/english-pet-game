...
assert.match(src,/const GRID_GAP_M\s*=\s*8/,'F1 grid positions must be staggered at 8 metre intervals');
...
assert.match(src,/hp:F1_GRID_WIRE\+gridSlot/,
  'Each client must report its reserved grid slot through a NetRoom-supported field');
...
const poseCtx={LINE,sfIdx:100,GRID_N:20,GRID_FRONT_M:18,GRID_GAP_M:8,GRID_SIDE_M:3.2,SAMPLE_M:5,
...
for(let i=0;i<poses.length;i++){
  assert.strictEqual(poses[i].back,18+i*8,`slot ${i} must be at its exact staggered distance`);
  assert.strictEqual(poses[i].side,(i%2?1:-1)*3.2,`slot ${i} must alternate left/right`);
  if(i){
    assert.ok(Math.abs((poses[i-1].z-poses[i].z)-8)<1e-9,`slots ${i-1}/${i} need 8 m longitudinal clearance`);
    const dist=Math.hypot(poses[i].x-poses[i-1].x,poses[i].z-poses[i-1].z);
    assert.ok(dist>10,`adjacent slots ${i-1}/${i} must be farther apart than the F1 footprint`);
  }
}
...
/* Prove hp survives the actual NetRoom cold packet as h and comes back unchanged. */
...
const split=netCtx.NetRoom._split({n:'driver',x:1,z:2,hp:'F1G:7'});
assert.strictEqual(split.cold.h,'F1G:7','grid reservation must enter the sharded cold packet');
assert.strictEqual(netCtx.NetRoom._merge(split.hot,split.cold).hp,'F1G:7','grid reservation must survive NetRoom merge');

console.log('PASS F1 deterministic 8 m staggered multiplayer start grid');
