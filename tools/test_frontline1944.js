...
// Phase 2.1 R11.1 d86056 — deterministic responsive HUD matrix.
(function testR111ResponsiveHudMatrix(){
  ...
  assert(source.includes("const R111_SYSTEM=Object.freeze({id:'P2.1R11.1-d86056'"),'R11.1 system identity present');
  assert(source.includes("function reflowFrontlineResponsiveLayout(reason='layout')"),'R11.1 responsive reflow helper present');
  assert(source.includes("function validateFrontlineLayoutRects(rects,safe)"),'R11.1 geometry validator present');
  assert(source.indexOf('placeSpecialControl(els.autoForward')<source.indexOf('placeSpecialControl(mgEl'),'AUTO controls are laid out before MG');
  assert(source.includes("el.style.setProperty('left',r.left+'px','important')"),'special controls can override R11 important defaults');
  assert(css.includes('--fl44-css-runtime-id:"P2.1R11.1-d86056-CSS"'),'R11.1 CSS identity synchronized');
  assert(html.includes("var FRONTLINE_RUNTIME_ID='P2.1R11.1-d86056';"),'R11.1 HTML loader identity synchronized');
  const start=source.indexOf('const R111_SYSTEM=');
  const end=source.indexOf('const R10_TANK_VEHICLE_MODELS',start);
  assert(start>=0&&end>start,'R11.1 pure responsive definitions are extractable');
  ...
  vm.runInNewContext(`
    ...
    ${defs}
    globalThis.__R111={R111_VIEWPORTS,frontlineResponsiveTier,frontlineResponsiveLayoutModel,validateFrontlineLayoutRects};
  `,sb,{filename:'frontline1944-r111-pure-layout.js'});
  const R=sb.__R111;assert(R&&R.frontlineResponsiveLayoutModel,'R11.1 pure layout model is executable');
  ...
  for(const [w,h] of matrix){const out=R.frontlineResponsiveLayoutModel(w,h);assert(out.report.pass,`${w}x${h} ${out.tier}: overlaps=${out.report.overlaps.join(',')} out=${out.report.outOfBounds.join(',')}`);}
  const critical=R.frontlineResponsiveLayoutModel(1253,553);assert.strictEqual(critical.tier,'standard-phone');assert(critical.report.pass,'1253x553 real-device regression must pass');
  console.log('PASS R11.1 responsive HUD matrix: 19/19 viewports, including 1253x553.');
})();
