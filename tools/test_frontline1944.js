...
const RUNTIME_ID='P1.2.6D-20260902-f02fff';
...
if(html){
  ...
  assert(html.includes("var FRONTLINE_RUNTIME_ID='"+RUNTIME_ID+"'")&&html.includes("var FRONTLINE_JS_URL='__VW_FRONTLINE_JS_URL__'")&&html.includes("var FRONTLINE_CSS_URL='__VW_FRONTLINE_CSS_URL__'")&&html.includes("if(!allowed())return false"),'Frontline assets remain admin-gated and lazy-loaded with Phase 1.2.6D runtime identity plus build-time immutable placeholders');
...
// Phase 1.2.6D delivery/freshness proof. These checks do not alter gameplay semantics.
assert(code.includes("runtimeVersion:'"+RUNTIME_ID+"'")&&code.includes('FRONTLINE_EXECUTING_SCRIPT_URL'),'Frontline executing JavaScript owns a unique Phase 1.2.6D runtime identity');
...
if(buildVersion)assert.strictEqual(buildVersion.version,'2026-09-02.1216','build identity advances for the Phase 1.2.6D delivery rebase');
...
assert(/^P1\.2\.6D-/.test(CFG.runtimeVersion),'Phase 1.2.6D runtime freshness identity marker is explicit');
...
let directAimLayoutsPass=true,legacyGapLayouts=0;
...
for(const spec of aimLayouts126c){
  ...
  if(!pointInRect(floating126c,actualCenter126c.x,actualCenter126c.y))legacyGapLayouts++;
...
accept126c(1,legacyGapLayouts===aimLayouts126c.length,'all representative mobile layouts reproduce the visible-AIM/outside-floating-zone geometry that caused the real-device dead control');
...
console.log('PASS Frontline 1944 Phase 1.2.6D REBASE: existing 57 baseline + 7 diagnostics-cleanup + 9 reverse-diagonal + 8 mobile-AIM recovery checks remain passing; runtime identity, immutable Frontline asset delivery, SW cache isolation and LAN-preview freshness checks pass; accepted DRIVE/AIM/FIRE and Tank Runtime physics remain locked');
