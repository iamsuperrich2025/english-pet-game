...
function runPhase21R7Tests(){
  ...
  check('authoritative 10-tier prices, names and ranges match approved progression',()=>{assert.strictEqual(T.R7_ARSENAL.id,'P2.1R7-d7f119');assert.strictEqual(T.TANK_UPGRADE_LEVELS.length,10);assert.deepStrictEqual(Array.from(T.TANK_UPGRADE_LEVELS,v=>v.cost),[100,250,500,900,1400,2000,2800,4000,6000,10000]);assert.deepStrictEqual(Array.from(T.TANK_UPGRADE_LEVELS,v=>v.rangeMeters),[50,75,100,150,200,300,400,550,750,1000]);assert.strictEqual(T.tankUpgradeSpec(10).name,'LEGEND');assert.strictEqual(T.tankUpgradeSpec(10).mgLabel,'Premium');});
  check('real cannon range solution is level-bound at LV1, LV5 and LV10',()=>{G.enemies=[];G.fortress=null;G.collision={hitSolidOnly(){return null;}};for(const [level,meters] of [[1,50],[5,200],[10,1000]]){const tank=fakeTank(level,0);G.player=tank;G.tankRuntime=null;const sol=T.projectedShotSolution(tank);assert.strictEqual(Math.round(sol.distance*T.R7_ARSENAL.metersPerWorldUnit),meters);assert.strictEqual(T.tankCannonRangeMeters(tank),meters);}});
  ...
  check('approved premium Garage and large rounded-rectangle tank Scope are present',()=>{for(const token of ['TANK GARAGE · R7','fl44-garage-cards','fl44-garage-card','fl44-mg','fl44-scope-capability','width:min(92vw,1440px)!important','height:min(80vh,840px)!important','border-radius:24px!important','RANGE '+"'+capability.rangeMeters+'"+' m','AP READY · MG LV'])assert(source.includes(token),token);assert(source.includes("DEFAULT_STEEL:0x5d6468"));assert(source.includes("G.root.dataset.patchTask=R7_ARSENAL.id"));});
...
function runPhase21R6CTests(){
  ...
  check('Scope presentation remains tank-optic oriented while R7 replaces the circular window',()=>{for(const token of ['fl44-scope-overlay:before','fl44-r7-style','width:min(92vw,1440px)!important','height:min(80vh,840px)!important','border-radius:24px!important','EL +0.0°','· EL '])assert(source.includes(token),token);assert(source.includes("drawer.innerHTML='<b>FRONTLINE 1944 · R7</b>"));assert(source.includes("G.root.dataset.patchTask=R7_ARSENAL.id"));});
...
function runPhase21R5Tests(){
  ...
  check('R5 remains localized and preserves accepted systems',()=>{
    for(const token of ["const SPECIAL_CONTROL_ROLES=Object.freeze(['autoForward','autoReverse','targetLock','scope'])",'GlobalMobileTouchRouter','targetLockHeading()','mobileFirePositionStorageKey','r3PlayerEmbedded()','scopeMode:false','DRIVE','AIM','FIRE'])assert(source.includes(token),token);assert(source.includes("drawer.innerHTML='<b>FRONTLINE 1944 · R7</b>"));assert(source.includes("G.root.dataset.patchTask=R7_ARSENAL.id"));
  });
