...
function runR17CurrentSourceGuards() {
  ...
  check("normal-zombie fling speed is 100x CURRENT R16 tuning", () => {
    ...
    assert.strictEqual(launch / baseline, 100);
  });

  check("open-world fling distance tuning is approximately 100x", () => {
    ...
    assert(ratio >= 99 && ratio <= 101, "expected ~100x distance ratio, got " + ratio);
  });
