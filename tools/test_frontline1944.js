...
function runPhase21R8Tests(){
  ...
  check('Garage exposes separate WEAPONS/ARMOR controls and layered tank preview with fallback',()=>{for(const token of ['TANK GARAGE · R8','fl44-garage-tab-weapon','fl44-garage-tab-armor','fl44-garage-tank-image','fl44-garage-armor-image','WEBP PREVIEW FALLBACK','GARAGE · W',"G.garageMode==='armor'"])assert(source.includes(token),token);assert(source.includes('RANGE 500 m'));assert(source.includes("G.root.dataset.patchTask=R8_SYSTEM.id"));assert(source.includes("DEFAULT_STEEL:0x5d6468"));});
  check('20 lightweight transparent WebP tank assets are wired and present',()=>{assert.strictEqual(T.R8_TANK_WEAPON_ASSETS.length,10);assert.strictEqual(T.R8_TANK_ARMOR_ASSETS.length,11);const paths=[...Array.from(T.R8_TANK_WEAPON_ASSETS),...Array.from(T.R8_TANK_ARMOR_ASSETS).filter(Boolean)];assert.strictEqual(paths.length,20);for(const p of paths){assert(fs.existsSync(p),p);const b=fs.readFileSync(p);assert.strictEqual(b.subarray(0,4).toString('ascii'),'RIFF',p);assert.strictEqual(b.subarray(8,12).toString('ascii'),'WEBP',p);assert(b.length>100&&b.length<250000,p+' size '+b.length);}});
...
function runPhase21R7Tests(){
  ...
  check('approved premium Garage and large rounded-rectangle tank Scope are present',()=>{for(const token of ['TANK GARAGE · R8','fl44-garage-cards','fl44-garage-card','fl44-mg','fl44-scope-capability','width:min(92vw,1440px)!important','height:min(80vh,840px)!important','border-radius:24px!important','RANGE '+"'+capability.rangeMeters.toLocaleString()+'"+' m','AP READY · MG LV'])assert(source.includes(token),token);assert(source.includes("DEFAULT_STEEL:0x5d6468"));assert(source.includes("G.root.dataset.patchTask=R8_SYSTEM.id"));});
...
function runPhase21R6CTests(){
  ...
  check('Scope presentation remains tank-optic oriented while R7 replaces the circular window',()=>{for(const token of ['fl44-scope-overlay:before','fl44-r7-style','width:min(92vw,1440px)!important','height:min(80vh,840px)!important','border-radius:24px!important','EL +0.0°','· EL '])assert(source.includes(token),token);assert(source.includes("drawer.innerHTML='<b>FRONTLINE 1944 · R8</b>"));assert(source.includes("G.root.dataset.patchTask=R8_SYSTEM.id"));});
...
function runPhase21R5Tests(){
  ...
  check('R5 remains localized and preserves accepted systems',()=>{
    ...
    for(const token of ["const SPECIAL_CONTROL_ROLES=Object.freeze(['autoForward','autoReverse','targetLock','scope'])",'GlobalMobileTouchRouter','targetLockHeading()','mobileFirePositionStorageKey','r3PlayerEmbedded()','scopeMode:false','DRIVE','AIM','FIRE'])assert(source.includes(token),token);assert(source.includes("drawer.innerHTML='<b>FRONTLINE 1944 · R8</b>"));assert(source.includes("G.root.dataset.patchTask=R8_SYSTEM.id"));
  });
