...
function runPhase21R9Tests(){
  ...
  check('R9 identity is current while R8 remains the locked baseline',()=>{assert.strictEqual(T.R9_SYSTEM.id,'P2.1R9-fb0971');assert.strictEqual(T.R9_SYSTEM.taskId,'VW-20260906-141911-fb0971');assert.strictEqual(T.R8_SYSTEM.id,'P2.1R8-b62279');assert(source.includes("G.root.dataset.patchTask=R9_SYSTEM.id"));assert(source.includes("G.root.dataset.baselineTask=R8_SYSTEM.id"));assert.deepStrictEqual(Array.from(T.TANK_UPGRADE_LEVELS,v=>v.rangeMeters),[500,750,1000,1500,2000,3000,4000,5500,7500,10000]);assert.strictEqual(T.CFG.playerHP,1000);});
...
function runPhase21R8Tests(){
  ...
  check('Garage exposes separate WEAPONS/ARMOR controls and layered tank preview with fallback',()=>{for(const token of ['TANK GARAGE · R9','fl44-garage-tab-weapon','fl44-garage-tab-armor','fl44-garage-tank-image','fl44-garage-armor-image','LOADING WEBP PREVIEW','GARAGE · W',"G.garageMode==='armor'"])assert(source.includes(token),token);assert(source.includes('RANGE 500 m'));assert(source.includes("G.root.dataset.patchTask=R9_SYSTEM.id"));assert(source.includes("G.root.dataset.baselineTask=R8_SYSTEM.id"));assert(source.includes("DEFAULT_STEEL:0x5d6468"));});
...
function runPhase21R7Tests(){
  ...
  check('approved premium Garage and large rounded-rectangle tank Scope are present',()=>{for(const token of ['TANK GARAGE · R9','fl44-garage-cards','fl44-garage-card','fl44-mg','fl44-scope-capability','width:min(92vw,1440px)!important','height:min(80vh,840px)!important','border-radius:24px!important','RANGE '+"'+capability.rangeMeters.toLocaleString()+'"+' m','AP READY · MG LV'])assert(source.includes(token),token);assert(source.includes("DEFAULT_STEEL:0x5d6468"));assert(source.includes("G.root.dataset.patchTask=R9_SYSTEM.id"));assert(source.includes("G.root.dataset.baselineTask=R8_SYSTEM.id"));});
...
function runPhase21R6CTests(){
  ...
  check('Scope presentation remains tank-optic oriented while R7 replaces the circular window',()=>{for(const token of ['fl44-scope-overlay:before','fl44-r7-style','width:min(92vw,1440px)!important','height:min(80vh,840px)!important','border-radius:24px!important','EL +0.0°','· EL '])assert(source.includes(token),token);assert(source.includes("drawer.innerHTML='<b>FRONTLINE 1944 · R9</b>"));assert(source.includes("G.root.dataset.patchTask=R9_SYSTEM.id"));assert(source.includes("G.root.dataset.baselineTask=R8_SYSTEM.id"));});
...
function runPhase21R5Tests(){
  ...
  check('R5 remains localized and preserves accepted systems',()=>{
    ...
    for(const token of ["const SPECIAL_CONTROL_ROLES=Object.freeze(['autoForward','autoReverse','targetLock','scope'])",'GlobalMobileTouchRouter','targetLockHeading()','mobileFirePositionStorageKey','r3PlayerEmbedded()','scopeMode:false','DRIVE','AIM','FIRE'])assert(source.includes(token),token);assert(source.includes("drawer.innerHTML='<b>FRONTLINE 1944 · R9</b>"));assert(source.includes("G.root.dataset.patchTask=R9_SYSTEM.id"));assert(source.includes("G.root.dataset.baselineTask=R8_SYSTEM.id"));
  });
