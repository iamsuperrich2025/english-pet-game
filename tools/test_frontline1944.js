...
assert(/^P1\.2\.3-/.test(CFG.runtimeVersion),'Phase 1.2.3 mobile-drive-hardened runtime version marker is explicit');
...
// Phase 1.2.3 mobile input delivery: reproduce a phone where setPointerCapture fails/lost capture fires,
// then require window-level pointermove to keep feeding the same MobileTankInputAdapter state.
class FakeControlTarget{
  constructor(rect){this.rect=rect;this.style={};this.listeners=new Map();this.captureAttempts=0;}
  addEventListener(type,fn){if(!this.listeners.has(type))this.listeners.set(type,[]);this.listeners.get(type).push(fn);}
  removeEventListener(type,fn){const a=this.listeners.get(type)||[],i=a.indexOf(fn);if(i>=0)a.splice(i,1);}
  getBoundingClientRect(){return this.rect;}
  setPointerCapture(){this.captureAttempts++;throw new Error('simulated mobile pointer-capture rejection');}
  hasPointerCapture(){return false;}
  releasePointerCapture(){}
  emit(type,event){for(const fn of [...(this.listeners.get(type)||[])])fn(event);}
}
function pointerEvt(pointerId,clientX,clientY){return {pointerId,clientX,clientY,cancelable:true,preventDefault(){this.defaultPrevented=true;},stopPropagation(){this.propagationStopped=true;}};}
function emitWindow(type,event){for(const fn of [...(windowListeners.get(type)||[])])fn(event);}
const rect={left:0,top:0,width:100,height:100},sampleUp=stickVectorFromRect(rect,50,10),sampleDown=stickVectorFromRect(rect,50,90),sampleRight=stickVectorFromRect(rect,90,50);
assert(sampleUp.y<-.99&&sampleDown.y>.99&&sampleRight.x>.99,'mobile stick geometry normalizes up/down/right correctly');
const driveState={x:0,y:0,id:null,active:false,transport:'idle',lastTransport:'none',captured:false,moves:0,lastEventAt:0},fakeStick=new FakeControlTarget(rect),fakeKnob={style:{}};
bindStickElement(fakeStick,fakeKnob,driveState,'drive');
fakeStick.emit('pointerdown',pointerEvt(41,50,50));assert(driveState.active&&driveState.id===41,'DRIVE begins even when pointer capture will fail');assert.strictEqual(driveState.captured,false,'pointer capture failure is non-fatal');
fakeStick.emit('lostpointercapture',pointerEvt(41,50,50));assert(driveState.active,'lostpointercapture must not cancel an active DRIVE session');
emitWindow('pointermove',pointerEvt(41,50,10));const mobileUpFromEvent=mobileCommandFromState(driveState,{active:false},false,null);assert(mobileUpFromEvent.throttle>.99&&Math.abs(mobileUpFromEvent.steering)<1e-12,'window pointermove produces forward mobile throttle without pointer capture');
emitWindow('pointermove',pointerEvt(41,50,90));const mobileDownFromEvent=mobileCommandFromState(driveState,{active:false},false,null);assert(mobileDownFromEvent.throttle<-.99,'window pointermove produces reverse mobile throttle');
emitWindow('pointermove',pointerEvt(41,90,50));const mobileRightFromEvent=mobileCommandFromState(driveState,{active:false},false,null);assert(mobileRightFromEvent.steering>.99&&Math.abs(mobileRightFromEvent.throttle)<1e-12,'window pointermove produces right steering');
emitWindow('pointerup',pointerEvt(41,90,50));assert(!driveState.active&&driveState.id===null&&driveState.x===0&&driveState.y===0,'window pointerup releases and centers DRIVE state');
...
// Event -> mobile command -> authoritative TankRuntime, using the same handler path as the real phone.
const eventDriveState={x:0,y:0,id:null,active:false,transport:'idle',lastTransport:'none',captured:false,moves:0,lastEventAt:0},eventStick=new FakeControlTarget(rect),eventKnob={style:{}};
bindStickElement(eventStick,eventKnob,eventDriveState,'drive');eventStick.emit('pointerdown',pointerEvt(77,50,50));emitWindow('pointermove',pointerEvt(77,50,10));
const eventTank=fakeRuntimeTank(0,40,0),eventTerrain=new TerrainSystem(),eventRuntime=new TankRuntime(eventTank,new CollisionSystem(eventTerrain),eventTerrain),eventCmd=mobileCommandFromState(eventDriveState,{active:false},false,null),eventStartZ=eventRuntime.z;
for(let i=0;i<60;i++)eventRuntime.step(eventCmd,1/60);assert(eventRuntime.z<eventStartZ-1,'real mobile pointer event path drives the authoritative tank forward along the visible default arrow');
emitWindow('pointermove',pointerEvt(77,90,10));const eventTurnCmd=mobileCommandFromState(eventDriveState,{active:false},false,null),beforeTurn=eventRuntime.heading;for(let i=0;i<30;i++)eventRuntime.step(eventTurnCmd,1/60);assert(eventRuntime.heading<beforeTurn-.05,'real mobile pointer event path steers the authoritative hull right with forward throttle');
emitWindow('pointerup',pointerEvt(77,90,10));
...
assert(code.includes("listen(window,'pointermove',move,{passive:false,capture:true})")&&code.includes("listen(window,'touchmove',move,{passive:false,capture:true})")&&code.includes('setPointerCapture')&&code.includes("stick.style.touchAction='none'")&&code.includes("el.style.touchAction='none'"),'drive/aim use window-level pointer/touch drag tracking plus touch-action:none; pointer capture is optional hardening');
assert(code.includes("kind:'tank',phase:'1.2.3'")&&code.includes('__VW_FRONTLINE1944_RUNTIME__'),'loaded Frontline runtime publishes the Phase 1.2.3 authoritative tank-runtime identity marker');
...
console.log('PASS Frontline 1944 Phase 1.2.3: mobile DRIVE survives failed/lost pointer capture via window-level drag tracking; canonical desktop/tank direction and all Phase 1.2 foundation guards remain locked');
