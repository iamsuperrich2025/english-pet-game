...
function runR20CurrentRevisionGuards() {
  ...
  check("SCOPE is manual-only and blocks target-lock/forward-return automation", () => {
    ...
    assert(frontlineSource.includes("else if(!scopeManual&&lockedHeading==null&&!G.aim.active)"));
    assert(frontlineSource.includes("if(next){s.forceHullForward=false;if(!(G.aim&&G.aim.active))holdCurrentTurretOrientation();}"));
    assert(frontlineSource.includes("if(!pose||s.scopeMode||(G.aim&&G.aim.active)||targetLockTargetValid(s.lockedTarget))return cmd"));
  });
  ...
  check("CURRENT patch identity is explicit while R18/R19 remain preserved baselines", () => {
    ...
    assert(frontlineSource.includes("REVISION:R18_SYSTEM.id,ART_REVISION:R19_ART_SYSTEM.id,PATCH_REVISION:R20_SYSTEM.id,PATCH_ART_REVISION:R20_ART_SYSTEM.id"));
  });
