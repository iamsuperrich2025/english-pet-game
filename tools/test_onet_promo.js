...
assert(js.includes('onet-promo-never') && js.includes('state.onetPromoNeverV4=true'), 'never-show-again account preference missing');
assert(js.includes("const KEY_VER = 'v4'") && js.includes('state.onetPromoSeenV4'), 'fresh round-1187 account counter missing');
assert(js.includes("typeof state !== 'undefined'") && !js.includes('window.state &&'), 'Classic top-level state detection is unsafe');
assert(js.includes("typeof Auth !== 'undefined'") && !js.includes('window.Auth'), 'Classic top-level Auth detection is unsafe');
