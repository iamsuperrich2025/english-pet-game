...
try{
 ...
 let role=await roles();console.log('ordinary',role);assert.equal(requests.filter(u=>u.includes('kart-menu')).length,0,'Hidden admin icon must not download for ordinary players');assert(role.classic.hidden&&role.home.hidden);assert.equal((await page.evaluate(()=>enterKart3D())).started,false);
 ...
 const imageSources=await page.locator('img[data-kart-icon]').evaluateAll(es=>es.map(e=>e.getAttribute('src')));assert.equal(imageSources.length,2);assert.equal(new Set(imageSources).size,1);
 ...
 console.log('PASS 12 real lobby/icon checks',root,'page errors',errors);assert.deepEqual(errors,[]);
