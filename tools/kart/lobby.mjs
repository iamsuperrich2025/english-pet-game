...
try{
 ...
 await page.locator('[data-vw2-action="worldKart"]').click();await page.waitForTimeout(100);
 assert.equal(await page.locator('.levelup-overlay .levelup-box').count(),0);
 assert.equal(await page.evaluate(()=>window.__kartStarts),1);assert.equal(await page.evaluate(()=>state.kartTicket),true);
