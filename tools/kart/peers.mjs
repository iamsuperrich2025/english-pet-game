...
const qaUid=new URLSearchParams(location.search).get('uid')||'kart-peer-1';const email=qaUid==='kart-peer-1'?'freddommun@gmail.com':'sumpajitshami@gmail.com';
...
</script><script src="/js/netroom.js"></script><script src="/js/kart-access.js"></script>`;
...
try{
 ...
 for(const [i,p] of [p1,p2].entries()){
  ...
  await p.goto('http://127.0.0.1:17478/__kart?uid=kart-peer-'+(i+1));await p.evaluate(async()=>{await KartAccess.authorize();KartWorld.start();});
 }
 ...
 console.log('PASS 7 two-browser multiplayer checks; real admission + NetRoom + isolated namespace + colour + exit');await fs.writeFile(path.join(work,'kart-peer-results.json'),JSON.stringify({checks:7,passed:true,slots,errors},null,2));
