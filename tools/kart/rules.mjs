...
try{
 ...
 for(const who of [{name:'logged out',t:null,allow:false},{name:'ordinary user',t:token('student@example.com'),allow:false},{name:'unverified admin',t:token('freddommun@gmail.com',false),allow:false},...['freddommun@gmail.com','sumpajitshami@gmail.com','parkerhulk2020@gmail.com'].map(email=>({name:email,t:token(email),allow:true}))]){
 ...
 const row={sec:100,n:'QA',g:'P1',ts:Date.now()};assert((await request('kartRank/qa-user',admin,'PUT',row)).ok);checks++;
 assert.equal((await request('kartRank/qa-user',admin,'PUT',{...row,sec:120})).ok,false);checks++;
 assert((await request('kartRank/qa-user',admin,'PUT',{...row,sec:90})).ok);checks++;
