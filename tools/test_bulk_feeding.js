...
vm.runInContext(read('js/data/pets.js')+'\n'+read('js/data/petshopping.js')+'\nwindow.__feedData={PETS,FOODS,FOOD_SPRITE_URL,foodSpriteHTML,foodSafeForPetMenu,PET_SHOP_FOODS};',ctx);
const {PETS,FOODS,FOOD_SPRITE_URL,foodSafeForPetMenu,PET_SHOP_FOODS}=ctx.window.__feedData;
...
const safeStandard=FOODS.filter(foodSafeForPetMenu);
assert(safeStandard.length===3&&safeStandard.every(f=>!f.human&&!f.badFor),'only three universally safe standard foods may enter animal menus');
assert(PET_SHOP_FOODS.length===12&&PET_SHOP_FOODS.every(foodSafeForPetMenu),'pet shop must contain 9 favorites plus 3 universally safe foods only');
...
assert(ui.includes("!foodSafeForPetMenu(food)"),'legacy direct-feed path must reject harmful food before stock debit');
