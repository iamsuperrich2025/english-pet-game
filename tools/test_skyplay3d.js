...
ok(/state\.skyCharacter = character\.id/.test(ui)&&/saveState\(\)/.test(ui)&&/authPushSave\(false\)/.test(ui)&&/enterBtn\.textContent = `🚪 เข้าเป็น/.test(ui),'pre-entry character choice is visible, saved and cloud-synced before free entry');
ok(/<img src="\$\{esc\(c\.thumb\)\}"[^>]*decoding="async"/.test(sky)&&/<img src="\$\{escapeHTML\(c\.thumb\|\|c\.atlas\)\}"[^>]*decoding="async"/.test(ui)&&/\.sp-character-grid button>img\{[^}]*object-fit:contain/.test(css)&&/\.sky-entry-character-grid button>img\{[^}]*object-fit:contain/.test(css)&&!/--(?:sp|sky-entry)-character-thumb/.test(sky+ui+css),'both character pickers use direct async-decoded lightweight thumbnail images instead of fragile relative CSS-variable URLs or six full atlases');
