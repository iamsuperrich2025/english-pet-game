...
const humanBuild=sky.slice(sky.indexOf('function makePlayerSprite'),sky.indexOf('function makePetChibi'));
...
const atlasPath=path.join(root,'img/characters/sky_soft_cuboid_chibi_8dir.webp'),atlas=fs.readFileSync(atlasPath);
ok(atlas.length>350000&&atlas.slice(0,4).toString()==='RIFF'&&atlas.slice(8,12).toString()==='WEBP','production 8-direction chibi atlas exists as a substantial WebP asset');
ok(/PLAYER_ATLAS_URL='img\/characters\/sky_soft_cuboid_chibi_8dir\.webp'/.test(sky),'Sky renderer points at the approved production character atlas');
ok(/new THREE\.Sprite\(new THREE\.SpriteMaterial/.test(humanBuild)&&/texture\.repeat\.set\(\.25,\.5\)/.test(humanBuild)&&/alphaTest:\.04/.test(humanBuild),'player uses a transparent 4x2 render atlas rather than primitive boxes');
ok(/sky-soft-cuboid-chibi-8dir-v1/.test(humanBuild)&&/styleStandard='docs\/PLAYER_CHARACTER_STYLE\.md'/.test(humanBuild),'render asset declares the mandatory Soft Cuboid Chibi standard');
ok(/fig=makePlayerSprite\(profileAvatar\(\)\)/.test(localPlayerBuild)&&/fig=makePlayerSprite\(/.test(peerPlayerBuild),'local player and every spawned online peer use the same approved renderer');
