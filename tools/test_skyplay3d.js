...
ok(/function openWorldEntryDialog\(w\)\{[\s\S]*startWorldEntry\(w/.test(ui)&&!/data-sky-entry-character/.test(ui)&&!/เข้าเลย!/.test(ui),'Sky and other worlds skip the pre-entry confirm overlay and enter immediately');
ok(/state\.skyCharacter=character\.id/.test(sky)&&/#sp-character/.test(sky)&&/authPushSave\(false\)/.test(sky),'in-world Sky character picker remains available after entry and still cloud-syncs');
ok(/<img src="\$\{esc\(c\.thumb\)\}"[^>]*decoding="async"/.test(sky)&&/\.sp-character-grid button>img\{[^}]*object-fit:contain/.test(css)&&!/--(?:sp|sky-entry)-character-thumb/.test(sky+ui+css)&&!/\.sky-entry-character-grid/.test(css),'in-world Sky picker uses direct async-decoded lightweight thumbnails without a lobby overlay');
