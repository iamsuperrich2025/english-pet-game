...
## Phase 1.2.6F — Mobile AIM Reposition Responsiveness (2026-09-03)

### Source-confirmed cause

The CURRENT `GlobalMobileTouchRouter` contains no multi-second AIM long-press timer. AIM base movement previously happened only when a new AIM pointer was acquired and `recenter()` ran. The supplied deterministic Phase 1.2.6C test also documented that the visible AIM control could sit outside the broader floating AIM region on representative mobile landscape sizes. Together, those two facts explain the real-device symptom: normal turret AIM can work, but a player who holds/drags the visible AIM control to change its position has no deterministic same-gesture reposition path and can experience repeated missed/late reposition attempts.

### Focused correction

- Touching the visible AIM control remains immediate normal AIM; the base no longer jumps merely because the pointer was acquired directly on the control.
- The floating AIM acquisition region now includes the visible AIM center while still respecting the FIRE-side boundary, closing the documented geometry gap.
- Touching a different valid floating AIM location still recenters immediately with no hold delay.
- For intentional same-control reposition, hold the visible AIM control mostly still for `320 ms`, then drag. The first post-threshold drag enters reposition mode and the base follows the finger.
- A normal AIM drag that moves at least `10 px` before the hold threshold locks the gesture to aiming, so a long normal aim does not later become repositioning.
- Repositioning zeroes only the transient AIM stick vector while keeping the last accepted mobile turret heading latched. DRIVE, reverse steering, FIRE, Tank Runtime, cannon/muzzle transforms and projectile direction are unchanged.
- No new `setTimeout` chain is used for AIM reposition activation; activation is based on the pointer gesture's elapsed time on the next movement event.

### Delivery identity evidence

The supplied `dist/index_classic.html` references `/assets/build/js/frontline1944.b0b0f1decd003ecf.js` and `/assets/build/css/frontline1944.790a765fbe901542.css`. Those 16-character aliases match the leading SHA-256 content hashes of the supplied current `js/frontline1944.js` and `css/frontline1944.css`, respectively. This confirms the Task ZIP's source and current DIST loader identity are aligned before this patch. Generated `dist/` output is intentionally not patched; VW Dev Studio must rebuild it after Apply.

The runtime ID string remains `P1.2.6F-20260902-5cc6a0` because the source `index_classic.html` loader file was not supplied as an editable context path. The rebuild will still generate new immutable JS/CSS content-hash aliases from the changed source bytes.

### Deterministic acceptance

The isolated Task ZIP test harness passes after the focused patch, including the existing Tank Runtime / reverse-diagonal / AIM latch / FIRE checks plus 10 new Phase 1.2.6F AIM-responsiveness checks. The loader and vocabulary audits remain full-project checks because `index_classic.html` source and `js/data/f1_vocab.js` were not included in this Task ZIP. Real-device mobile acceptance remains mandatory before production deploy.
