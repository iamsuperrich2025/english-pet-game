...
## Phase 2.1 R14 — Titan Zombie Boss (`VW-20260907-105208-668820`)

- Runtime delivery ID: `P2.1R14-668820`.
- Adds exactly one optional Titan lifecycle entity, kept outside `G.enemies` so it is not counted by or refilled through the normal enemy population manager.
- Visual target: an approximately 50-story battlefield-scale silhouette implemented with 15 shared low-poly meshes, shared materials, no skeletal rig, no ragdoll, no high-resolution Titan texture, and no per-triangle physics.
- Placement: offset into the open-battlefield mission sector, away from the center checkpoint/mission corridor, with a dedicated bounded movement area. Small props are deliberately not queried for Titan movement, while blocked/water terrain is rejected.
- Behavior: `PASSIVE` until a valid player-owned projectile actually damages the Titan. Proximity, scenery events, normal zombies, and enemy projectiles do not provoke it. After provocation it turns slowly, pursues at a heavy pace, telegraphs a fixed stomp impact circle for 1.25 seconds, and then resolves one lightweight player-position hit query.
- Stomp: a confirmed hit bypasses armor and invokes the existing Frontline death/safe-respawn path; no second death system is introduced.
- Boss HP: `9,000`. The accepted cannon remains `24` raw damage and is not nerfed. This is 375 unmodified cannon hits before MG contribution (about 71 seconds of ideal 190 ms cannon cadence), making the Titan substantially tougher than the existing `bossBaseHP:190` while keeping weapon tuning intact. MG uses a Titan-specific `0.65` resistance multiplier and throttled hit FX.
- Targeting: Titan is injected into the existing direct-selection candidate/validation/label/details path, so same-target deselect, cannon target heading, the five-second selected-target timeout, and SCOPE exception remain owned by the accepted targeting system.
- Performance: Titan AI updates at distance-aware intervals, uses a fixed lightweight hit proxy, never performs Titan-vs-all-zombies physics, never runs autonomous Titan-vs-enemy-tank combat, and removes its large visual after a short lightweight defeat animation.
- R13.1 preservation: building scale/palette and fordable-water systems remain unchanged. Titan does not register itself into tank water traversal or rewrite current enemy-tank behavior.

### Source-baseline limitation recorded for R14 acceptance

The supplied CURRENT source is still the R13.1 gameplay baseline with `CFG.enemyCap:10`; it does not contain an actual ~950 Guardian + ~50 Letter Carrier population runtime, and it does not contain a separate on-foot player runtime. R14 therefore declares the requested normal-zombie population contract (`1000`, Titan excluded) and implements the Titan independently, but it does **not** fabricate or silently replace those missing systems. Acceptance item “normal zombie population remains approximately 1,000” requires the separate population implementation to exist in a later/current source before it can be verified as a live count. In this supplied runtime, Titan stomp lethality is fully integrated with the authoritative tank/player death path; no unsupported second on-foot death architecture is invented.


## Phase 2.1 R15 — CURRENT Runtime / Portrait Crowd Restoration (VW-20260907-113226-cf4076)

Runtime delivery identity: `P2.1R15-cf4076`  
Baseline gameplay systems: CURRENT R14 Titan + R13.1 battlefield/water/buildings + accepted tank/aim/targeting rules.

### Why the device still appeared to be R11.2

The supplied CURRENT `js/frontline1944.js` was already newer internally (`P2.1R14-668820`), but the visible compact HUD brand and info drawer still hard-coded `FRONTLINE 1944 · R11.2`. The CURRENT `index_classic.html` also still treated the Frontline route as landscape-only. R15 removes both stale presentation paths, synchronizes JS/CSS/HTML delivery identity to `P2.1R15-cf4076`, and leaves the existing build-time source/hashed-asset identity guard intact.

### Mobile-safe ~1,000 normal zombies

- Logical population is exactly 1,000 normal zombies: **950 Guardian Zombies + 50 Letter Carrier Zombies**.
- Population is arranged as **50 clusters**, each with **1 Letter Carrier + 19 Guardians**.
- Titan remains a separate R14 lifecycle (`G.titan`) and is never inserted into the 1,000-zombie budget or required letter-carrier flow.
- Only the nearest **30 full actors** are active on normal devices; low-FX devices cap full actors at **18**. These actors reuse the accepted normal zombie combat/targeting/ramming behavior and are pooled rather than constantly recreated.
- Mid/far crowd presence is represented by two `THREE.InstancedMesh` batches, capped at 480 visible proxies (260 on low-FX), while the complete 1,000-record logical population remains alive/recyclable.
- Crowd scheduling runs at 0.125 s intervals instead of doing 1,000 heavy AI updates every animation frame. Mid-range records use simplified motion; far records are cheap logical presence.
- The 50 clusters recycle across a bounded sector window (2 behind / 14 ahead), preserving the impression of a continuous army without growing world memory.
- Letter carriers are assigned letters from the current target word. The mission-assist rule guarantees a required next-letter carrier remains reachable; killing the correct carrier uses the same idempotent `claim()`/coin/word-bonus progression primitives without forcing fortress progression. Titan is never needed to complete a word.
- Crowd full actors are excluded from the legacy `CFG.enemyCap` check so fortress defenders and enemy-tank tactical AI still spawn normally.

### Portrait-first Frontline presentation

- Frontline gameplay now treats portrait as the primary presentation (`PORTRAIT_PRIMARY_9_16`).
- The global landscape gate remains for Lobby only; it no longer blocks Frontline in portrait and it no longer requests a landscape lock while Frontline is active.
- Portrait camera uses FOV 62°, height 12.2, chase distance 21.5 and look-ahead 38 world units to expose substantially more forward battlefield depth while retaining the existing cannon/turret transform path. SCOPE still uses its existing dedicated scope FOV/aim rules.
- HUD panels are compacted into a portrait-safe top stack: HP/player, coins/garage, target word, mission objective, boss/Titan status and accessible Exit.
- DRIVE stays lower-left. AIM stays lower-right. FIRE/MG/SCOPE occupy a right-side portrait-safe cluster. AUTO controls remain near DRIVE. Existing orientation-specific saved positions for DRIVE/AIM/FIRE continue to win over defaults.
- Scope becomes a rounded rectangular 92vw × 62vh portrait viewport. Garage cards become a two-column portrait layout.
- Landscape remains a legacy fallback, including the existing **1253×553** regression path.

### Preserved accepted systems

No tank physics rewrite was made. The accepted no-strafe hull-forward movement, reverse/steering, turret behavior, five-second return-to-front behavior, same-target deselect, SCOPE exception, MG/FIRE paths, enemy tank AI, shallow-water traversal, destructible environment and R13.1 buildings remain on their CURRENT paths. R14 Titan stays passive until damaged by the player, then can pursue; its telegraphed stomp remains lethal and its HP remains 9,000.

### Verification

`tools/test_frontline1944.js` adds R15 acceptance for synchronized CURRENT delivery identity, the 950/50/1 population split, pooling/LOD/instancing, mission-carrier completion, portrait camera/layout at common phone viewports, lobby-only landscape gate behavior, build identity guard, Titan separation and legacy 1253×553 support. Full-project Build + Browser Test remains required after importing this patch; no deploy is performed by ChatGPT.
