...
SYSTEM: Vocab Force
PURPOSE: Isolated admin/dev third-person NEX combat vocab prototype: melee zombies, collect letters in order, complete a word, pay central coins.
...
PUBLIC ENTRY POINTS / KEY SYMBOLS: `window.VocabForce.open/close`, `NexAnimationController`, `LetterProgressController`, `VocabularyRoundController`
DEPENDENCIES: Existing NEX Mixamo GLBs under `minigames/vocab-force/characters/next/animations/`, lazily loaded THREE r149 + GLTFLoader, `addCoins`/`saveState`.
NOTES: Hidden admin rail + standalone preview. Not in the 3D city. Normal players never see the button. Pre-game `ui/character-select.js` picks NEX or Lyravyn (PNG portraits only; one playable rig loads after START). BGM `sound/vocab-force/Vocab_force_bgm.mp3` starts on that select/loading screen via `VF.audio.startBgm` (content-hash Cache Storage `vw-assets-content-v1`, Blob ObjectURL loop, `preload=none`, no lobby preload). Night industrial lot look uses JPEG plates in `map/arena-sky.jpg` / `arena-ground.jpg` / `arena-lot.jpg`. The city is an aspect-correct horizon cylinder (`VF._t.skyBand`, 3 wraps) plus a sky dome, not a stretched 360 photo. Three confirmed melee hits on the same zombie inside 1s (`combat/rapid-finisher-tune.js`) hide the mesh and play a pooled burst finisher; letter rewards still use `rewardAt` before the body vanishes. Outside melee, double-tap ATTACK within 1s fires a 3-orb energy burst (`combat/energy-attack-tune.js`); holding ATTACK fills a HUD charge bar and orb size scales 1×→5× with charge (release fires at current size, 3s is full). In-range ATTACK stays melee only. Flagged arena panels (`breakable: true` in `map/prototype-arena.js`) shatter once when a launched zombie hits them at ≥16 speed (`combat/breakable-wall-tune.js` + `effects/breakable-wall-controller.js`); broken AABBs become passable openings and letter rewards still ignore corpse position. One visible NEX body (`nex_walk.glb`) or Lyravyn body (`ly_Run.glb`); extra Mixamo GLBs contribute clips only. 22 NEX Mixamo GLBs and 8 Lyravyn Mixamo GLBs share mixamorig 28 bones. Letter enemies are cloned Mixamo zombies from `characters/zom/animations/` (walk/scream/fall only; one template, clip reuse). HUD `letter-quest-marker.js` points at the next required letter (on-screen bounce, off-screen edge arrow). No dedicated idle/fall/dodge clips for NEX; jump uses `nex_Hop_with_Arms_Raised.glb`. Lyravyn V1 uses held `Running.001` as idle, slowed `Running` as walk, `Elbow_Strike` punch, `Roundhouse_Kick` kick; no block GLB. Dash uses the existing charge/run clip plus physics burst in `combat/dash-tune.js`. Melee feel lives in `combat/combat-tune.js` (hit-stop, knockback levels, camera FOV punch); launched wall/ground crashes live in `combat/secondary-impact-tune.js` + `effects/secondary-impact-controller.js` (pooled shockwave/dust/debris, no gore). `effects/motion-trail-manager.js` owns short punch/kick trails. GLB mesh/texture duplicates are excluded from production copy until Blender consolidation. NEX/Lyravyn are explicit Mixamo exceptions to Soft Cuboid Chibi for this unfinished prototype. Do not deploy as a public game.
