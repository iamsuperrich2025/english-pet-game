...
NOTES: Inspect only the relevant loader/entry zone in `ui.js`. Round 1508: `openWorldEntryDialog` starts the world immediately with no free-play confirm overlay. Sky character picking stays in-world (`#sp-character`).
...
NOTES: Ten original texture-free Soft Cuboid Chibi GLBs with Idle/Walk/Attack clips, forward -Z, feet y=0, height 4.61m. Each cached template shares geometry/materials across peers; four cloned pivots use existing distance-driven walking. Disposed/replaced peers ignore late loads; failures retain the old colored fallback and can retry on re-entry. Mecha's own view remains the selected cockpit. GLBs keep portable PBR; MechaModels bridges their colors to the engine's existing linear-output Phong pipeline. MechaCombatFX owns ten cosmetic styles with ballistic `launch/sync/impact` (Word Fleet gravity G=8.4, mass=1.2, muzzle=62); legacy `fire(from,to)` remains for fx-preview. FIRE/step/warn/impact/explode/enemy/pickup/shield use clipped one-shots from sound/robot/allroboteffect.mp3 under sound/robot/mecha/ (shared by every robot); fire.mp3 kept as primary fire with short variants; mecha BGM `sound/robot/bgm.mp3` starts only on mecha enter via MechaAudio.startBgm (content-hash Cache Storage `vw-assets-content-v1`, Blob ObjectURL loop, preload=none — no lobby preload / no PCM decode); letter hits play Arena fire clip; any ballistic impact (ground or letter) shows volumetric fireball plus stacked shock rings (orange inner → white outer) with white-hot core, billow fire, orange/white ember sparks (no dark spheres), radial streaks, ~1500ms, capacity 512); mothership-style smoke trail puffs in Adventure3D. Wave foes are red-blood zombies (`makeAlien`) that face the player while engaging/attacking; explode spawns red blood particles. Six instanced batches, max 12 active shots; no new network fields, rewards, cadence or heat changes. Daytime open field (no blocking rocks). Coins bank into `state.coins` via `mechaBankCoins`. Public entry (round 1521): Classic / Home V2 / City — `mechaAdminAllowed()` always true; not in ADMIN_ONLY or city admin gate. Ticket/entry dialog still applies. Preview: tools/mecha/preview.html and fx-preview.html.
...
SYSTEM: Vocab Force
PURPOSE: Isolated admin/dev third-person combat vocab hunt: scatter the round's letters across a 10× lot, complete the Shoot Word pool word for 5,000 coins, NetRoom `vforce` up to 14, 5 hunter zombies per player each vocab round.
PRIMARY FILE: `minigames/vocab-force/runtime/vocab-force-runtime.js`
RELATED FILES: `minigames/vocab-force/` (character/animation/combat/enemies/vocabulary/effects/controls/camera/ui/map), `minigames/vocab-force/index.html` (dev preview), `js/ui.js` (`openVocabForce`), `css/lobby.css`, `js/home-v2.js`, `tools/test_vocab_force.js`
PUBLIC ENTRY POINTS / KEY SYMBOLS: `window.VocabForce.open/close`, `NexAnimationController`, `LetterProgressController`, `VocabularyRoundController`, `LetterField.dropAround/applyPeerDrop`, `VocabForceNet.consumeDrops/consumeJumps/consumeOverdrive`, `VocabForceNet.roomHud`, `HealthBar`, `VF.DashTune`, `VF.OverdriveDashTune`, `VF.PowerJumpTune`, `VF.WIN_ACK_SEC`, `VF._t.winnerPhoto`, `VF._t.collusionJudge`, `VF._t.roomHud`, `VF._t.wantHunters`, `VF.SedanController`, `VF.OilTankerController.canGrab/grab/throwBy`, `VF.VehicleGrabController.onKick`
DEPENDENCIES: Existing NEX Mixamo GLBs under `minigames/vocab-force/characters/next/animations/`, lazily loaded THREE r149 + GLTFLoader, `vocabForStudent` (same as Shoot Word), `NetRoom` map `vforce`, `addCoins`/`saveState`.
NOTES: Hidden admin rail + standalone preview. Round 1563 skips zombie GLB load and hunter spawn when a lot has 5+ players (`HUNTER_SKIP_HUMANS`); rooms under 5 still get 5 hunters each. Round 1562 cuts hunters to 5 per player (`HUNTER_OPEN` 2, then trickle) so the lot stays playable; dead still stay dead until `beginRound`. Round 1561 HUD chip shows `ลาน N · X/14 คน` (solo `ลานฝึก · 1 คน`) via `VF._t.roomHud` / `net.roomHud` instead of the tiny NetRoom short status. Round 1560 opens the next Vocab Force lot when a room hits 14 (`roomsCap` 36, `openNewRoom`); players are not stuck in solo training until all 36 lots are full. Round 1559 sets player HP to 1,000. Gun headshots kill instantly; body/limb use `combat/gun-tune.js` (NEX Iron Jackal 175, Lyravyn Aurora Fang 120). Punch 12/90 and kick 20/160 vs zombie/player; block 0.22 vs bites and 0.38 vs players. Zombie bite 80. Heal pad 240 HP/s. Round 1558 surrounds each player with 20 easy (36 HP) hunters: opening ring of 4, then trickle (`HUNTER_SPAWN_SEC` 0.65, ring 12–19m); dead stay dead until `beginRound` / `resetHunterWave`. Live cap 50. Round 1557 dims heal-station glow (`HealPadTune.BRIGHTNESS` 0.48, `GLOW_SCALE` 1.05); previous look is `PREV` (1 / 1.22). Round 1556 raises the heal-station heart to 3.35m and 2.45m wide so it reads from across the lot; refill rules stay 5.4m / 24 HP/s. Round 1555 maps NEX JUMP to `nex_Jump_with_Arms_Open.glb` (`Jump_with_Arms_Open`), matching Lyravyn; `nex_Hop_with_Arms_Raised.glb` stays on disk unused. Round 1554 restyles the heal pad into a compact metallic cyan holographic station (~3.1m platform, energy column, floating heart, orbiting rings) without changing the 5.4m / 24 HP/s refill. Round 1553 flashes a red screen-edge vignette (`hud.hurtFlash`) when a bite drops HP. Round 1552 adds a neon heal pad (`combat/heal-pad-tune.js` + `effects/heal-pad.js`) at (0, 22): holographic heart, cyan rings, stand inside to refill 24 HP/s. Round 1551 keeps Vocab Force BGM looping (`ended` → replay) until the HUD mute switch or exit; it no longer follows lobby `state.sound`. Round 1550 turns Power Jump back on for JUMP+stick: ~12m high and ~64m far (`VERTICAL_SPEED` 24, `HORIZONTAL_SPEED` 32); JUMP alone stays the short hop. Round 1549 holds ATTACK to charge only: full charge stays at 5× until finger release (`energyHoldRelease`); no auto-burst at 3s. Round 1548 detonates energy shots as Arena fireballs (`fx.arenaFire`) plus 2–3 ground shock rings whose `endR` follows charge (`VF._t.energyBlastRadius`: 2.4→7.2m, last orb ×1.15); expire-at-range also detonates. Round 1547 restyles Overdrive fire after Arena วงเพลิง (js/arena-field-visuals.js shader): ember rings + flame tongues (dark red / gold / hot white) instead of dense orange cones. Round 1546 makes JUMP+stick a dash-speed burst (4.6m / 0.16s from DashTune) then a normal hop; JUMP alone stays the short hop and Power Jump stays off. Round 1545 maps Lyravyn JUMP to ly_Jump_with_Arms_Open.glb (Jump_with_Arms_Open); the old 360 spin clip stays on disk unused. Round 1544 keeps energy orbs at chest height (`muzzleHeight` 1.22, level `vy=0`) and shows a movable world `+` while holding ATTACK with stick (`VF._t.energyAimSteer`, `EnergyProjectileManager.setAim`). Round 1543 makes Overdrive fire a dense orange cone blaze on the ground (no white/additive streak). Round 1542 moves the HUD HP capsule to the top-right under sound/exit so it no longer covers the playfield. Round 1541 replaces the Overdrive white dash streak with pooled upright ground flames (`effects/overdrive-fire-trail.js` canvas sprites + floor scorches, 0.32m spacing) and stops the cyan `drive` motion line during Overdrive. Round 1539 turns Power Jump off (`PowerJumpTune.ENABLED=false`) so JUMP is the original short hop even while holding the stick; the unused arc/impact helpers stay in `combat/power-jump-tune.js`. Round 1538 restores the tiled wet asphalt ground (`map/arena-ground.jpg`, 32×32) on the 3D plane; `arena-lot.jpg` stays on the select-screen CSS only. Round 1537 double-taps DASH within 1s (`combat/overdrive-dash-tune.js`) to Overdrive: 5× current dash speed and 5× distance with the same 0.16s duration, swept 0.18m substeps, and a pooled distance-sampled fire trail (`effects/overdrive-fire-trail.js`) that covers only the actual Overdrive path (shortens on walls). Fire starts at Overdrive conversion, not the first DASH; optional enemy burns use a 0.35s per-target cooldown and skip players. Compact `hp` code `Ossph` force-sends start/end; remotes paint a local line and never get particles. Round 1530 scatters the current word's exact letters across a 10× industrial lot (`ARENA_HALF=280`), pays 5,000 coins per completed `vocabForStudent` word (Shoot Word 3–10 filter), and joins NetRoom map `vforce` (14/room, WORLD_CAP 80). Round 1531 completing a word opens a winner overlay (name + celebration + coin burst); each player taps `รับทราบ`, then the next word starts immediately. Round 1532 puts the winner's profile photo on that card (`photoOf`/`photoFetch`, else playable portrait) and auto-acks after `VF.WIN_ACK_SEC` (8s) if nobody taps. Round 1533 extra hunter zombies attack colluding groups: nearby idle players who refuse to contest letters for 6s (`VF._t.collusionJudge`); extras are not trimmed by `HUNTER_FILL` and bite only the local prey. Players bag letters in any order with no cap; dying scatters that life's bag around the corpse as loot (`LetterField.dropAround`) so others can pick them up (packed into existing `hp` ≤28). Original field letters stay taken. Hunter zombies have 36 HP and the 3-hit burst only executes after HP is gone. Peer bodies clone the already-loaded local rig (no second 28MB hero GLB). Hunter zombies lazy-load only when humans < 6 and bite player HP; HUD HP is the bottom capsule (`100 / 100`), world HP bars are zombies only and stay camera-facing (not parented to the mesh yaw). Round 1534 uses the lot plate + neon fill lights + filmic tone mapping. Not in the 3D city. Normal players never see the button. Pre-game `ui/character-select.js` picks NEX or Lyravyn (PNG portraits only; one playable rig loads after START). After START, `ui/loading/nex.png` or `ui/loading/lyravyn.png` fills the boot screen with a slow pan/petal overlay, and a real progress bar tracks character/clip/zombie load. BGM `sound/vocab-force/Vocab_force_bgm.mp3` starts on that select/loading screen via `VF.audio.startBgm` (content-hash Cache Storage `vw-assets-content-v1`, Blob ObjectURL loop, `preload=none`, no lobby preload). Hold-charge uses `chargeEnergy.mp3` and the energy burst uses `energy_beam_shot.mp3` through the same hashed blob cache; clips warm only after Vocab Force opens. Night industrial lot look uses JPEG plates in `map/arena-sky.jpg` / `arena-ground.jpg` / `arena-lot.jpg`. The city is an aspect-correct horizon cylinder (`VF._t.skyBand`, 3 wraps) plus a sky dome, not a stretched 360 photo. Three confirmed melee hits on the same zombie inside 1s (`combat/rapid-finisher-tune.js`) hide the mesh and play a pooled burst finisher; letter rewards still use `rewardAt` before the body vanishes. Outside melee, double-tap ATTACK within 1s fires a 3-orb energy burst (`combat/energy-attack-tune.js`) that travels the full lot diagonal (`VF._t.energyMapRange`); holding ATTACK fills a HUD charge bar and orb size scales 1×→5× with charge (release fires at current size, 3s is full). While charging, `effects/energy-vortex-manager.js` spins a light storm around the player and nearby `loose` crates in `map/prototype-arena.js` orbit like a tornado then scatter and stay where they land; walls and platforms stay fixed. In-range ATTACK stays melee only. Flagged arena panels (`breakable: true` in `map/prototype-arena.js`) shatter once when a launched zombie hits them at ≥16 speed (`combat/breakable-wall-tune.js` + `effects/breakable-wall-controller.js`); broken AABBs become passable openings and letter rewards still ignore corpse position. One visible NEX body (`nex_walk.glb`) or Lyravyn body (`ly_Run.glb`); extra Mixamo GLBs contribute clips only. 22 NEX Mixamo GLBs and 9 Lyravyn Mixamo GLBs share mixamorig 28 bones. Lyravyn jump uses Jump_with_Arms_Open from ly_Jump_with_Arms_Open.glb (not the 360 power spin). Letter enemies are cloned Mixamo zombies from `characters/zom/animations/` (walk/scream/fall only; one template, clip reuse). Fall deaths pin the back to the floor (`_pinBackToFloor`) so the corpse does not float, then `fx.arenaFire` plays an Arena-style fire burst at the contact point using the cached `sound/arena/fire-a6fea31058694941.mp3` clip. HUD `letter-quest-marker.js` points at the next required letter (on-screen bounce, off-screen edge arrow). No dedicated idle/fall/dodge clips for NEX; jump uses `nex_Jump_with_Arms_Open.glb`. Lyravyn V1 uses held `Running.001` as idle, slowed `Running` as walk, `Elbow_Strike` punch, `Roundhouse_Kick` kick; no block GLB. Dash uses the existing charge/run clip plus physics burst in `combat/dash-tune.js`. JUMP alone stays the existing hop; JUMP plus a held camera-relative move (including diagonals, 150ms combo window) starts a directional Power Jump (`combat/power-jump-tune.js`) that travels through the current physics/collision, then a pooled heavy landing (shockwave/dust/debris/crack, local-only camera shake, enemy knockback with no player friendly-fire). Launch/impact sync uses a compact `hp` code (`Pssph`); remotes replay FX locally and never duplicate. Melee feel lives in `combat/combat-tune.js` (hit-stop, knockback levels, camera FOV punch); launched wall/ground crashes live in `combat/secondary-impact-tune.js` + `effects/secondary-impact-controller.js` (pooled shockwave/dust/debris, no gore). `effects/motion-trail-manager.js` owns short punch/kick trails. GLB mesh/texture duplicates are excluded from production copy until Blender consolidation. NEX/Lyravyn are explicit Mixamo exceptions to Soft Cuboid Chibi for this unfinished prototype. Do not deploy as a public game.
VALIDATION: `tools/test_vocab_force.js`, `tools/inspect_vocab_force_glb.py`
