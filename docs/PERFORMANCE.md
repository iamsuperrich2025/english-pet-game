...
### Arena maps, fire and permanent catalogues (round 1390)

- Only the selected full ground texture loads: sky 217,521 / crystal 245,070 / forest 274,132 bytes AVIF, 1024×1024. WebP fallback (326,872 / 356,726 / 431,698 bytes) loads only on failure. Three picker thumbnails total 75,454 bytes; alpha home WebP remains 29,822 bytes. The 1024 variants were compared with 1200 variants in actual 1015/812/1366-wide renders and enlarged detail crops: similar visible floor detail, 15–16% fewer downloaded bytes and 27% fewer decoded pixels. The full art is one horizontal mesh with one texture; a border-sampling shader extends surrounding terrain with no extra download/draw. Eight perimeter columns/crystals use three instanced meshes. The original 48° perspective camera follows the player; screen-fixed DOM plates were removed. Round 1388 restores the original articulated battle actors at scale 1.12, removing their portrait-texture request. Character selection and HUD portraits remain. No PNG runtime assets were added.
...
- Shared budget remains 12 combat zones, 640/48 default particles/meshes or 256/28 low-power. Round 1390's turbulent combustion shader compiles only on first fire use and shares one material across flame/smoke/embers. Per-draw uniforms provide independent erosion phase and opacity. Flat transparent cards use forceSinglePass (avoiding a redundant back-face draw); the original 640/48 or 256/28 pools are unchanged. A normal fire uses 8 flame cards, 4 smoke cards and one ground card (low-power: 5/3/1); GPU work is bounded, with no raster/model/video download and no per-frame geometry allocation. Visual QA covers bright Sky, dark Crystal and Forest through the original camera. On the final isolated three-map fixture, ordinary fire peaks at 120 total draws, sustains at 118 and returns to 105 after expiry (18 textures throughout); shader cards add no GPU textures. Desktop fixture counters are not physical-phone FPS measurements. DPR/shadow policy unchanged. Room presence frequency unchanged; picker occupancy polls only while open.
- Desktop Edge sample after restoring the following camera and world ground (round 1389): 106–109 calls, about 12.6–12.9k triangles, 19 textures during ordinary map fixtures. The round-1388 fixed camera used 107 calls / 11.9k triangles / 18 textures. The new ground adds one GPU texture while replacing the DOM plate; compact instanced scenery supplies depth. The round-1389 artificial 50-spell sequence remains inside 640/48 pools (194 calls / 29.8k triangles in its final sample). These are scene costs from isolated desktop fixtures, not physical-phone FPS or production multiplayer measurements.
...
- Round 1384: six static pedestal cores become dynamic letter crystals (up to six concurrent station gems); radius18 MEGA adds fixed shared ring geometry to the existing FX pool, without raising caps or adding raster/model assets. Desktop Edge spell snapshots:127–146 calls,17–20k triangles; not a physical-phone FPS measurement. `tools/test_arena_crystals.cjs` validates low-power28/default48 mesh limits and three-pulse damage only inside the visible ring.
