...
|---|---|---|---|
| P0 | Drive, Haunted Hotel, and Soccer in Adventure3D | Shared `BLOCK_AVATARS` / `makeBlockFigure()` uses hard `BoxGeometry` limbs and body | Create a reusable Soft Cuboid Chibi figure path with softened forms and chibi proportions. Keep a separate legacy path for exempt original Adventure mode. |
