...
| P0 ✅ | Mecha online peers | Ten original rounded chibi GLBs from the robot image collection, selected via existing m_01..m_10 payload | Conforming since round 1399; named cloned limb pivots, shared geometry/materials, 10 draws per robot. Preserve the first-person cockpit and existing admission rules. |
...
| P0 ✅ | Cute Word Skirmish | Soft Cuboid Chibi 3D toy-gun avatars with hip/knee/shoulder/elbow/wrist pivots; gun parented to the right wrist; crouch/kneel/prone/dodge stances | Conforming; keep head/body hitboxes, chibi proportions, the bent-elbow aim pose, and the Free Fire-style stance set. |
